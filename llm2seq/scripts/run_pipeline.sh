...
REPO_ID="${HF_REPO_ID:-}"
WIKI_DIR="${WIKI_DIR:-llm2seq/wikilingua}"
DATA_DIR="${DATA_DIR:-llm2seq/data/processed}"
...
"${PYTHON_BIN}" llm2seq/scripts/prepare_wikilingua_json.py \
  --input_dir "${WIKI_DIR}" \
  --output_dir "${DATA_DIR}" \
  --max_train -1 \
  --max_eval -1 \
...
if [[ -n "${REPO_ID}" ]]; then
  ...
  "${PYTHON_BIN}" llm2seq/scripts/push_to_hf.py \
    --repo_id "${REPO_ID}" \
    "${PUSH_FOLDERS[@]}" \
