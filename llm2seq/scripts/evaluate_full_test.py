...
LLM2SEQ_ROOT = Path(__file__).resolve().parents[1]
...
def load_env_file() -> None:
    ...
    if not env_file.is_absolute():
        env_file = LLM2SEQ_ROOT.parent / env_file
...
def hf_download_checkpoint(
    ...
    if not cache_dir.is_absolute():
        cache_dir = LLM2SEQ_ROOT.parent / cache_dir
...
def main() -> None:
    ...
    parser.add_argument("--output_dir", default="llm2seq/eval_outputs")
    parser.add_argument(
        "--decode_mode",
    ...
    test_file = Path(args.test_file or raw_cfg["data"].get("test_file", "llm2seq/data/processed/test.jsonl"))
    ...
    if args.base_checkpoint:
        ...
        if not base_checkpoint_path.exists():
            base_stage = infer_stage_from_path(str(base_checkpoint_path)) or "phase2_lora_encoder"
    ...
    if (
        "phase3_mtp_self_distill" in stage
