...
if cuda_build and not cuda_build.startswith("12.4"):
    raise SystemExit(
        f"This LLM2Seq setup expects a CUDA 12.4 PyTorch wheel, but torch was built with CUDA {cuda_build}. "
...
if torch.cuda.get_device_capability(0)[0] >= 9 and "sm_90" not in torch.cuda.get_arch_list():
    raise SystemExit("Installed PyTorch wheel does not advertise sm_90 support for H100/LLM2Seq-class GPUs.")
PY
