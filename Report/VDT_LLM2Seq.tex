...
On WikiLingua, we also study inference speed with self-distilled Multi-Token Prediction (MTP).
...
The WikiLingua MTP results also show that speedups should be measured by real throughput, not only by accepted tokens.
\end{abstract}

\noindent\textbf{Keywords:} abstractive summarization; encoder-decoder models; LLM source encoders; adapters; multi-token prediction; Vietnamese WikiLingua; LR-Sum
...
Encoder-decoder models fit this structure.
...
They generate well, but they mix the source and target in one causal stream.
This makes the source-summary boundary less clear.

\subsection{Problem Statement}

This report asks a simple question.
Can pretrained LLM source encoders be reused inside an encoder-decoder summarizer?
...
But summarization still needs a stable bridge from source states to decoder cross-attention.
...
It is less natural for source-grounded summarization.
Summaries often need names, numbers, steps, and local details.
...
It is an encoder-decoder summarizer built from repurposed LLM source encoders.
\begin{itemize}
    \item We adapt LLM source encoders for Vietnamese abstractive summarization on WikiLingua and LR-Sum.
    \item We build an adapter that turns LLM states into decoder memory.
    ...
    \item On WikiLingua, we show that MTP must be judged by real throughput, not only accepted tokens.
\end{itemize}
...
It asks whether LLM source states can become useful decoder memory for summarization.
...
First, the system uses a pretrained LLM-based source encoder.
...
The adapter section later expands this memory as learned global tokens plus adapted token-level source states.
In Figure~\ref{fig:architecture}, \textit{memory adapter} refers to this whole bridge.
It is not a separate summarizer.
It converts LLM hidden states into the memory that the decoder reads through cross-attention.
...
The overall adapter composition is specific to LLM2Seq.
However, its individual mechanisms are not claimed as new neural primitives.
Some are direct applications of standard Transformer components.
Others adapt layer mixing, gated updates, feature modulation, and learned prompt tokens to the source-memory interface.
...
LLM2Seq does not claim this gating equation as a new neural block.
Instead, we apply it to a specific interface problem.
It bridges a frozen or lightly adapted LLM source encoder and a smaller cross-attentive decoder.
...
LLM2Seq adapts this idea to token-level source memory.
The goal is not to remove tokens, but to rescale their representations before decoder cross-attention.
\begin{equation}
...
In those methods, trainable continuous tokens condition a language model.
LLM2Seq uses them differently.
They are decoder-visible source-memory slots trained for summarization, not a prompt prepended to the decoder input.
\begin{equation}
M=[G_m;\,\tilde{R}].
\end{equation}
...
\end{equation}
Here $\gamma(u)$ ramps in the KL term during training.
...
We report F1 for all ROUGE scores.
...
This matters because Vietnamese word segmentation can change overlap scores.
...
On WikiLingua, the Qwen-based LLM2Seq system achieves 54.01 ROUGE-1, 20.83 ROUGE-2, 31.83 ROUGE-L, and 73.42 BERTScore F1.
The WikiLingua result shows that a pretrained LLM-based source encoder can provide useful cross-attention memory for an encoder-decoder summarizer.
...
This shows that the encoder configuration matters.

On WikiLingua, LLM2Seq-Qwen is close to T5Gemma on ROUGE-1.
It is lower on ROUGE-2 and ROUGE-L.
This supports the LLM2Seq design, but it does not beat T5Gemma.
The result also depends on the encoder choice and output length.
...
\midrule
\multirow{3}{*}{WikiLingua} & LLM2Seq-Llama & 48.36 & 15.54 & 29.05 & \textbf{74.08} & 72.11 & 73.02 & 29.10 & 5.46 \\
...
\midrule
\multirow{3}{*}{LR-Sum} & LLM2Seq-Llama & 50.49 & 12.08 & 29.41 & 70.74 & 69.26 & 69.98 & 26.78 & 4.18 \\
 & LLM2Seq-Qwen & 51.59 & 15.58 & 31.21 & 72.24 & 71.04 & 71.62 & 29.50 & 6.92 \\
 & T5Gemma & \textbf{58.79} & \textbf{27.34} & \textbf{39.86} & \textbf{74.13} & \textbf{75.24} & \textbf{74.65} & 33.73 & 14.18 \\
\bottomrule
\end{tabular}
...
T5Gemma is still much stronger on ROUGE-2 and ROUGE-L.
The LR-Sum LLM2Seq outputs are fluent, but they can choose wrong entities or events.
This lowers n-gram overlap with the reference.
...
This gives a trade-off on WikiLingua.
Qwen improves quality and length control.
Llama is faster, but it under-generates.
...
It is a conservative estimate from the evaluation logs.
...
LLM2Seq does not optimize compression ratio.
It studies whether token-level LLM states can work as source memory for summarization.
...
This WikiLingua example matches the table.
...
The demo is for inspection.
The main evidence comes from the evaluation logs.

\section{Discussion}
\label{sec:discussion}

LLM2Seq shows that LLM-based source encoders can be reused inside an encoder-decoder summarizer.
On WikiLingua, the Qwen variant is close to T5Gemma on ROUGE-1.
It also produces shorter summaries.
This supports the design.
It does not show that LLM2Seq is better than standard encoder-decoder baselines.
On WikiLingua, the Qwen-based LLM2Seq variant achieves higher ROUGE than the Llama-based variant.
This should be read as a system-level comparison.
The tokenizer, vocabulary, and representation space also change.
For Qwen on WikiLingua, MTP gives 2.30 tokens per step but only 1.17x real speedup.
MTP helps only when accepted tokens pay for verification.
Token-level memory is useful when the summary needs local source evidence.
This differs from fixed-ratio context compression \citep{li2026contextcompression}.
...
First, the main experiments use Vietnamese WikiLingua \citep{ladhak2020wikilingua}.
LR-Sum \citep{palen-michel-lignos-2023-lr} is only an additional Phase 1--2 quality evaluation.
The result may not transfer to dialogue, legal text, scientific text, or other summarization domains.
