...
CONFIG="${2:-${PHASE2_CONFIG:-llm2seq_final/configs/phase2_lora_encoder_4096.yaml}}"
...
if [[ ! -f "${PHASE1_CKPT}" ]]; then
  ...
  echo "Usage: bash llm2seq_final/scripts/train_phase2.sh /path/to/phase1/best.pt [config]" >&2
  exit 1
fi
