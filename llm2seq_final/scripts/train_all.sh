...
bash llm2seq_final/scripts/train_phase1.sh "${PHASE1_CONFIG}"
if [[ "${RUN_PHASE_EVAL}" == "true" ]]; then
  bash llm2seq_final/scripts/evaluate_phase.sh phase1_main "${PHASE1_CONFIG}" "${PHASE1_DIR}/best.pt" "${PHASE1_EVAL_DIR}" autoregressive
fi

bash llm2seq_final/scripts/train_phase2.sh "${PHASE1_DIR}/best.pt" "${PHASE2_CONFIG}"
if [[ "${RUN_PHASE_EVAL}" == "true" ]]; then
  bash llm2seq_final/scripts/evaluate_phase.sh phase2_main "${PHASE2_CONFIG}" "${PHASE2_DIR}/best.pt" "${PHASE2_EVAL_DIR}" autoregressive
fi

bash llm2seq_final/scripts/train_phase3.sh "${PHASE2_DIR}/best.pt" "${PHASE3_CONFIG}"
if [[ "${RUN_PHASE_EVAL}" == "true" ]]; then
  bash llm2seq_final/scripts/evaluate_phase.sh phase3_main "${PHASE3_CONFIG}" "${PHASE3_DIR}/best.pt" "${PHASE3_EVAL_DIR}" autoregressive "${PHASE2_DIR}/best.pt"
  bash llm2seq_final/scripts/evaluate_phase.sh phase3_mtp "${PHASE3_CONFIG}" "${PHASE3_DIR}/best.pt" "${PHASE3_MTP_EVAL_DIR}" mtp_verified "${PHASE2_DIR}/best.pt"
