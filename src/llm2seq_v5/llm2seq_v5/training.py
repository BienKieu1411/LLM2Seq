...
def validation_loss(
    ...
    metric_names = (
        ...
        # Representation-only diagnostic. Causal prefix use is measured by the
        # paired intervention below, not inferred from prefix self-drift.
        "prefix_drift_ratio",
        "prefix_swap_nll_gap",
    ...
    for batch in loader:
        ...
        for name in totals:
            ...
            if value is not None:
                # Most forward diagnostics are batch means. valid_slots is the
                # number of active alignment slots summed over the batch, so
                # multiplying it by batch_size would over-count it again.
                multiplier = 1 if name == "response_alignment_valid_slots" else batch_size
                totals[name] += float(value.detach().float()) * multiplier
        examples += batch_size
        ...
        for batch_index, batch in enumerate(loader, start=1):
            ...
            if local_step % log_every == 0 or local_step == total_steps:
                ...
                payload = {
                    ...
                    "prefix_swap_nll_gap": running.get("prefix_swap_nll_gap", 0.0) / divisor,
                    "prefix_swap_accuracy": running.get("prefix_swap_accuracy", 0.0) / divisor,
