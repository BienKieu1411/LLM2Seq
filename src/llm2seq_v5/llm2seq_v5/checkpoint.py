"""One complete, atomic last.pt checkpoint for LLM2Seq-v5."""
...
def save_last_checkpoint(
    ...
    if path.name != "last.pt":
        raise ValueError("LLM2Seq-v5 only writes the canonical last.pt checkpoint")
    ...
    payload = {
        ...
        # Count unique nn.Parameters, not state_dict entries: tied embeddings
        # may appear under multiple keys in the serialized tensor state.
        "parameter_element_count": sum(parameter.numel() for parameter in model.parameters()),
        "parameter_summary": (model.parameter_summary() if hasattr(model, "parameter_summary") else None),
    }
