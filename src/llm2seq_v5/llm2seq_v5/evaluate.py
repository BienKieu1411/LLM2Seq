...
def evaluate(
    ...
    if checkpoint.name != "last.pt":
        raise ValueError("LLM2Seq-v5 evaluates last.pt only")
    config = load_config(config_path)
    if split not in {"validation", "test"}:
        raise ValueError("split must be 'validation' or 'test'")
    limits = config.get("limits", {})
    configured_limit = int(limits.get(f"max_{split}_examples", 0))
    if split == "test":
        if int(limits.get("max_train_examples", 0)) > 0:
            raise ValueError("Test evaluation is disabled for smoke/pilot subset-training configs")
        if int(max_samples) > 0 or configured_limit > 0:
            raise ValueError("The locked test split must be evaluated in full; partial test evaluation is disabled")
    device = _device()
    ...
    current_split_fingerprint = dataset_fingerprint(
        split_file,
        effective_limit,
        subset_strategy=subset_strategy,
        subset_seed=subset_seed,
    )
    checkpoint_split_fingerprint = payload.get("data_manifest", {}).get(split, {})
    checkpoint_split_matches_current = int(checkpoint_split_fingerprint.get("num_examples", -1)) == int(
        current_split_fingerprint["num_examples"]
    ) and str(checkpoint_split_fingerprint.get("sha256", "")) == str(current_split_fingerprint["sha256"])
    ...
    for start in range(0, len(rows), batch_size):
        ...
        if model.phrase_pointer is not None:
            batch_modes, batch_phrase_observations = model.phrase_pointer.generation_statistics()
            if batch_phrase_observations > 0:
                weighted_modes = batch_modes.detach().float().cpu() * batch_phrase_observations
                phrase_mode_sum = weighted_modes if phrase_mode_sum is None else phrase_mode_sum + weighted_modes
                phrase_mode_observations += batch_phrase_observations
        if device.type == "cuda":
            torch.cuda.synchronize()
    ...
    metrics: Dict[str, Any] = {
        ...
        **_diagnostics(predictions, references, sources),
        ...
        "decoder_revision": config["model"].get("decoder_revision"),
        ...
        "summary_planner": bool(config.get("adapter", {}).get("use_summary_planner", True)),
        ...
        "response_alignment_weight": float(config.get("objectives", {}).get("response_alignment_weight", 0.0)),
        "phrase_pointer_enabled": model.phrase_pointer is not None,
        "phrase_pointer_parameters": int(parameter_summary.get("phrase_pointer", 0)),
        "phrase_pointer_config": dict(config.get("phrase_pointer", {})),
        "phrase_objectives": {
            name: float(config.get("objectives", {}).get(name, 0.0))
            for name in (
                "phrase_mixture_weight",
                "phrase_copy_weight",
                "phrase_continue_weight",
                "phrase_label_weight",
                "phrase_coverage_weight",
            )
        },
        ...
        "label_smoothing": float(config.get("objectives", {}).get("label_smoothing", 0.0)),
        "generation_mode": "greedy",
        "generation_min_new_tokens": int(generation.get("min_new_tokens", 16)),
        "generation_max_new_tokens": int(generation.get("max_new_tokens", 256)),
        "generation_num_beams": 1,
        "generation_do_sample": False,
        "generation_repetition_penalty": float(generation.get("repetition_penalty", 1.05)),
        "generation_no_repeat_ngram_size": int(generation.get("no_repeat_ngram_size", 3)),
        "generation": {
            "max_new_tokens": int(generation.get("max_new_tokens", 256)),
            "min_new_tokens": int(generation.get("min_new_tokens", 16)),
            "num_beams": 1,
        ...
        "max_source_length": int(data.get("max_source_length", 0)),
        "max_target_length": int(data.get("max_target_length", 0)),
        ...
        "evaluation_data_fingerprint": current_split_fingerprint,
        "checkpoint_evaluation_data_fingerprint": checkpoint_split_fingerprint,
        "checkpoint_evaluation_data_matches_current": checkpoint_split_matches_current,
    }
    if split == "test":
        metrics["test_data_fingerprint"] = current_split_fingerprint
        metrics["checkpoint_test_data_fingerprint"] = checkpoint_split_fingerprint
        metrics["checkpoint_test_matches_current"] = checkpoint_split_matches_current
    if phrase_mode_sum is not None and phrase_mode_observations > 0:
        phrase_modes = phrase_mode_sum / phrase_mode_observations
        metrics["phrase_generation_modes"] = {
            name: round(float(phrase_modes[index]), 8)
            for index, name in enumerate(("generate", "new_span", "continue_span"))
        }
        metrics["phrase_generation_mode_observations"] = phrase_mode_observations
    ...
    locked_test_matches = (
        split == "test"
        and int(locked_test.get("num_examples", -1)) == int(current_split_fingerprint["num_examples"])
        and str(locked_test.get("sha256", "")) == str(current_split_fingerprint["sha256"])
    )
    ...
    if all(name in diagnostic_benchmark for name in ("rouge1", "rouge2", "rougeL")):
        ...
        comparable = split == "test" and len(rows) == expected_examples and locked_test_matches
    ...
    if all(name in paper_benchmark for name in ("rouge1", "rouge2", "rougeL")):
        ...
        metrics["paper_comparison_status"] = (
            "pending_perl_rouge155" if split == "test" and locked_test_matches else "not_full_locked_test"
        )
    ...
    metrics["predictions_sha256"] = _sha256(output)
    metrics["resolved_config_sha256"] = _sha256(Path(config_path).resolve())
...
def main() -> None:
    ...
    evaluate(args.config, args.checkpoint, args.output, args.max_samples, args.split)
