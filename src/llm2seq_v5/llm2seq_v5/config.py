"""Configuration loading and validation for LLM2Seq-v5."""
...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    phrase_pointer = config.get("phrase_pointer", {})
    ...
    for field in (
        "encoder_revision",
        "decoder_revision",
        "encoder_tokenizer_revision",
        "decoder_tokenizer_revision",
    ):
        if field in model and model[field] is not None and not str(model[field]).strip():
            raise ValueError(f"model.{field} cannot be empty")
    ...
    if bool(adapter.get("use_summary_planner", True)):
        ...
        if float(objectives.get("response_alignment_weight", 0.0)) != 0.0:
    ...
    response_alignment_weight = float(objectives.get("response_alignment_weight", 0.0))
    ...
    phrase_enabled = bool(phrase_pointer.get("enabled", False))
    if "enabled" in phrase_pointer and not isinstance(phrase_pointer["enabled"], bool):
        raise ValueError("phrase_pointer.enabled must be boolean")
    if int(phrase_pointer.get("rank", 128)) <= 0:
        raise ValueError("phrase_pointer.rank must be positive")
    if int(phrase_pointer.get("phrase_hidden_size", 256)) <= 0:
        raise ValueError("phrase_pointer.phrase_hidden_size must be positive")
    pointer_dropout = float(phrase_pointer.get("dropout", 0.1))
    if not 0.0 <= pointer_dropout < 1.0:
        raise ValueError("phrase_pointer.dropout must be in [0, 1)")
    if float(phrase_pointer.get("phrase_bias_scale", 0.5)) < 0.0:
        raise ValueError("phrase_pointer.phrase_bias_scale must be non-negative")
    if float(phrase_pointer.get("continuation_strength", 1.0)) < 0.0:
        raise ValueError("phrase_pointer.continuation_strength must be non-negative")
    generate_init = float(phrase_pointer.get("generate_probability_init", 0.98))
    if not 0.0 < generate_init < 1.0:
        raise ValueError("phrase_pointer.generate_probability_init must be in (0, 1)")
    for field in ("use_continuation", "detach_recurrent_state"):
        if field in phrase_pointer and not isinstance(phrase_pointer[field], bool):
            raise ValueError(f"phrase_pointer.{field} must be boolean")
    phrase_orders = [int(order) for order in data.get("phrase_orders", [2, 3, 4])]
    if phrase_orders != [2, 3, 4]:
        raise ValueError("data.phrase_orders must be exactly [2, 3, 4]")
    mixture_weight = float(objectives.get("phrase_mixture_weight", 1.0 if phrase_enabled else 0.0))
    if not 0.0 <= mixture_weight <= 1.0:
        raise ValueError("objectives.phrase_mixture_weight must be in [0, 1]")
    phrase_weight_defaults = {
        "phrase_copy_weight": 0.10 if phrase_enabled else 0.0,
        "phrase_continue_weight": 0.10 if phrase_enabled else 0.0,
        "phrase_label_weight": 0.05 if phrase_enabled else 0.0,
        "phrase_coverage_weight": 0.02 if phrase_enabled else 0.0,
    }
    for field, default in phrase_weight_defaults.items():
        if float(objectives.get(field, default)) < 0.0:
            raise ValueError(f"objectives.{field} must be non-negative")
    if not phrase_enabled and (
        mixture_weight != 0.0
        or any(float(objectives.get(field, default)) != 0.0 for field, default in phrase_weight_defaults.items())
    ):
        raise ValueError("all phrase objective weights must be zero when phrase_pointer.enabled=false")
    ...
    if str(data.get("subset_strategy", "head")) not in {"head", "hashed"}:
        raise ValueError("data.subset_strategy must be 'head' or 'hashed'")
    ...
    if bool(checkpoint.get("save_best", False)):
        raise ValueError("LLM2Seq-v5 intentionally supports last.pt only; set checkpoint.save_best=false")
    if bool(checkpoint.get("save_each_epoch", False)):
        raise ValueError("LLM2Seq-v5 intentionally avoids epoch checkpoints")
