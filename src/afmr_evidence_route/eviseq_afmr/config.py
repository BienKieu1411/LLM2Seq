...
def validate_config(config: dict[str, Any], *, train_only: bool = False) -> None:
    ...
    router = decoder.get("evidence_router", {})
    if not isinstance(router, dict):
        raise ValueError("decoder.evidence_router must be a mapping")
    _check_keys(
        router,
        {
            "enabled",
        ...
        "decoder.evidence_router",
    )
    if not isinstance(router.get("enabled", False), bool):
        raise ValueError("decoder.evidence_router.enabled must be a boolean")
    if router.get("enabled", False):
        if not copy_config.get("enabled", False):
            raise ValueError("shared evidence routing requires grounded_copy.enabled=true")
        if int(router.get("region_width", 32)) <= 0 or int(router.get("last_n_layers", 2)) <= 0:
            raise ValueError("evidence region_width and last_n_layers must be positive")
        for branch in ("cross", "copy"):
            init = float(router.get(f"{branch}_strength_init", 0.10))
            maximum = float(router.get(f"{branch}_strength_max", 0.50))
            if not 0 < init < maximum:
                raise ValueError(f"evidence {branch} strength must satisfy 0 < init < max")
