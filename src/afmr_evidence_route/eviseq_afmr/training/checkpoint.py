...
def architecture_spec(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if router.get("enabled", False):
        spec["evidence_router"] = {
            "region_width": int(router.get("region_width", 32)),
            "last_n_layers": int(router.get("last_n_layers", 2)),
            "cross_strength_max": float(router.get("cross_strength_max", 0.50)),
            "copy_strength_max": float(router.get("copy_strength_max", 0.50)),
        }
    return spec
