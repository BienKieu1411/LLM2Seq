...
from .evidence_route import pool_source_regions
...
class EviSeqAFMR(nn.Module):
    ...
    def encode_source(
        ...
        **copy_inputs: torch.Tensor,
        ...
        router_config = self.config["decoder"].get("evidence_router", {})
        if router_config.get("enabled", False):
            bridge.region_states, bridge.source_region_ids = pool_source_regions(
                bridge.memory,
                bridge.content_mask,
                int(router_config.get("region_width", 32)),
            )
