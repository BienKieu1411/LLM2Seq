...
def test_task_recipes_match_t5gemma_protocol(afmr_name, t5_name, total_epochs):
    ...
    t5_config = root / "src" / "T5Gemma" / "configs" / t5_name
    if not t5_config.exists():
        pytest.skip(f"T5Gemma reference config is unavailable locally: {t5_name}")
    t5 = yaml.safe_load(t5_config.read_text(encoding="utf-8"))
