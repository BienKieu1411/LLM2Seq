...
if [[ -n "${AFMR_OUTPUT_DIR:-}" ]]; then
  ...
  RUN_ROOT="${ROOT}/runs/afmr/pubmed_pair_${AFMR_ARCHITECTURE}_query_regions_evidence_prior_${COPY_VARIANT}"
elif [[ "${BRIDGE_VARIANT}" == afmr && "${AFMR_REGION_QUERY}" == true ]]; then
  RUN_ROOT="${ROOT}/runs/afmr/pubmed_pair_${AFMR_ARCHITECTURE}_query_regions_${COPY_VARIANT}"
