...
def test_direct_projection_cannot_silently_keep_region_route():
    config = _config()
    ...
    try:
        ...
        assert "region_query requires" in str(error)
    else:
        raise AssertionError("direct projection must disable the regional bridge explicitly")
