...
class RegionQueryRead(nn.Module):
    ...
    The scalar residual gate is bounded by ``gate_max`` and initialized at
    ``gate_init``.
    """
    ...
    def __init__(
        ...
        if not 0.0 < float(gate_init) < float(gate_max) <= 1.0:
            raise ValueError("gate_init must lie strictly between zero and gate_max <= 1")
    ...
    def forward(
        ...
        """Return a ``[B,T,H]`` bounded residual, without a ``[B,T,S]`` map."""
        ...
        raw_delta = self.out_proj(attended).float()
        ...
        # decoder query, while keeping a smooth, nonzero output gradient at the
        # zero-initialized projection.  Detach only the reference radius.
        query_rms = query_states.float().detach().square().mean(dim=-1, keepdim=True).sqrt()
        ...
        return delta.to(query_states.dtype)
