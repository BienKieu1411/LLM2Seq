...
def region_query_settings(architecture: dict[str, Any]) -> dict[str, Any]:
    ...
    if not 0 < settings["gate_init"] < settings["gate_max"] <= 1:
        raise ValueError("region_query gate must satisfy 0 < init < max <= 1")
    if settings["enabled"] and architecture.get("bridge_mode", "afmr") != "afmr":
        raise ValueError("region_query requires architecture.bridge_mode=afmr")
    if settings["enabled"] and architecture.get("name") != "afmr_value_anchor":
