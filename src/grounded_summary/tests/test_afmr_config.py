...
import pytest
from grounded_summary.config import load_config, resolve_path
...
def test_sampling_and_attention_overrides_are_valid(tmp_path):
    source = Path(__file__).parents[1] / "configs" / "afmr_smoke.yaml"
    text = source.read_text(encoding="utf-8")
    text = text.replace("_base_: afmr_base.yaml", f"_base_: {source.parent / 'afmr_base.yaml'}")
    text = text.replace(
        "generation:\n  batch_size: 2",
        "model:\n  attention_implementation: flash_attention_2\n\ngeneration:\n  batch_size: 2",
    )
    text = text.replace(
        "  min_new_tokens: 0", "  min_new_tokens: 0\n  do_sample: true\n  temperature: 0.7\n  top_k: 40\n  top_p: 0.9"
    )
    path = tmp_path / "sampling.yaml"
    path.write_text(text, encoding="utf-8")
    config = load_config(path)
    assert config["model"]["attention_implementation"] == "flash_attention_2"
...
def test_pubmed_recipe_matches_t5gemma_prompt_and_decode_contract():
    ...
    assert config["decoder"]["grounded_copy"]["key_dim"] == 256
    assert config["decoder"]["grounded_copy"]["alpha_max"] == 0.20
    assert config["decoder"]["grounded_copy"]["generate_reserve"] == 0.05
    assert config["architecture"]["region_router"]["enabled"] is True
    assert config["architecture"]["region_router"]["start_fraction"] == 0.75


def test_region_router_start_fraction_is_bounded(tmp_path):
    source = Path(__file__).parents[1] / "configs" / "afmr_smoke.yaml"
    text = source.read_text(encoding="utf-8")
    text = text.replace("_base_: afmr_base.yaml", f"_base_: {source.parent / 'afmr_base.yaml'}")
    text = text.replace(
        "architecture:\n  controller_dim", "architecture:\n  region_router:\n    start_fraction: 1.0\n  controller_dim"
    )
    path = tmp_path / "bad_region_router.yaml"
    path.write_text(text, encoding="utf-8")
...
architecture: {name: afmr_shared_memory, focus_windows: [128, 32], depth_gate_init: 0.02, depth_gate_max: 0.15, feature_gate_init: 0.02, feature_gate_max: 0.2, focus_strength_init: 0.1, focus_strength_max: 1.0, temperature_init: 1.0, temperature_min: 0.5, temperature_max: 2.0}
