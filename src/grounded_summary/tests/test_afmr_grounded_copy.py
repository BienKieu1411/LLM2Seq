...
from grounded_summary.data.collate import SummarizationCollator
from grounded_summary.data.copy_alignment import COPY_INPUT_KEYS, align_copy_tokens, pad_copy_alignments
from grounded_summary.data.schema import CanonicalRecord
from grounded_summary.evaluation.generate import generate_greedy
from grounded_summary.modeling.grounded_copy import CopyState, GroundedCopyHead
from grounded_summary.modeling.model import AFMRModel
from grounded_summary.runtime import _TinyTokenizer, build_loaders
from grounded_summary.training.checkpoint import architecture_spec, load_checkpoint, save_checkpoint
from grounded_summary.training.optimizer import build_optimizer, set_stage_trainability


def setup_model(region_router=False):
...
def test_alignment_keeps_encoder_token_that_crosses_instruction_boundary():
    class Tokenizer:
        def __call__(self, text, **kwargs):
            assert text == "article"
            return {"input_ids": [17], "offset_mapping": [(0, 7)]}

    # The first encoder token starts in the instruction and ends in the article.
    row = align_copy_tokens("article", 3, [(0, 10)], Tokenizer())
    assert row["copy_token_ids"] == [17]
    assert row["copy_encoder_indices"] == [0]
...
def test_alignment_keeps_complete_token_at_truncation_boundary():
    class Tokenizer:
        def __call__(self, text, **kwargs):
            assert text == "Spain 2003"
            return {"input_ids": [17, 18], "offset_mapping": [(0, 5), (6, 10)]}

    row = align_copy_tokens("Spain 2003 hidden", 0, [(0, 5), (6, 10)], Tokenizer())
    assert row["copy_token_ids"] == [17, 18]
    assert row["copy_encoder_indices"] == [0, 1]


def test_alignment_excludes_partial_contraction_at_truncation_boundary():
    class Tokenizer:
        def __call__(self, text, **kwargs):
            assert text == "doesn'"
            return {"input_ids": [17], "offset_mapping": [(0, 6)]}

    row = align_copy_tokens("doesn't recover", 0, [(0, 6)], Tokenizer())
    assert row["copy_token_ids"] == []
...
def test_copy_mass_is_capped_and_lm_reserve_is_kept():
    head = GroundedCopyHead(8, 4, 0.05, alpha_max=0.20, generate_reserve=0.05)
    head.gate.bias.data.fill_(1000.0)
    state = CopyState(
        torch.randn(1, 2, 4),
        torch.tensor([[4, 5]]),
    ...
    _, log_copy, log_generate = head.distribution(torch.randn(1, 3, 8), state)
    assert log_copy.exp().max().item() <= 0.20 + 1.0e-6
    assert log_generate.exp().min().item() >= 0.80 - 1.0e-6
...
def test_prepare_ignores_masked_candidates_before_lexical_lookup():
    class RecordingEmbedding(torch.nn.Embedding):
        def __init__(self):
            super().__init__(13, 8)
            self.seen = None
    ...
    rows["copy_token_mask"][0, 1] = False
    state = head.prepare(
        torch.randn(1, 3, 8),
        torch.zeros(1, 3),
    ...
    assert embedding.seen.tolist() == [4]
    assert state.mask.tolist() == [[True, False]]
    assert state.keys[0, 1].eq(0).all()
    assert state.bias[0, 1].item() == 0.0


@pytest.mark.parametrize(
    "bad_row",
    [
        dict(copy_token_ids=[4], copy_encoder_indices=[0], copy_token_indices=[1], copy_alignment_weights=[1.0]),
        dict(copy_token_ids=[4], copy_encoder_indices=[0], copy_token_indices=[0], copy_alignment_weights=[-1.0]),
        dict(copy_token_ids=[4], copy_encoder_indices=[0], copy_token_indices=[], copy_alignment_weights=[1.0]),
    ],
)
def test_padding_rejects_malformed_alignment_rows(bad_row):
    with pytest.raises(ValueError, match="copy alignment row"):
        pad_copy_alignments([bad_row])


def test_padding_rejects_edges_without_candidate_tokens():
    with pytest.raises(ValueError, match="outside candidate tokens"):
        pad_copy_alignments(
            [
                dict(
                    copy_token_ids=[],
                    copy_encoder_indices=[0],
                    copy_token_indices=[0],
                    copy_alignment_weights=[1.0],
                )
            ]
        )


def test_empty_copy_state_has_finite_attention_and_exact_lm_fallback():
    head = GroundedCopyHead(8, 4, 0.05)
    ...
    attention, log_copy, log_generate = head.distribution(hidden, state)
    assert torch.isfinite(attention).all()
    assert torch.isfinite(log_copy).all()
    assert torch.isfinite(log_generate).all()
    assert attention.exp().sum(-1).eq(0).all()
    mixed = head.mix_logits(hidden, logits, state)
    torch.testing.assert_close(mixed, logits.float(), rtol=0, atol=0)
...
def test_model_training_fp32_autocast_and_warmup_optimizer():
    ...
    assert group_lrs["grounded_copy"] == config["training"]["warmup_grounded_copy_lr"]
...
def test_checkpoint_compatibility_and_mixture_dense_model_parity(tmp_path):
    ...
    assert architecture_spec(config)["grounded_copy"]["alignment"] == "char_overlap_boundary_safe"
    assert architecture_spec(config)["grounded_copy"]["alpha_max"] == 1.0
...
def test_region_router_keeps_copy_state_and_value_anchor_separate():
    config, baseline, batch, _ = setup_model()
    ...
    baseline_state = bridge(baseline, batch)
    routed_state = bridge(routed, batch)
    assert baseline_state.copy_state is not None and routed_state.copy_state is not None
    assert baseline_state.value_memory is not None and routed_state.value_memory is not None
    assert routed_state.region_memory is not None
    assert routed_state.source_bias.ndim == 2
    for left, right in (
        (baseline_state.value_memory, routed_state.value_memory),
        (baseline_state.copy_state.keys, routed_state.copy_state.keys),
        (baseline_state.copy_state.bias, routed_state.copy_state.bias),
        (baseline_state.copy_state.mask, routed_state.copy_state.mask),
    ):
