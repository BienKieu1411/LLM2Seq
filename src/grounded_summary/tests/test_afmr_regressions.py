...
from grounded_summary.data.dataset import JsonlSummarizationDataset
from grounded_summary.data.prepare import prepare_split
...
from grounded_summary.runtime import _TinyTokenizer, build_loaders, evaluate
from grounded_summary.training.engine import AFMRTrainer, seed_everything
...
def test_decoder_cached_matches_uncached_with_left_padding():
    ...
    decoder = AFMRModel(config()).decoder.eval()
...
def test_training_log_is_compact_and_uses_accumulated_ce(caplog):
    ...
    with caplog.at_level("INFO", logger="grounded_summary.train"):
        metrics = trainer._run_epoch(loader, optimizer, "interface_warmup", True)
    messages = [record.getMessage() for record in caplog.records if record.name == "grounded_summary.train"]
...
def test_resume_rejects_wrong_reference_before_model_load(tmp_path, monkeypatch):
    import grounded_summary.runtime as runtime

    monkeypatch.setattr(runtime, "AFMRModel", lambda cfg: pytest.fail("must validate before model load"))
