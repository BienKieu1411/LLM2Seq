...
class AFMRModel(nn.Module):
    def __init__(self, config: dict[str, Any]):
        ...
        self.decoder = QwenCrossDecoder(
            ...
            config["architecture"].get("region_router", {}),
        )
        ...
        **copy_inputs: torch.Tensor,
        ...
        if bridge.memory.dtype != decoder_dtype:
            bridge = BridgeState(
                ...
                region_memory=None if bridge.region_memory is None else bridge.region_memory.to(decoder_dtype),
                region_mask=bridge.region_mask,
                region_index=bridge.region_index,
                region_token_mask=bridge.region_token_mask,
            )
    ...
    def forward(
        ...
        **copy_inputs: torch.Tensor,
        ...
        logits, _, loss_ce = self.decoder(
            ...
            copy_state=bridge.copy_state,
            region_memory=bridge.region_memory,
            region_mask=bridge.region_mask,
            region_index=bridge.region_index,
            region_token_mask=bridge.region_token_mask,
        )
