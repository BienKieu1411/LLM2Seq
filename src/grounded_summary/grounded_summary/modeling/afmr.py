...
class AdaptiveFullMemoryResidualBridge(nn.Module):
    def __init__(self, encoder_hidden: int, decoder_hidden: int, config: dict):
        super().__init__()
        self.value_anchored = config.get("name", "afmr_shared_memory") == "afmr_value_anchor"
        ...
        region_router = config.get("region_router", {})
        self.region_router_enabled = bool(region_router.get("enabled", False))
        self.region_window_size = int(region_router.get("window_size", 128))
        if self.region_window_size <= 0:
            raise ValueError("AFMR region_router.window_size must be positive")
    ...
    def _region_summaries(
        self, memory: torch.Tensor, content_mask: torch.Tensor
    ) -> tuple[torch.Tensor | None, torch.Tensor | None, torch.Tensor | None, torch.Tensor | None]:
        if not self.region_router_enabled:
            return None, None, None, None
        batch, length, hidden = memory.shape
        max_regions = max(1, math.ceil(length / self.region_window_size))
        content = content_mask.bool()
        compact_index = (content.long().cumsum(-1) - 1).clamp_min(0)
        region_index = (compact_index // self.region_window_size).clamp_max(max_regions - 1)
        index = region_index[..., None].expand(-1, -1, hidden)
        weights = content.to(dtype=torch.float32)
        sums = torch.zeros(batch, max_regions, hidden, device=memory.device, dtype=torch.float32).scatter_add(
            1, index, memory.float() * weights[..., None]
        )
        counts = torch.zeros(batch, max_regions, device=memory.device, dtype=torch.float32).scatter_add(
            1, region_index, weights
        )
        region_mask = counts.gt(0)
        region_memory = sums / counts[..., None].clamp_min(1.0)
        region_memory = region_memory.masked_fill(~region_mask[..., None], 0.0).to(memory.dtype)
        return region_memory, region_mask, region_index, content
    ...
    def forward(
        ...
        region_memory, region_mask, region_index, region_token_mask = self._region_summaries(memory, content)
