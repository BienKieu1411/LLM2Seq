...
class CopiedCrossAttention(nn.Module):
    def __init__(
        ...
        config: Any,
        dropout: float,
        region_router_config: Optional[dict[str, Any]] = None,
    ):
        super().__init__()
        ...
        self._region_key_cache: Optional[torch.Tensor] = None
        region_config = region_router_config or {}
        self.region_router_enabled = bool(region_config.get("enabled", False))
        self.region_router_rank = int(region_config.get("rank", 64))
        self.region_router_gate_max = float(region_config.get("gate_max", 0.05))
        self.region_router_temperature = float(region_config.get("temperature", 1.0))
        if self.region_router_enabled:
            if self.region_router_rank <= 0:
                raise ValueError("AFMR region router rank must be positive")
            if not 0.0 < self.region_router_gate_max <= 1.0:
                raise ValueError("AFMR region router gate_max must lie in (0, 1]")
            if self.region_router_temperature <= 0.0:
                raise ValueError("AFMR region router temperature must be positive")
            self.region_router_query_norm = copy.deepcopy(input_norm)
            self.region_router_key_norm = copy.deepcopy(input_norm)
            self.region_router_query = nn.Linear(self.hidden_size, self.region_router_rank, bias=False)
            self.region_router_key = nn.Linear(self.hidden_size, self.region_router_rank, bias=False)
            self.region_router_gate = nn.Parameter(torch.zeros((), dtype=torch.float32))
    ...
    def _region_bias(
        ...
        region_memory: Optional[torch.Tensor],
        region_mask: Optional[torch.Tensor],
        region_index: Optional[torch.Tensor],
        region_token_mask: Optional[torch.Tensor],
        region_keys: Optional[torch.Tensor] = None,
    ) -> Optional[torch.Tensor]:
        if not self.region_router_enabled:
            return None
        if any(value is None for value in (region_memory, region_mask, region_index, region_token_mask)):
            raise ValueError("Enabled region router requires region memory, masks and source indices")
        assert region_memory is not None
        assert region_mask is not None
        assert region_index is not None
        assert region_token_mask is not None
        batch, query_length, hidden = query_states.shape
        if region_memory.ndim != 3 or region_memory.shape[0] != batch or region_memory.shape[-1] != hidden:
            raise ValueError("region_memory must be [batch, regions, hidden] and match query hidden size")
        if region_mask.shape != region_memory.shape[:2]:
            raise ValueError("region_mask must match region_memory [batch, regions]")
        if region_index.ndim != 2 or region_token_mask.shape != region_index.shape:
            raise ValueError("region_index and region_token_mask must be [batch, source_tokens]")
        if region_index.shape[0] != batch:
            raise ValueError("region tensors must have the same batch size as query_states")
        if region_keys is not None:
            if region_keys.ndim != 3 or region_keys.shape[:2] != region_memory.shape[:2]:
                raise ValueError("region_keys must match region_memory [batch, regions, rank]")
            if region_keys.shape[-1] != self.region_router_rank:
                raise ValueError("region_keys must match the configured region router rank")
            keys = region_keys.float()
        else:
            keys = self._project_region_keys(region_memory)
        projection_dtype = self.region_router_query.weight.dtype
        query_input = self.region_router_query_norm(query_states.float()).to(projection_dtype)
        query = self.region_router_query(query_input).float()
        scores = torch.matmul(query, keys.transpose(-1, -2))
        scores = scores / math.sqrt(self.region_router_rank) / self.region_router_temperature
        valid_regions = region_mask.bool()
        has_region = valid_regions.any(dim=-1)
        scores = scores.masked_fill(~valid_regions[:, None, :], torch.finfo(scores.dtype).min)
        route_weights = torch.softmax(scores, dim=-1)
        route_weights = torch.where(has_region[:, None, None], route_weights, torch.zeros_like(route_weights))
        token_scores = route_weights.gather(-1, region_index.long()[:, None, :].expand(-1, query_length, -1))
        token_valid = region_token_mask.bool()[:, None, :]
        token_scores = token_scores.masked_fill(~token_valid, 0.0)
        count = token_valid.sum(dim=-1, keepdim=True).clamp_min(1).to(token_scores.dtype)
        mean = token_scores.sum(dim=-1, keepdim=True) / count
        centered = (token_scores - mean).masked_fill(~token_valid, 0.0)
        gate = self.region_router_gate_max * torch.tanh(self.region_router_gate.float())
        return gate * torch.tanh(centered)

    def _project_region_keys(self, region_memory: torch.Tensor) -> torch.Tensor:
        projection_dtype = self.region_router_key.weight.dtype
        region_input = self.region_router_key_norm(region_memory.float()).to(projection_dtype)
        return self.region_router_key(region_input).float()
    ...
    def forward(
        ...
        region_memory: Optional[torch.Tensor] = None,
        region_mask: Optional[torch.Tensor] = None,
        region_index: Optional[torch.Tensor] = None,
        ...
        if self.region_router_enabled:
            for name, tensor in (("region_index", region_index), ("region_token_mask", region_token_mask)):
                if tensor is not None and tensor.shape[-1] != memory.shape[1]:
                    raise ValueError(f"{name} must have the same source length as memory")
        cached_region_keys = self._region_key_cache if self._cache is not None and not self.training else None
        dynamic_bias = self._region_bias(
            query_states,
            region_memory,
            region_mask,
            region_index,
        ...
        if dynamic_bias is not None:
            dynamic_mask = dynamic_bias[:, None, :, :].to(query.dtype)
            if mask is None:
                mask = dynamic_mask
            elif mask.dtype == torch.bool:
                mask = dynamic_mask.masked_fill(~mask, float("-inf"))
            else:
    ...
    def prepare_cache(
        ...
        self._region_key_cache = None
        if self.region_router_enabled and region_memory is not None:
            if (
                region_memory.ndim != 3
                or region_memory.shape[0] != memory.shape[0]
                or region_memory.shape[-1] != memory.shape[-1]
                or region_memory.device != memory.device
            ):
                raise ValueError("region_memory must match memory batch, hidden size and device")
            self._region_key_cache = self._project_region_keys(region_memory).contiguous()
...
class DecoderLayerWithCross(GradientCheckpointingLayer):
    def __init__(
        ...
        self.cross = CopiedCrossAttention(base.self_attn, base.input_layernorm, config, dropout, region_router_config)
    ...
    def forward(
        ...
        encoder_region_index: Optional[torch.Tensor] = None,
        encoder_region_token_mask: Optional[torch.Tensor] = None,
        **kwargs: Any,
    ) -> torch.Tensor:
        ...
        if encoder_hidden_states is not None:
            cross = self.cross(
                ...
                region_memory=encoder_region_states,
                region_mask=encoder_region_mask,
                region_index=encoder_region_index,
                region_token_mask=encoder_region_token_mask,
            )
            hidden_states = (
    ...
    def prepare_cache(
        ...
        self.cross.prepare_cache(memory, value_memory, region_memory)
...
class QwenCrossDecoder(nn.Module):
    def __init__(
        ...
        if copy_config.get("enabled", False):
            with torch.random.fork_rng(devices=[]):
                self.grounded_copy = GroundedCopyHead(
                    ...
                    float(copy_config.get("alpha_max", 1.0)),
                    float(copy_config.get("generate_reserve", 0.0)),
                )
        ...
        region_config = dict(region_router_config or {})
        region_start_layer = len(self.backbone.layers)
        if bool(region_config.get("enabled", False)):
            fraction = float(region_config.get("start_fraction", 0.75))
            if not 0.0 <= fraction < 1.0:
                raise ValueError("AFMR region router start_fraction must lie in [0, 1)")
            region_start_layer = max(
                0, min(len(self.backbone.layers) - 4, math.ceil(len(self.backbone.layers) * fraction))
            )
        wrapped = []
        with torch.random.fork_rng(devices=[]):
            for index, layer in enumerate(list(self.backbone.layers)):
                ...
                layer_region_config = dict(region_config)
                layer_region_config["enabled"] = index >= region_start_layer and bool(
                    region_config.get("enabled", False)
                )
                wrapped.append(
    ...
    def forward(
        ...
        outputs = self.backbone(
            ...
            encoder_region_states=region_memory,
            encoder_region_mask=region_mask,
            encoder_region_index=region_index,
            encoder_region_token_mask=region_token_mask,
        )
    ...
    def prepare_cross_cache(
        ...
        for layer in self.backbone.layers:
            if isinstance(layer, DecoderLayerWithCross):
                layer.prepare_cache(memory, value_memory, region_memory)
    ...
    def select_cross_cache(self, indices: torch.Tensor) -> None:
        for layer in self.backbone.layers:
            if isinstance(layer, DecoderLayerWithCross) and layer.cross._cache is not None:
                ...
                if layer.cross._region_key_cache is not None:
                    layer.cross._region_key_cache = layer.cross._region_key_cache.index_select(0, indices)
