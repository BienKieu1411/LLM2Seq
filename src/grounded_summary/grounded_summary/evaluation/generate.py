"""Greedy/candidate generation helpers with append-only JSONL resume."""
...
def _sample_token(
    ...
    *,
    ...
    """Sample one token after applying temperature, top-k, then nucleus top-p.

    Filtering is performed on logits, and every row keeps at least one finite
    token.  The latter matters when no-repeat constraints temporarily mask the
    whole vocabulary (for example, a tiny offline tokenizer).
    """
    if top_k < 0:
...
def generate_greedy(
    ...
    *,
    ...
    region_memory = getattr(bridge, "region_memory", None)
    region_mask = getattr(bridge, "region_mask", None)
    region_index = getattr(bridge, "region_index", None)
    region_token_mask = getattr(bridge, "region_token_mask", None)
    try:
        cache_kwargs = {}
        if value_memory is not None:
            cache_kwargs["value_memory"] = value_memory
        if region_memory is not None:
            cache_kwargs["region_memory"] = region_memory
        model.decoder.prepare_cross_cache(memory, **cache_kwargs)
        for step in range(max_new_tokens):
            ...
            logits, past, _ = model.decoder(
                ...
                **({"region_memory": region_memory} if region_memory is not None else {}),
                **({"region_mask": region_mask} if region_mask is not None else {}),
                **({"region_index": region_index} if region_index is not None else {}),
                **({"region_token_mask": region_token_mask} if region_token_mask is not None else {}),
            )
            ...
            if (
                ...
                if surviving.numel() != active_rows.numel():
                    ...
                    if region_memory is not None:
                        region_memory = region_memory.index_select(0, surviving)
                    if region_mask is not None:
                        region_mask = region_mask.index_select(0, surviving)
                    if region_index is not None:
                        region_index = region_index.index_select(0, surviving)
                    if region_token_mask is not None:
                        region_token_mask = region_token_mask.index_select(0, surviving)
    finally:
...
def generate_sampled(
    ...
    *,
    ...
    """Candidate generation; training never calls this path."""
