...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if attention_implementation not in {"sdpa", "flash_attention_2", "eager"}:
        raise ValueError("model.attention_implementation must be sdpa, flash_attention_2, or eager")
    ...
    if architecture.get("name") not in {"afmr_shared_memory", "afmr_value_anchor"}:
        raise ValueError("architecture.name must be afmr_shared_memory or afmr_value_anchor")
    ...
    region_router = architecture.get("region_router", {})
    if not isinstance(region_router, dict):
        raise ValueError("architecture.region_router must be a mapping")
    _check_keys(
        region_router,
        {"enabled", "window_size", "rank", "gate_max", "temperature", "start_fraction"},
        "architecture.region_router",
    )
    if not isinstance(region_router.get("enabled", False), bool):
        raise ValueError("architecture.region_router.enabled must be a boolean")
    if int(region_router.get("window_size", 128)) <= 0:
        raise ValueError("architecture.region_router.window_size must be positive")
    if int(region_router.get("rank", 64)) <= 0:
        raise ValueError("architecture.region_router.rank must be positive")
    if not 0 < float(region_router.get("gate_max", 0.05)) <= 1.0:
        raise ValueError("architecture.region_router.gate_max must lie in (0, 1]")
    if float(region_router.get("temperature", 1.0)) <= 0:
        raise ValueError("architecture.region_router.temperature must be positive")
    start_fraction = float(region_router.get("start_fraction", 0.75))
    if not 0.0 <= start_fraction < 1.0:
        raise ValueError("architecture.region_router.start_fraction must lie in [0, 1)")
    ...
    _check_keys(
        copy_config,
        {"enabled", "key_dim", "gate_init", "alpha_max", "generate_reserve"},
    ...
    if int(copy_config.get("key_dim", 128)) <= 0:
        raise ValueError("Grounded copy requires key_dim > 0")
    alpha_max = float(copy_config.get("alpha_max", 1.0))
    generate_reserve = float(copy_config.get("generate_reserve", 0.0))
    effective_alpha_max = min(alpha_max, 1.0 - generate_reserve)
    if not 0 < alpha_max <= 1:
        raise ValueError("decoder.grounded_copy.alpha_max must lie in (0, 1]")
    if not 0 <= generate_reserve < 1:
        raise ValueError("decoder.grounded_copy.generate_reserve must lie in [0, 1)")
    if not 0 < float(copy_config.get("gate_init", 0.05)) < effective_alpha_max:
        raise ValueError("decoder.grounded_copy.gate_init must lie below the effective alpha_max")
