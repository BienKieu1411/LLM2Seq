...
def architecture_spec(config: dict[str, Any]) -> dict[str, Any]:
    ...
    spec = {
        "graph_version": "afmr_token_depth_lowrank",
        ...
        "region_router": {
            "enabled": bool(arch.get("region_router", {}).get("enabled", False)),
            "window_size": int(arch.get("region_router", {}).get("window_size", 128)),
            "rank": int(arch.get("region_router", {}).get("rank", 64)),
            "gate_max": float(arch.get("region_router", {}).get("gate_max", 0.05)),
            "temperature": float(arch.get("region_router", {}).get("temperature", 1.0)),
            "start_fraction": float(arch.get("region_router", {}).get("start_fraction", 0.75)),
        },
    ...
    if copy_config.get("enabled", False):
        spec["grounded_copy"] = {
            "alignment": "char_overlap_boundary_safe",
            "key_dim": int(copy_config.get("key_dim", 128)),
            "gate_init": float(copy_config.get("gate_init", 0.05)),
...
def save_checkpoint(
    ...
    *,
    ...
    """Save a checkpoint collectively, with filesystem I/O on rank zero.

    DDP workers have independent Python/NumPy/torch RNG streams.  Capturing
    each stream makes a resumed run deterministic per worker instead of
    silently replaying rank zero's random state everywhere.
    """
    rng = {
...
def _save_checkpoint(
    ...
    *,
    ...
    elapsed_train_seconds: float | None,
    scheduler: Any,
    rng_states: list,
) -> None:
