...
LOGGER = logging.getLogger("grounded_summary.train")
...
class _LossOnlyModel(torch.nn.Module):
    """Expose one backward root and detached CE statistics to DDP."""

    def __init__(self, model: torch.nn.Module):
        super().__init__()
        self.model = model

    def forward(self, *args, ce_scale: float = 1.0, local_token_count: int = 0, **kwargs):
        output = self.model(*args, **kwargs)
        loss_ce = getattr(output, "loss_ce", None)
        if loss_ce is None:
            raise RuntimeError("AFMR training requires decoder labels")
        # AFMRModel returns a mean CE.  Convert it to a local token sum before
        # applying the global-token scale.  DDP then averages rank gradients.
        ce_sum = loss_ce * int(local_token_count)
        return (
            ce_sum * float(ce_scale),
            ce_sum.detach(),
            torch.as_tensor(local_token_count, device=loss_ce.device, dtype=torch.long),
        )
...
class AFMRTrainer:
    ...
    def _configure_distributed(self) -> None:
        # Trainability changes between warmup and full fine-tuning; rebuilding
        # the reducer prevents stale buckets and keeps unused parameters safe.
    ...
    def _run_epoch(
        ...
        while window := list(islice(iterator, accum if train else 1)):
            ...
            if train and world_size() > 1:
                ...
                window_tokens, examples = (int(totals[0]), int(totals[1]))
            if train:
                optimizer.zero_grad(set_to_none=True)
            step_loss = torch.zeros((), device=self.device, dtype=torch.float64)
            for microstep, (raw_batch, tokens) in enumerate(zip(window, counts)):
                ...
                with sync, torch.set_grad_enabled(train):
                    with torch.autocast("cuda", dtype=torch.bfloat16) if self.use_bf16 else nullcontext():
                        backward, local_ce, _ = forward_model(
                            batch["input_ids"],
                    ...
                    if train:
                        backward.backward()
                step_loss += local_ce.double()
                ce_sum += local_ce.double()
                token_total += tokens
                del backward, local_ce, batch
            if train:
                ...
                if self.global_step % int(self.config["training"]["log_every_steps"]) == 0:
                    ...
                    step_ce = float(step_loss) / max(1, window_tokens)
        ...
        totals = torch.tensor([float(ce_sum), float(token_total)], device=self.device, dtype=torch.float64)
        if world_size() > 1:
            dist.all_reduce(totals)
        ce = float(totals[0]) / max(1.0, float(totals[1]))
