# GroundedSummary AFMR

AFMR (Adaptive Full-Memory Residual) is a full-source encoder–decoder architecture for general text summarization:

```text
pretrained encoder → value-anchored AFMR interface → cross-attention Qwen decoder → contextual copy/LM mixture → greedy (or sampled candidates)
```

The bridge preserves every valid source token. A document/prompt/requested-budget controller conditions a low-rank depth residual in encoder space, a low-rank encoder-to-decoder feature residual, and one multi-scale source prior. Depth weights have shape `[batch, source_tokens, depth_taps]`: a shared learned scorer reads each normalized candidate token representation, plus a document-conditioned depth preference. Softmax is over depth, not tokens; selection runs once after the encoder. Residual output factors and the depth/focus scorers start at zero; gates and prior strength are bounded, but learned residual vector norms are not mathematically bounded. The prior is additive `[batch, source_tokens]` and is consumed by every decoder cross-attention layer. There are no banks, hard top-k pruning, rerankers, beam search, KD, or test-time evidence labels.

Focus regions are overlapping **token windows**, not selected sentences. Decoder memory contains the entire source. The optional `architecture.region_router` is a separate, query-conditioned coarse route. It averages compact content tokens from `M` into non-overlapping regions, scores those regions from each decoder query with a low-rank projection, and adds a bounded centered bias to cross-attention. It is enabled in the PubMed recipe only in the upper decoder quarter (with at least four layers when available), starts with a zero route gate, and leaves the static source bias, `H0` value memory and grounded-copy preparation unchanged. During cached generation the query score is recomputed for each token while projected region keys are cached alongside the existing cross-attention key/value cache.

The only objective is token-level cross-entropy (CE): depth, feature and focus routes all learn through it. There is no positive-sentence mining, allocation loss, or contrastive head. External evidence labels are ignored.
...
The previous `afmr_shared_memory` uses `M` for both K and V. The new interface separates retrieval adaptation from the value content: depth/feature residuals and the focus prior learn where to retrieve, while the final encoder states supply the values. The encoder, value projection and shared width projection remain trainable in full fine-tuning. Nothing is detached. Gradients from CE reach both paths; for retrieval-only residual parameters, the direct derivative of V is zero. This restriction does not guarantee factuality or ROUGE improvement, and normalization of keys can still change addressing.
...
The PubMed candidate in `configs/afmr_pubmed.yaml` widens the grounded-copy key space from the base value of 128 to 256, caps copy mass at 0.20, and uses a compact decoder instruction that prioritizes the four abstract components while preserving biomedical terms, entities, numbers, units, negation and result direction. Encoder prompt, bridge, objective, data protocol, seed and greedy decoding remain fixed. Keep a matched run with the previous decoder prompt when attributing a score change to key dimension, copy behavior or prompt conditioning.
...
At hidden width 1024, the base rank-128 head has **393,473 parameters**; the PubMed rank-256 candidate has **786,945**. Context is projected before sparse pooling; no dense encoder-token × decoder-token alignment tensor is materialized. Keys and token IDs are prepared once per source and reused by greedy decoding, including finished-row compaction. The copy head has its own optimizer learning-rate group (`warmup_grounded_copy_lr` / `full_grounded_copy_lr`) so its small parameter set is not forced to follow the much larger cross-attention block. Training checkpoints CE in time chunks, computes vocabulary logits only for supervised positions and aggregates mixture likelihood in FP32. It adds one output-side attention calculation and sparse pooling, not another encoder/decoder pass or cross-attention bank. Extra tokenization, attention and activations still cost time/VRAM; real B200 throughput must be measured.
...
Each input record contains the configured source and target fields (strings or lists of strings). Training can read raw JSONL directly. For the same three-split preparation used by earlier data preparation runs, run `prepare-dataset`: PubMed/ArXiv expect `train.label.jsonl`, `val.label.jsonl`, and `test.label.jsonl`; CNNDM/WikiLingua accept the usual `train`, `val`/`validation`, and `test` JSONL/JSON/TXT names. The command copies raw files, writes canonical `id/text/summary` JSONL, emits `preparation_report.json`, and rejects duplicate IDs or exact source text across splits. Evidence labels are not consumed by AFMR.
...
Start from `configs/afmr_base.yaml`, copy it to a task recipe, and set only model locations, data files, lengths, batch resources, and output directory. The generic base uses one warm-up epoch and four full-finetuning epochs; benchmark recipes override this to match the corresponding T5Gemma total (PubMed 1+3, CNNDM/WikiLingua 1+5). Benchmark decoding is greedy (`num_beams: 1`, `do_sample: false`), and training is CE-only. Candidate generation can opt into temperature/top-k/top-p sampling without changing the training graph.

The PubMed queue pins `batch_size: 84` per GPU, `gradient_accumulation_steps: 1`,
and `max_grad_norm: 2.0`. With two processes this gives 168 examples per
optimizer update. These values are task-local so a generic base cannot silently
change the comparison protocol; reproduce the historical one-GPU control
separately before making a score claim.
...
PYTHON=/absolute/path/to/bienkieu_env/bin/python \
  ...
  configs/afmr_pubmed.yaml runs/grounded_summary/pubmed_key256_copy/last.pt \
runs/grounded_summary/pubmed_key256_copy/test_predictions.jsonl --split test
```

For the two-GPU PubMed queue that prepares data, trains the PPLX
encoder recipe, then trains a Qwen3-Embedding control and evaluates both
`last.pt` checkpoints with DDP, run:
...
`EVAL_BATCH_SIZE` to control per-GPU evaluation memory. Set
`ROUGE155_SCRIPT` to the local `evaluate_rouge.py` wrapper to append the Perl
ROUGE-1.5.5 audit. `EVAL_DO_SAMPLE=true` enables candidate sampling in the
queue; `EVAL_TEMPERATURE`, `EVAL_TOP_K`, and `EVAL_TOP_P` select its filter.

The queue now writes to `runs/grounded_summary/pubmed_pair_afmr_value_anchor_copy_region_k256`, leaving earlier results untouched. Set `AFMR_COPY_KEY_DIM=128` for the matched key-rank control, or `AFMR_GROUNDED_COPY=false` for the value-anchor LM-only control. Set `AFMR_REGION_ROUTER=false` for the static-routing ablation; its output directory is suffixed with `_static`. Additionally set `AFMR_ARCHITECTURE=afmr_shared_memory` for the shared-memory LM-only control. Numerical/text fixes remain enabled. `BATCH_SIZE` and `GRADIENT_ACCUMULATION_STEPS` are per-GPU settings; the queue defaults to `84 × 1 × 2`, an effective 168-example optimizer update. Set `ATTENTION_IMPLEMENTATION=flash_attention_2` when the installed Transformers/PyTorch stack supports it; `sdpa` is the default fallback. For a single experiment, use `run_afmr.sh train` with one task config instead.
...
Train a new checkpoint for the new prompt and architecture. Do not edit an old `resolved_config.yaml` and evaluate old weights as if they had been trained with these inputs. Changing `afmr_shared_memory` to `afmr_value_anchor` is rejected by checkpoint compatibility checks; their parameter shapes alone do not imply semantic compatibility.

The token-wise graph (`afmr_token_depth_lowrank`) is intentionally incompatible with earlier document-wise AFMR checkpoints. Do not use `strict=False` to force-load those checkpoints. Train this graph from pretrained backbones in a separate output directory. New token-wise checkpoints resume normally.
...
For bundled `configs/*.yaml`, relative data/output paths are rooted at `grounded_summary/`, independent of the calling directory or whether files already exist. An external YAML resolves relative paths against its own directory. At encoder/decoder hidden size 1024, the widened AFMR bridge contains 3,459,335 parameters; this excludes both pretrained backbones, the copied decoder cross-attention and the optional query-conditioned region router. Token-wise depth selection does not stack a second full-depth memory tensor.
...
To generate candidates instead of the benchmark's deterministic output, pass
`--do-sample --temperature 0.7 --top-k 50 --top-p 0.9` to `evaluate`, or set
the same keys in `generation`. Filtering is applied in the order temperature,
top-k, then nucleus top-p, and each DDP rank uses a rank-offset seed. Training
never calls this path or creates extra candidates. A two-GPU evaluation uses
the same launcher as training and writes one ordered JSONL on rank zero:

```bash
CUDA_VISIBLE_DEVICES=0,1 torchrun --standalone --nproc_per_node=2 run_afmr.py evaluate \
  configs/afmr_pubmed.yaml runs/.../last.pt runs/.../test_samples.jsonl \
  --split test --batch-size 64 --do-sample --temperature 0.7 --top-k 50 --top-p 0.9
```

Distributed evaluation expects a new output JSONL; use the single-process
launcher when resuming an existing contiguous prediction prefix.
...
```text
grounded_summary/
├── grounded_summary/       public AFMR namespace
...
│   ├── evaluation/    greedy/sampled generation, metrics
│   └── distributed.py torchrun lifecycle and rank-zero I/O
