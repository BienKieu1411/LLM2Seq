...
4. Router không có tín hiệu khi `Wo=0`, trong khi mass semantic tại init chỉ khoảng `0.05`. Main `v5.1-safe` có thể không bao giờ đánh thức được nhánh mà nó muốn đo.
...
C2 đòi `semantic_only_v2` trùng logits/loss/gradient v2 flat. Nếu implement theo `DESIGN.md`, C2 fail dù C1 pass. `IMPLEMENTATION_PLAN` nói “giữ RMS của v2” nhưng không chỉ RMS nào.
...
Plan xây cả v5.1 trên câu chuyện:
...
v5.1 gán regression cho (2) bị nhân `(1-g)`, rồi thay bằng mixture. Nó đồng thời **bỏ (3)**. Đó là hai thay đổi. Nếu D thua B, không biết vì mixture hay vì mất residual gate.
...
v5.1 sau khi học: semantic chỉ được `α ≤ 0.20` mass. `P0` luôn còn. Khoảng cách TV tới phân phối copy-only của `new` bị chặn:

```text
‖P − ((1−g)P0 + g Pcopy)‖₁ / 2  ≤  α  ≤  0.20
```

Mixture không thể sửa quá 20% mass generate. Plan không biện minh 0.20 từ dữ liệu `g` của `new`. Nếu `g` trung bình ~0.1 thì generate còn 0.9, và v2 được phép sửa cả 0.9; v5.1 chỉ 0.20. Nếu giả thuyết “semantic hữu ích nhưng quá mạnh” đúng, 0.20 có lý. Nếu giả thuyết sai và v2 chỉ yếu vì recipe, 0.20 biến D thành `new` cộng nhiễu nhỏ — sẽ “parity” rồi tuyên bố mixture không giúp, trong khi chưa bao giờ cho semantic đủ mass.
...
V5.1 **chuyển hệ số nhỏ từ quy mô residual sang quy mô mixture**. Init `α≈0.05` gần bằng `semantic_gate_init` của v2, nhưng ý nghĩa khác: 0.05 của v2 là scale của `δ`, 0.05 của v5.1 là trọng số expert. Gradient semantic yếu hơn khoảng `(1−g)/α ≈ 10–20×` lúc đầu.

`v5.1-tiny` tồn tại đúng vì lý do này, nhưng main vẫn là `Wo=0`. Parity control không nên là ứng viên chất lượng. Đảo vai: `tiny` là train candidate; `zero` chỉ để C4 và một run ngắn chứng minh endpoint, không full-train ba seed trừ khi tiny đã thắng.
...
| Source prior `b` | Cộng vào semantic score, copy score, và cross-attn | Một prior, ba lần. `v5.1-KM` mới giảm prior. Thiếu control `semantic_prior=0` trên main K=H0. |
...
So với v4/v3: không còn region mass `A[j]≤π[r]`, không usage penalty giả coverage, không tangent+restore-norm đổi thứ tự từ trong khi tuyên bố “giữ câu”. Audit v4 trong `ARCHITECTURE_AUDIT` là phần mạnh nhất của folder này; v5.1 đi đúng hướng khi từ chối đưa planner trở lại main.
