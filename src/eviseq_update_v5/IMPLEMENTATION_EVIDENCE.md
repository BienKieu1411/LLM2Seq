# EviSeq v5.1 implementation evidence

## Scope

Implementation was made on branch `codex/implement-eviseq-v5` in the local
workspace. No remote, push, pull request or model download was used. The
...
|---|---|
| Python compilation | `PYTHONPATH=src/eviseq_update_v5 bienkieu_env/bin/python -m compileall -q src/eviseq_update_v5/eviseq_update_v5` — passed |
| Ruff lint | `bienkieu_env/bin/ruff check src/eviseq_update_v5` — passed |
| Ruff formatter | `bienkieu_env/bin/ruff format --check src/eviseq_update_v5` — passed |
| v5 acceptance tests | `HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 PYTHONPATH=src/eviseq_update_v5 bienkieu_env/bin/pytest -q src/eviseq_update_v5/tests` — **26 passed**, 2 dependency deprecation warnings; no model download path was exercised |
| Offline smoke | `HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 PYTHON=bienkieu_env/bin/python src/eviseq_update_v5/run_afmr.sh smoke` — passed (`checkpoint_step=4`, `examples=2`, train CE `4.4775705839 -> 4.4749635646`) |
...
| C2 | Flat reader ordering/source-mask tests; no v2 checkpoint is present | Partial: graph verified, checkpoint parity pending |
...
| C10 | FP32 finite checks pass; BF16 CUDA rounding gate is unrun | Partial |
...
identify a drive-by edit outside `src/eviseq_update_v5`. The plan/assumption
