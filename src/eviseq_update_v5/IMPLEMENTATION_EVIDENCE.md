...
| v5 acceptance tests | `HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 PYTHONPATH=src/eviseq_update_v5 bienkieu_env/bin/pytest -q src/eviseq_update_v5/tests` — **31 passed**, 2 dependency deprecation warnings; no model download path was exercised |
...
| C2 | Explicit `legacy_v2` readout/cap/inner-gate config plus flat reader ordering/source-mask tests; no v2 checkpoint is present | Partial: graph contract verified, checkpoint parity pending |
