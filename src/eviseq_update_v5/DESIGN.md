...
cho main; variant `v5.1-KM` phải ghi rõ scale (mặc định `0.0`) để không đếm
source prior hai lần. Nếu thử `r=256`, denominator là `sqrt(r)`, không cố định
`sqrt(128)`.

`K=M,V=H0` là variant `v5.1-KM`. Khi chạy variant này, giảm hoặc tắt source prior trong semantic score để đo nguy cơ double-count. Không dùng `M` làm value ở vòng đầu vì nó có thể làm AFMR drift tác động trực tiếp vào content và copy.
