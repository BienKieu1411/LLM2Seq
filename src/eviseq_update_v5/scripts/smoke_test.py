#!/usr/bin/env python3
"""Offline v5.1 integration smoke using only the bundled ``__tiny__`` models."""
...
from eviseq_update_v5.training.checkpoint import load_checkpoint
from eviseq_update_v5.training.engine import AFMRTrainer, seed_everything
...
def main() -> None:
    ...
    if initial.loss is None or not torch.isfinite(initial.loss):
        raise RuntimeError("v5.1 tiny loss is not finite")
    ...
    if not any(parameter.grad is not None and torch.isfinite(parameter.grad).all() for parameter in model.parameters()):
        raise RuntimeError("v5.1 tiny backward produced no finite gradient")
    model.zero_grad(set_to_none=True)

    with tempfile.TemporaryDirectory(prefix="eviseq_v51_smoke_") as directory:
        ...
        resumed_trainer = AFMRTrainer(EviSeqAFMR(config), config, "cpu")
        ...
        if metrics != resumed or metadata["step"] <= 0:
            raise RuntimeError("v5.1 checkpoint/evaluation resume is not deterministic")
    print(
        json.dumps(
            {
                "status": "ok",
