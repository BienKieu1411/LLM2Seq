...
from .dual_readout import DualReadout
from .grounded_copy import CopyState, GroundedCopyHead
from .semantic_read import SemanticReader, SemanticState
...
class CopiedCrossAttention(nn.Module):
    def __init__(
        ...
        self.query_gate: Optional[nn.Linear] = None
        if query_cross_gate:
            # Preserve both the copied attention function and downstream random
            # initialization: 2 * sigmoid(0) is exactly one for every head.
            device = self.q_proj.weight.device
            devices = [device.index] if device.type == "cuda" else []
            with torch.random.fork_rng(devices=devices):
                self.query_gate = nn.Linear(
                    self.hidden_size, self.num_heads, device=device, dtype=self.q_proj.weight.dtype
                )
    ...
    def forward(
        ...
        if self.query_gate is not None:
            # This depends on the current target query, so only source K/V may
            # be cached. Each head is gated before the shared output projection.
            gate = 2.0 * torch.sigmoid(self.query_gate(query_states.to(self.query_gate.weight.dtype)).float())
...
class DecoderLayerWithCross(GradientCheckpointingLayer):
    def __init__(
        ...
        self.cross = CopiedCrossAttention(
            base.self_attn, base.input_layernorm, config, dropout, query_cross_gate=query_cross_gate
        )
    ...
    def __init__(
        ...
        if copy_config.get("enabled", False):
            with torch.random.fork_rng(devices=[]):
                ...
                self.dual_readout = DualReadout(
                    int(model_config.hidden_size),
                    int(semantic_config.get("rank", 128)),
                    mode=str(copy_config.get("readout_mode", "copy_mass_preserving_capped_simplex")),
                    alpha_max=float(copy_config.get("alpha_max", 0.20)),
                    generate_reserve=float(copy_config.get("generate_reserve", 0.05)),
                    alpha_init=float(copy_config.get("alpha_init", 0.05)),
                    base_floor=float(copy_config.get("base_floor", 0.05)),
                    hidden_lambda=float(copy_config.get("hidden_lambda", 0.10)),
                    detach_copy_route_features=bool(copy_config.get("detach_copy_route_features", True)),
                    hard_source_fallback=bool(copy_config.get("hard_source_fallback", True)),
                )
        if semantic_config.get("enabled", False):
            with torch.random.fork_rng(devices=[]):
                self.semantic_reader = SemanticReader(
                    int(model_config.hidden_size),
                    int(semantic_config.get("rank", 128)),
                    key_source=str(semantic_config.get("key_source", "H0")),
        ...
        for index, layer in enumerate(list(self.backbone.layers)):
            ...
            wrapped.append(
                DecoderLayerWithCross(
                    ...
                    gate_max,
                    index,
                    query_cross_gate=bool(config.get("query_cross_gate", False)),
                )
            )
    ...
    def forward(
            ...
            if self.grounded_copy is None:
                if self.semantic_reader is not None:
                    if semantic_state is None:
                        raise ValueError("Semantic-only decoder requires cached semantic state")
                    semantic_hidden, _ = self.semantic_reader.read(output_hidden, semantic_state)
                    logits = self.lm_head(semantic_hidden)
                else:
                    logits = self.lm_head(output_hidden)
            else:
                if copy_state is None or self.dual_readout is None:
                    raise ValueError("Grounded copy requires copy state and dual readout")
                copy_read = self.grounded_copy.attention(output_hidden, copy_state)
                if self.semantic_reader is not None:
                    if semantic_state is None:
                        raise ValueError("Semantic reader requires cached semantic state")
                    generation_hidden, diagnostics = self.semantic_reader.read(output_hidden, semantic_state)
                else:
                    generation_hidden = output_hidden
                    diagnostics = {
                        "q": output_hidden.new_zeros((*output_hidden.shape[:2], self.dual_readout.rank)),
                        "u": output_hidden.new_zeros((*output_hidden.shape[:2], self.dual_readout.rank)),
                        "evidence": output_hidden.new_zeros((*output_hidden.shape[:2], 1)),
                    }
                if self.dual_readout.mode == "hidden_interpolation":
                    read_state = self.dual_readout.hidden_interpolation_state(
                        output_hidden,
                        generation_hidden - output_hidden,
                        copy_read,
                        copy_state,
                ...
                logits = read_state.output_logits
        loss = None
        if labels is not None:
            ...
            if self.grounded_copy is not None:
                if copy_state is None or self.dual_readout is None:
                    raise ValueError("Grounded copy requires copy state and dual readout")
                train_hidden = hidden[:, :-1]
                if self.semantic_reader is not None:
                    if semantic_state is None:
                        raise ValueError("Semantic reader requires cached semantic state")
                    semantic_hidden, semantic_diagnostics = self.semantic_reader.read(train_hidden, semantic_state)
                else:
                    semantic_hidden = train_hidden
                    semantic_diagnostics = {
                        "q": train_hidden.new_zeros((*train_hidden.shape[:2], self.dual_readout.rank)),
                        "u": train_hidden.new_zeros((*train_hidden.shape[:2], self.dual_readout.rank)),
                        "evidence": train_hidden.new_zeros((*train_hidden.shape[:2], 1)),
                    }
                train_copy = self.grounded_copy.attention(train_hidden, copy_state)
                if not return_logits:
                    if self.dual_readout.mode == "hidden_interpolation":
                        semantic_hidden = train_hidden + self.dual_readout.hidden_lambda * (
                            semantic_hidden - train_hidden
                        )
                        semantic_diagnostics = {
                            "q": train_hidden.new_zeros((*train_hidden.shape[:2], self.dual_readout.rank)),
                    ...
                    loss_sum, token_count = self.dual_readout.loss_sum_from_hidden(
                        train_hidden,
                        semantic_hidden,
                        semantic_diagnostics,
                        train_copy,
                    ...
                    return (
                        ...
                        loss_sum / token_count.clamp_min(1),
                    )
                if self.dual_readout.mode == "hidden_interpolation":
                    state = self.dual_readout.hidden_interpolation_state(
                        train_hidden,
                        semantic_hidden - train_hidden,
                        train_copy,
                        copy_state,
                ...
                loss_sum, token_count = self.dual_readout.loss_sum_from_state(state, shift_labels)
                loss = loss_sum / token_count.clamp_min(1)
                return logits, getattr(outputs, "past_key_values", None) if use_cache else None, loss
            if self.semantic_reader is not None:
                if semantic_state is None:
                    raise ValueError("Semantic-only decoder requires cached semantic state")
                train_hidden, _ = self.semantic_reader.read(hidden[:, :-1], semantic_state)
                if logits is None:
                    flat_hidden = train_hidden.reshape(-1, train_hidden.shape[-1])
                    flat_labels = shift_labels.reshape(-1)
                    valid = flat_labels.ne(-100)
                    logits_for_loss = self.lm_head(flat_hidden[valid]).float()
                    loss = F.cross_entropy(logits_for_loss, flat_labels[valid], reduction="sum")
                else:
