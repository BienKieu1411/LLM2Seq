"""Strict configuration loading for the EviSeq v5.1 implementation.
...
try:
    ...
    raise ImportError("EviSeq v5.1 requires PyYAML; install pyyaml in the active environment") from error
...
def _load(path: Path, stack: tuple[Path, ...]) -> dict[str, Any]:
    ...
    if len(stack) > 1:
        raise ValueError("EviSeq v5.1 supports only one base and one task override")
    ...
    if len(parents) > 1:
        raise ValueError("EviSeq v5.1 supports only one base config")
    merged: dict[str, Any] = {}
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if model.get("tokenizer_use_fast", True) is not True:
        raise ValueError("EviSeq v5.1 requires tokenizer_use_fast=true for offset alignment")
    ...
    if arch.get("name") != "afmr_value_anchor":
        raise ValueError("architecture.name must be afmr_value_anchor for v5.1")
    ...
    if copy_cfg.get("copy_entropy_feature", "normalized_attention_entropy") != "normalized_attention_entropy":
        raise ValueError("v5.1 supports only copy_entropy_feature=normalized_attention_entropy")
    ...
    if copy_cfg.get("logit_offset", "z0_log_partition") != "z0_log_partition":
        raise ValueError("v5.1 uses grounded_copy.logit_offset=z0_log_partition")
    ...
    if int(semantic.get("rank", 0)) <= 0 or int(semantic.get("num_heads", 1)) != 1:
        raise ValueError("v5.1 main uses one semantic head with positive rank")
