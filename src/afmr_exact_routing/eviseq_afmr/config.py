...
def exact_routing_settings(architecture: dict[str, Any]) -> dict[str, Any]:
    """Validate the query-dependent exact-token routing branch."""
    defaults = {
        "enabled": True,
        "rank": 128,
        "block_size": 128,
    ...
    supplied = architecture.get("exact_routing", {})
    if not isinstance(supplied, dict):
        raise ValueError("architecture.exact_routing must be a mapping")
    _check_keys(supplied, set(defaults), "architecture.exact_routing")
    settings = {**defaults, **supplied}
    if not isinstance(settings["enabled"], bool):
        raise ValueError("architecture.exact_routing.enabled must be a boolean")
    for key in ("rank", "block_size", "query_chunk_size"):
        if isinstance(settings[key], bool) or int(settings[key]) <= 0:
            raise ValueError(f"architecture.exact_routing.{key} must be positive")
    settings["gate_init"] = float(settings["gate_init"])
    settings["gate_max"] = float(settings["gate_max"])
    if settings["gate_max"] <= 0 or abs(settings["gate_init"]) >= settings["gate_max"]:
        raise ValueError("exact routing requires |gate_init| < gate_max and gate_max > 0")
    if architecture.get("bridge_mode", "afmr") == "direct_projection":
        settings["enabled"] = False
    if architecture.get("name") != "afmr_exact_routing" and settings["enabled"]:
        raise ValueError("architecture.exact_routing requires architecture.name=afmr_exact_routing")
    return settings
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if architecture.get("name") not in {"afmr_v1", "afmr_value_anchor", "afmr_exact_routing"}:
        raise ValueError("architecture.name must be afmr_v1, afmr_value_anchor, or afmr_exact_routing")
    ...
    if architecture.get("name") == "afmr_exact_routing":
        exact_routing_settings(architecture)
    else:
    ...
    if salience_weight > 0 and architecture.get("name") == "afmr_exact_routing":
        raise ValueError("afmr_exact_routing is CE-only; salience_loss_weight must be zero")
    data = config["data"]
