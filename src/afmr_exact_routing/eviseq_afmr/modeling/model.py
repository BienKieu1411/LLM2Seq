...
from .exact_routing import ExactRoutingBridge
...
class EviSeqAFMR(nn.Module):
    def __init__(self, config: dict[str, Any]):
        ...
        exact_enabled = (
            config["architecture"].get("name") == "afmr_exact_routing"
        ...
        self.decoder = QwenCrossDecoder(
            ...
            exact_route_config=config["architecture"].get("exact_routing", {}) if exact_enabled else {},
        )
        if exact_enabled:
            self.bridge = ExactRoutingBridge(
                self.encoder.hidden_size,
                int(self.decoder.config.hidden_size),
                config["architecture"],
                route_builder=self.decoder.exact_router,
            )
        else:
        ...
        **copy_inputs: torch.Tensor,
        ...
        if bridge.memory.dtype != decoder_dtype:
            bridge = BridgeState(
                ...
                exact_route=None
                if bridge.exact_route is None
                else bridge.exact_route.with_values(bridge.memory.to(decoder_dtype)),
            )
    ...
    def forward(
        ...
        **copy_inputs: torch.Tensor,
        ...
        logits, _, loss_ce = self.decoder(
            ...
            exact_route=bridge.exact_route,
        )
        loss_salience = None
        total_loss = loss_ce
