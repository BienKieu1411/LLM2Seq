...
from .exact_routing import ExactRoutingState, ExactTokenRouter
...
class DecoderLayerWithCross(GradientCheckpointingLayer):
    def __init__(
        ...
        index: int,
        value_residual_max_relative_rms: float = 0.0,
        exact_router: Optional[ExactTokenRouter] = None,
        exact_route_gate_init: float = 0.0,
        exact_route_gate_max: float = 0.20,
    ):
        super().__init__()
        ...
        self.exact_router = exact_router
        self._exact_route: Optional[ExactRoutingState] = None
        if exact_router is not None:
            if exact_route_gate_max <= 0 or abs(exact_route_gate_init) >= exact_route_gate_max:
                raise ValueError("exact route gate must satisfy |init| < max and max > 0")
            self.exact_route_norm = copy.deepcopy(base.input_layernorm)
            self.exact_route_gate_raw = nn.Parameter(
                torch.tensor(
                    math.atanh(float(exact_route_gate_init) / float(exact_route_gate_max)), dtype=torch.float32
                )
            )
            self.exact_route_gate_max = float(exact_route_gate_max)
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if encoder_hidden_states is not None:
            ...
            if self.exact_router is not None and self._exact_route is not None:
                route = self.exact_router(self.exact_route_norm(hidden_states), self._exact_route)
                route_gate = self.exact_route_gate_max * torch.tanh(self.exact_route_gate_raw)
                hidden_states = hidden_states + route_gate.to(hidden_states.dtype) * route
        residual = hidden_states
    ...
    def prepare_cache(
        ...
        value_memory: Optional[torch.Tensor] = None,
        value_residual: Optional[torch.Tensor] = None,
        exact_route: Optional[ExactRoutingState] = None,
    ) -> None:
        self.cross.prepare_cache(memory, value_memory, value_residual)
        self._exact_route = exact_route

    def clear_cache(self) -> None:
        self.cross.clear_cache()
        self._exact_route = None

    def set_exact_route(self, exact_route: Optional[ExactRoutingState]) -> None:
        self._exact_route = exact_route
...
class QwenCrossDecoder(nn.Module):
    def __init__(
        ...
        value_residual_max_relative_rms: float = 0.0,
        exact_route_config: Optional[dict] = None,
    ):
        super().__init__()
        ...
        exact_route_config = exact_route_config or {}
        shared_rng_state = torch.get_rng_state() if exact_route_config.get("enabled", False) else None
        self.exact_router: Optional[ExactTokenRouter] = None
        if exact_route_config.get("enabled", False):
            self.exact_router = ExactTokenRouter(
                int(model_config.hidden_size),
                int(exact_route_config.get("rank", 128)),
                int(exact_route_config.get("block_size", 128)),
                int(exact_route_config.get("query_chunk_size", 128)),
            )
        route_gate_init = float(exact_route_config.get("gate_init", 0.0))
        route_gate_max = float(exact_route_config.get("gate_max", 0.20))
    ...
    def forward(
        ...
        for layer in self.backbone.layers:
            if isinstance(layer, DecoderLayerWithCross):
                layer.set_exact_route(exact_route)
    ...
    def prepare_cross_cache(
        ...
        for layer in self.backbone.layers:
            if isinstance(layer, DecoderLayerWithCross):
                layer.prepare_cache(memory, value_memory, value_residual, exact_route)
    ...
    def select_cross_cache(self, indices: torch.Tensor) -> None:
        for layer in self.backbone.layers:
            if isinstance(layer, DecoderLayerWithCross):
                ...
                if layer._exact_route is not None:
                    layer._exact_route = layer._exact_route.index_select(indices)
