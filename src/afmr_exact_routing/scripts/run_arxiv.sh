...
if [[ -n "${AFMR_OUTPUT_DIR:-}" ]]; then
  ...
  OUTPUT_DIR="${ROOT}/runs/afmr_exact_routing/arxiv_direct_projection_${COPY_VARIANT}"
else
  OUTPUT_DIR="${ROOT}/runs/afmr_exact_routing/arxiv_exact_routing_${COPY_VARIANT}"
fi
...
echo "Query-dependent exact-token routing: $([[ "${AFMR_BRIDGE_MODE}" == afmr ]] && echo true || echo false)"
