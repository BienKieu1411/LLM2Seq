...
AFMR_ARCHITECTURE="${AFMR_ARCHITECTURE:-afmr_exact_routing}"
...
RUN_ROOT="${AFMR_OUTPUT_DIR:-${ROOT}/runs/afmr_exact_routing/pubmed_${BRIDGE_VARIANT}_${COPY_VARIANT}}"
GENERATED_CONFIG_DIR="${RUN_ROOT}/configs"
LOG_DIR="${ROOT}/logs/afmr_exact_routing"
...
[[ "${AFMR_ARCHITECTURE}" == afmr_exact_routing ]] || die "AFMR_ARCHITECTURE must be afmr_exact_routing"
[[ "${AFMR_BRIDGE_MODE}" == afmr || "${AFMR_BRIDGE_MODE}" == direct_projection ]] || die "AFMR_BRIDGE_MODE must be afmr or direct_projection"
[[ "${AFMR_SALIENCE_WEIGHT}" == 0 || "${AFMR_SALIENCE_WEIGHT}" == 0.0 ]] || die "Exact routing is CE-only; AFMR_SALIENCE_WEIGHT must be 0"

echo "=== AFMR exact-routing PubMed sequential benchmark ==="
...
echo "=== Exact-token routing: $([[ "${AFMR_BRIDGE_MODE}" == afmr ]] && echo true || echo false) ==="
...
run_one() {
  ...
  local base_config="${ROOT}/configs/afmr_pubmed.yaml"
  make_config "${base_config}" "${config_path}" "${encoder}" "${output_dir}"
  echo "=== Training ${name} ==="
