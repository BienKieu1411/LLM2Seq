...
def main() -> None:
    seed_everything(7)
    config = load_config(ROOT / "configs/exact_routing_smoke.yaml")
    ...
    for name, parameter in model.named_parameters():
        if "exact_router" in name and parameter.requires_grad:
            if parameter.grad is None or not torch.isfinite(parameter.grad).all():
                raise RuntimeError(f"Exact-routing CE gradient is missing or non-finite: {name}")
