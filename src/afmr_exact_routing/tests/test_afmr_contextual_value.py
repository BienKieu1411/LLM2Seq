...
from eviseq_afmr.config import contextual_value_settings, exact_routing_settings, load_config, validate_config
...
def test_runner_materialization_selects_full_and_ablation_graphs(script, mode, context, tmp_path):
    ...
    if script == "run_pubmed_pair.sh":
        args += ["afmr_exact_routing", mode, "true", context, "0"]
    else:
    ...
    assert exact_routing_settings(result["architecture"])["enabled"] is (mode == "afmr")
