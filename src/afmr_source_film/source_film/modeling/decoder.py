...
def _inverse_bounded_gate(value: float, maximum: float) -> float:
    if not 0.0 < value < maximum <= 1.0:
        raise ValueError("source-film gate must satisfy 0 < init < max <= 1")
    return math.log(value / (maximum - value))


class SourceConditionedAdaptiveNorm(nn.Module):
    """Low-rank FiLM on a decoder pre-attention norm.

    Up projections are zero-initialized, giving exact baseline parity at
    initialization while retaining a non-zero gate so gradients can train the
    branch immediately.  The relative-RMS cap prevents a controller from
    overwhelming the pretrained hidden state during early fine-tuning.
    """
    ...
    def __init__(
        ...
        super().__init__()
        if min(hidden_size, controller_dim, rank) <= 0:
            raise ValueError("source-film dimensions must be positive")
        if not 0.0 < gate_init < gate_max <= 1.0:
            raise ValueError("source-film gate must satisfy 0 < init < max <= 1")
        if not 0.0 < max_relative_rms <= 1.0:
            raise ValueError("source-film max_relative_rms must lie in (0, 1]")
        self.hidden_size = int(hidden_size)
        self.controller_dim = int(controller_dim)
        self.max_relative_rms = float(max_relative_rms)
        self.controller_down = nn.Linear(self.controller_dim, int(rank), bias=False)
        self.scale_up = nn.Linear(int(rank), self.hidden_size, bias=False)
        self.shift_up = nn.Linear(int(rank), self.hidden_size, bias=False)
        nn.init.zeros_(self.scale_up.weight)
        nn.init.zeros_(self.shift_up.weight)
        self.gate_raw = nn.Parameter(torch.tensor(_inverse_bounded_gate(gate_init, gate_max)))
        self.gate_max = float(gate_max)

    def forward(self, hidden_states: torch.Tensor, controller: torch.Tensor | None) -> torch.Tensor:
        if controller is None:
            return hidden_states
        if controller.ndim != 2 or controller.shape[0] != hidden_states.shape[0]:
            raise ValueError("source-film controller must be [batch, controller_dim]")
        control = self.controller_down(controller.float())
        scale = self.scale_up(control).float()[:, None, :]
        shift = self.shift_up(control).float()[:, None, :]
        delta = scale * hidden_states.float() + shift
        radius = self.max_relative_rms * hidden_states.detach().float().square().mean(-1, keepdim=True).sqrt()
        delta_rms = delta.square().mean(-1, keepdim=True).sqrt()
        # The cap is a bounded routing coefficient, not a second objective.
        # Detaching it avoids the 0/0 derivative at the exact-parity init while
        # retaining gradients through both low-rank output projections.
        cap = (radius / delta_rms.clamp_min(1.0e-6)).clamp(max=1.0).detach()
        delta = delta * cap
        gate = self.gate_max * torch.sigmoid(self.gate_raw.float())
        return (hidden_states.float() + gate * delta).to(hidden_states.dtype)
...
class DecoderLayerWithCross(GradientCheckpointingLayer):
    def __init__(
        ...
        index: int,
        value_residual_max_relative_rms: float = 0.0,
        source_film_config: Optional[dict[str, Any]] = None,
    ):
        super().__init__()
        if not 0.0 < gate_init < gate_max <= 1.0:
        ...
        film = source_film_config or {}
        self.source_film = None
        if bool(film.get("enabled", False)):
            self.source_film = SourceConditionedAdaptiveNorm(
                int(config.hidden_size),
                int(film.get("controller_dim", 256)),
                int(film.get("rank", 128)),
                float(film.get("gate_init", 0.05)),
                float(film.get("gate_max", 0.20)),
                float(film.get("max_relative_rms", 0.10)),
            )
        # Kept as an attribute instead of a forward kwarg because HF gradient
        # checkpoint wrappers reject unknown kwargs and recompute layer calls.
        self.source_controller: Optional[torch.Tensor] = None
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if self.source_film is not None:
            normalized = self.source_film(normalized, self.source_controller)
    ...
    def set_source_controller(self, controller: Optional[torch.Tensor]) -> None:
...
class QwenCrossDecoder(nn.Module):
    def __init__(
        ...
        film_config = dict(config.get("source_film", {}))
        film_config.setdefault("controller_dim", int(config.get("source_film_controller_dim", 256)))
    ...
    def set_source_controller(self, controller: Optional[torch.Tensor]) -> None:
        for layer in self.backbone.layers:
            if isinstance(layer, DecoderLayerWithCross):
                layer.set_source_controller(controller)
        ...
        if not trainable:
            ...
            for layer in self.backbone.layers:
                if isinstance(layer, DecoderLayerWithCross):
                    ...
                    # Adaptive norms are part of the bridge, so keep them
                    # trainable during the interface warm-up even while the
                    # pretrained decoder backbone remains frozen.
                    if layer.source_film is not None:
                        layer.source_film.requires_grad_(True)
