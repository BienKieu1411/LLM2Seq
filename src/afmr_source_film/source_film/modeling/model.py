...
class EviSeqAFMR(nn.Module):
    def __init__(self, config: dict[str, Any]):
        ...
        decoder_options = dict(config["decoder"])
        architecture = config["architecture"]
        decoder_options["source_film"] = {
            **dict(decoder_options.get("source_film", {})),
            "enabled": architecture.get("bridge_mode", "source_film") == "source_film",
            "controller_dim": int(architecture.get("controller_dim", 256)),
            "rank": int(architecture.get("film_rank", decoder_options.get("source_film", {}).get("rank", 128))),
            "gate_init": float(architecture.get("film_gate_init", 0.05)),
            "gate_max": float(architecture.get("film_gate_max", 0.20)),
            "max_relative_rms": float(architecture.get("film_max_relative_rms", 0.10)),
        }
        self.decoder = QwenCrossDecoder(
            ...
            value_residual_max_relative_rms=0.0,
        )
    ...
    def forward(
        ...
        **copy_inputs: torch.Tensor,
        ...
        self.decoder.set_source_controller(bridge.controller if self.bridge.bridge_mode == "source_film" else None)
        ...
        loss_salience = None
        total_loss = loss_ce
        if labels is not None and self.salience_loss_weight > 0:
            raise ValueError("source-film uses CE-only training; salience supervision must be disabled")
