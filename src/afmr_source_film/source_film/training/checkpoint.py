...
def architecture_spec(config: dict[str, Any]) -> dict[str, Any]:
    ...
    spec = {
        "graph_version": "source_conditioned_adaptive_norm_v1",
        "architecture": arch.get("name"),
        "controller_dim": int(arch.get("controller_dim", 0)),
        "film_rank": int(arch.get("film_rank", 0)),
        "film_gate_max": float(arch.get("film_gate_max", 0.0)),
        "film_max_relative_rms": float(arch.get("film_max_relative_rms", 0.0)),
        "cross_attention_every": int(decoder.get("cross_attention_every", 1)),
        "cross_gate_max": float(decoder.get("cross_gate_max", 0.0)),
    }
    spec["bridge_mode"] = str(arch.get("bridge_mode", "source_film"))
    salience_weight = float(config.get("training", {}).get("salience_loss_weight", 0.0))
    if salience_weight != 0:
        raise ValueError("source-film checkpoints require CE-only training")
