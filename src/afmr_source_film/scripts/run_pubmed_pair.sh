...
AFMR_ARCHITECTURE="${AFMR_ARCHITECTURE:-source_conditioned_adaptive_norm}"
AFMR_BRIDGE_MODE="${AFMR_BRIDGE_MODE:-source_film}"
...
[[ "${AFMR_SALIENCE_WEIGHT}" == 0 || "${AFMR_SALIENCE_WEIGHT}" == 0.0 ]] || {
  echo "source-film is CE-only; AFMR_SALIENCE_WEIGHT must be 0" >&2
  exit 1
}
COPY_VARIANT=lm
[[ "${AFMR_GROUNDED_COPY}" == false ]] || COPY_VARIANT=copy
BRIDGE_VARIANT=source_film
[[ "${AFMR_BRIDGE_MODE}" == direct_projection ]] && BRIDGE_VARIANT=direct_projection
if [[ -n "${AFMR_OUTPUT_DIR:-}" ]]; then
  RUN_ROOT="${AFMR_OUTPUT_DIR}"
elif [[ "${BRIDGE_VARIANT}" == source_film ]]; then
  RUN_ROOT="${ROOT}/runs/source_film/pubmed_pair_${AFMR_ARCHITECTURE}_${COPY_VARIANT}"
else
...
[[ "${AFMR_ARCHITECTURE}" == source_conditioned_adaptive_norm ]] || die "Unsupported AFMR_ARCHITECTURE"
[[ "${AFMR_BRIDGE_MODE}" == source_film || "${AFMR_BRIDGE_MODE}" == direct_projection ]] || die "AFMR_BRIDGE_MODE must be source_film or direct_projection"
...
echo "=== Source-conditioned adaptive norm: $([[ "${AFMR_BRIDGE_MODE}" == source_film ]] && echo true || echo false) ==="
...
from source_film.config import load_config, validate_config
...
config["decoder"]["source_film"]["enabled"] = bridge_mode == "source_film"
