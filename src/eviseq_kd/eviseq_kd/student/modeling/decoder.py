...
class QwenCopiedCrossAttention(nn.Module):
    ...
    def __init__(
        ...
        self._memory_cache_signature: Optional[tuple[Any, ...]] = None
    ...
    @staticmethod
    def _memory_signature(memory: torch.Tensor) -> tuple[Any, ...]:
        return (
            int(memory.data_ptr()),
            int(memory.storage_offset()),
            tuple(int(value) for value in memory.shape),
            str(memory.device),
            str(memory.dtype),
        )
    ...
    def forward(
        ...
        signature = self._memory_signature(encoder_hidden_states) if encoder_hidden_states.ndim == 3 else None
        cached = self._memory_cache if not self.training and signature == self._memory_cache_signature else None
        if cached is None:
            ...
            if not self.training:
                self._memory_cache = (key.contiguous(), value.contiguous())
    ...
    def prepare_memory_cache(self, encoder_hidden_states: torch.Tensor) -> None:
        if encoder_hidden_states.ndim != 3:
            raise ValueError("Cross-attention cache requires one routed [B, S, D] memory")
        self._memory_cache = tuple(value.contiguous() for value in self._project_memory(encoder_hidden_states))
        self._memory_cache_signature = self._memory_signature(encoder_hidden_states)

    def clear_memory_cache(self) -> None:
        self._memory_cache = None
        self._memory_cache_signature = None
