# Kế hoạch tích hợp AFMR

## 1. Quyết định đã chốt

Đây là bản kế hoạch implementation sau deep research và hai vòng phản biện chéo của `critic_gradient`, `critic_architecture` và `critic_eval_novelty`. Package AFMR đã được dựng trong folder này; checkpoint PubMed và kết quả ROUGE vẫn chưa có.
...
Main AFMR dùng **copy-mass-preserving capped simplex**. Semantic branch có
...
Các module bên dưới nằm trong `src/afmr_core`; acceptance evidence được ghi ở `IMPLEMENTATION_EVIDENCE.md`.
...
| `configs/afmr.yaml` | Main và named controls | Không kế thừa silent defaults của v4 |
...
| `semantic_read.key_source` | `H0` | `M` ở `AFMR-KM` |
| `semantic_read.value_source` | `H0` | Không dùng `M` làm value |
| `semantic_read.semantic_prior_scale` | `1.0` | `0.0` cho `AFMR-KM` mặc định |
| `semantic_read.inner_gate` | `false` | `true` + `gate_init=.05` chỉ cho `legacy_semantic` |
| `semantic_read.cap_mode` | `smooth_relative_rms` | `legacy_copy_mixture` chỉ cho endpoint tái lập v2 |
...
| `readout.mode` | `copy_mass_preserving_capped_simplex` | `legacy_copy_mixture`, `independent_capped_simplex`, `hidden_interpolation` |
...
2. **Dựng legacy endpoints.** `base_only` phải tương đương `new`; `legacy_semantic` phải tương đương v2 flat. Dùng cùng pretrained base state theo từng seed.
...
| C2 | V2 endpoint | `legacy_semantic` trùng probability/loss/gradient v2 flat; raw logits chỉ so sánh khi control dùng gauge `Zs`, vì main gauge là `Z0` |
...
- D: AFMR-zero-parity, `K=H0,V=H0,Wo=0`; chỉ dùng để kiểm tra endpoint/smoke.
- E: AFMR-tiny, chỉ đổi `Wo` init; candidate training chính sau pilot.
...
## 9. Hướng mở sau khi AFMR đã được cô lập
...
SDACL/contrastive training được giữ ở nhánh nghiên cứu sau, không dùng để che failure của architecture CE-only. Nếu AFMR tăng ROUGE nhưng chưa có bằng chứng giảm hallucination, phải báo riêng hai kết quả.
...
Không có checkpoint/predictions lịch sử đầy đủ để suy ra causal attribution từ các điểm ROUGE đã báo. Plan này đưa ra cách kiểm chứng và điều kiện bác bỏ; nó không phải cam kết AFMR sẽ thắng.
