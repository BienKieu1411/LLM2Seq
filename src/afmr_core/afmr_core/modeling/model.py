    ...
    def encode_source(
        ...
        **copy_inputs: torch.Tensor,
        ...
        if bridge.value_memory is None:
            raise ValueError("AFMR requires architecture.name=afmr_value_anchor and bridge.value_memory")
        ...
        if bridge.memory.dtype != decoder_dtype:
            bridge = BridgeState(
                ...
                value_memory=bridge.value_memory.to(decoder_dtype),
            )
        ...
        if self.decoder.semantic_reader is not None:
            bridge.semantic_state = self.decoder.semantic_reader.prepare(
                H0=bridge.value_memory,
                M=bridge.memory,
                source_mask=bridge.content_mask,
                source_bias=bridge.source_bias,
            )
        elif copy_inputs:
    ...
    def forward(
        ...
        **copy_inputs: torch.Tensor,
        ...
        logits, _, loss_ce = self.decoder(
            ...
            semantic_state=bridge.semantic_state,
        )
