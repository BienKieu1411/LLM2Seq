...
class AdaptiveFullMemoryResidualBridge(nn.Module):
    def __init__(self, encoder_hidden: int, decoder_hidden: int, config: dict, gradient_checkpointing: bool = False):
        ...
        architecture_name = str(config.get("name", "afmr_v1"))
        self.hyper_operator = architecture_name == "afmr_hyper_operator"
        self.value_anchored = architecture_name in {"afmr_value_anchor", "afmr_hyper_operator"}
        ...
        # The hyper-operator uses the controller only to condition decoder-side
        # low-rank K/V projections.  Its source path is the untouched direct
        # projection; none of AFMR's depth/feature/focus residuals are built.
        if self.hyper_operator:
    ...
    def forward(
        ...
        if self.hyper_operator:
            ...
            return BridgeState(memory, memory_mask, content, source_bias, controller, memory)
