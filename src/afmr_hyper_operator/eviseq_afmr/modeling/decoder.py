...
class ConditionalLowRankModulator(nn.Module):
    """Document-conditioned mixture of low-rank projection residuals."""
    ...
    def __init__(
        ...
        self.down = nn.Parameter(torch.empty(self.num_experts, self.rank, self.input_dim))
        self.up = nn.Parameter(torch.zeros(self.num_experts, self.output_dim, self.rank))
        self.router = nn.Linear(int(controller_dim), self.num_experts)
        self.gate_raw = nn.Parameter(
            torch.tensor(math.log(float(gate_init) / (float(gate_max) - float(gate_init))), dtype=torch.float32)
        )
        nn.init.normal_(self.down, mean=0.0, std=self.input_dim**-0.5)
        # Output factors are zero, so this preserves exact anchor parity while
        # allowing document-conditioned expert gradients on the first update.
        nn.init.normal_(self.router.weight, std=0.1 * int(controller_dim) ** -0.5)
        nn.init.zeros_(self.router.bias)
        self.enabled = True

    def forward(
        self, inputs: torch.Tensor, base_projection: torch.Tensor, controller: Optional[torch.Tensor]
    ) -> torch.Tensor:
        if not self.enabled:
            return base_projection
        if controller is None:
            raise ValueError("Document-conditioned cross-attention requires an encoder controller")
        if inputs.ndim != 3 or base_projection.shape[:2] != inputs.shape[:2]:
            raise ValueError("Low-rank modulation expects token-aligned [B,L,D] tensors")
        if controller.shape != (inputs.shape[0], self.router.in_features):
            raise ValueError("Controller batch or width does not match the hyper-operator")

        source = inputs.float()
        weights = torch.softmax(self.router(controller.float()), dim=-1)
        reduced = torch.einsum("bli,eri->bler", source, self.down.float())
        # Contract the document mixture into the small up matrices first.
        # Materializing [B, L, E, O] would cost several GiB for PubMed.
        batch, length = source.shape[:2]
        weighted_up = (weights[:, :, None, None] * self.up.float()[None]).transpose(-1, -2)
        delta = torch.bmm(
            reduced.reshape(batch, length, self.num_experts * self.rank),
            weighted_up.reshape(batch, self.num_experts * self.rank, self.output_dim),
        )
        gate = self.gate_max * torch.sigmoid(self.gate_raw.float())
        delta = gate * delta

        base = base_projection.float()
        radius = self.max_relative_rms * base.detach().square().mean(dim=-1, keepdim=True).sqrt()
        # A hard min(radius / RMS(delta), 1) has an undefined derivative at a
        # zero-initialized delta.  This smooth cap is exactly zero-preserving,
        # has finite gradients at initialization, and still bounds the final
        # residual RMS by ``radius``.
        scale = radius / (radius.square() + delta.square().mean(dim=-1, keepdim=True) + 1.0e-12).sqrt()
        return (base + scale * delta).to(base_projection.dtype)
...
class CopiedCrossAttention(nn.Module):
    def __init__(
        ...
        hyper_operator: Optional[dict[str, Any]] = None,
        ...
        settings = dict(hyper_operator or {})
        self.hyper_enabled = bool(settings.get("enabled", False))
        self.hyper_query: Optional[ConditionalLowRankModulator] = None
        self.hyper_key: Optional[ConditionalLowRankModulator] = None
        self.hyper_value: Optional[ConditionalLowRankModulator] = None
        if self.hyper_enabled:
            common = {
                "controller_dim": int(controller_dim),
                "num_experts": int(settings["num_experts"]),
                "rank": int(settings["rank"]),
                "gate_init": float(settings["gate_init"]),
                "gate_max": float(settings["gate_max"]),
                "max_relative_rms": float(settings["max_relative_rms"]),
            }
            key_value_dim = self.num_kv_heads * self.head_dim
            self.hyper_key = ConditionalLowRankModulator(self.hidden_size, key_value_dim, **common)
            self.hyper_value = ConditionalLowRankModulator(self.hidden_size, key_value_dim, **common)
            if bool(settings.get("modulate_query", False)):
                self.hyper_query = ConditionalLowRankModulator(self.hidden_size, self.hidden_size, **common)
    ...
    def _memory_kv(
        ...
        controller: Optional[torch.Tensor] = None,
        ...
        if self.hyper_key is not None:
            key_projection = self.hyper_key(hidden, key_projection, controller)
        key = self.k_norm(key_projection.view(batch, length, self.num_kv_heads, self.head_dim)).transpose(1, 2)
        value_hidden = hidden if value_memory is None else self.memory_norm(value_memory)
        value_projection = self.v_proj(value_hidden)
        if self.hyper_value is not None:
            value_projection = self.hyper_value(value_hidden, value_projection, controller)
        value = value_projection.view(batch, length, self.num_kv_heads, self.head_dim).transpose(1, 2)
    ...
    def forward(
        ...
        query_projection = self.q_proj(query_states)
        if self.hyper_query is not None:
            query_projection = self.hyper_query(query_states, query_projection, controller)
        query = self.q_norm(query_projection.view(batch, query_length, self.num_heads, self.head_dim)).transpose(1, 2)
        key, value = (
            self._cache
            if self._cache is not None and not self.training
            else self._memory_kv(memory, value_memory, value_residual, controller)
        )
    ...
    def prepare_cache(
        ...
        self._cache = tuple(
            value.contiguous() for value in self._memory_kv(memory, value_memory, value_residual, controller)
        )

    def clear_cache(self) -> None:
        self._cache = None

    def set_hyper_enabled(self, enabled: bool) -> None:
        for modulator in (self.hyper_query, self.hyper_key, self.hyper_value):
            if modulator is not None:
                modulator.enabled = bool(enabled)
...
class DecoderLayerWithCross(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        encoder_controller: Optional[torch.Tensor] = None,
        **kwargs: Any,
    ) -> torch.Tensor:
    ...
    def prepare_cache(
        ...
        self.cross.prepare_cache(memory, value_memory, value_residual, controller)

    def clear_cache(self) -> None:
        self.cross.clear_cache()

    def set_hyper_enabled(self, enabled: bool) -> None:
        self.cross.set_hyper_enabled(enabled)
...
class QwenCrossDecoder(nn.Module):
    def __init__(
        ...
        # Hyper modules add randomly initialized low-rank factors to every
        # copied cross-attention layer.  Preserve the external RNG stream so
        # a same-seed direct-projection control receives identical weights and
        # data-loader randomness; the experts still differ across layers.
        shared_rng_state = torch.get_rng_state() if bool((hyper_operator or {}).get("enabled", False)) else None
        wrapped = []
    ...
    def prepare_cross_cache(
        ...
        for layer in self.backbone.layers:
            if isinstance(layer, DecoderLayerWithCross):
                layer.prepare_cache(memory, value_memory, value_residual, controller)
    ...
    def set_hyper_enabled(self, enabled: bool) -> None:
        self.clear_cross_cache()
        for layer in self.backbone.layers:
            if isinstance(layer, DecoderLayerWithCross):
                layer.set_hyper_enabled(enabled)
