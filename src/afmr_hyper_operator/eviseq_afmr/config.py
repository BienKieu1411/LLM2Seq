...
def hyper_operator_settings(architecture: dict[str, Any]) -> dict[str, Any]:
    """Resolve the document-conditioned low-rank cross-attention operator."""
    defaults = {
        "num_experts": 4,
        "rank": 16,
        "gate_init": 0.05,
    ...
    supplied = architecture.get("hyper_operator", {})
    if not isinstance(supplied, dict):
        raise ValueError("architecture.hyper_operator must be a mapping")
    _check_keys(supplied, set(defaults), "architecture.hyper_operator")
    settings = {**defaults, **supplied}
    for key in ("num_experts", "rank"):
        value = settings[key]
        if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
            raise ValueError(f"architecture.hyper_operator.{key} must be a positive integer")
    for key in ("gate_init", "gate_max", "max_relative_rms"):
        value = float(settings[key])
        if not math.isfinite(value):
            raise ValueError(f"architecture.hyper_operator.{key} must be finite")
        settings[key] = value
    if not 0 < settings["gate_init"] < settings["gate_max"] <= 1:
        raise ValueError("hyper_operator gate requires 0 < gate_init < gate_max <= 1")
    if not 0 < settings["max_relative_rms"] <= 1:
        raise ValueError("hyper_operator.max_relative_rms must lie in (0, 1]")
    if not isinstance(settings["modulate_query"], bool):
        raise ValueError("architecture.hyper_operator.modulate_query must be a boolean")
    settings["enabled"] = (
        architecture.get("name") == "afmr_hyper_operator"
        and architecture.get("bridge_mode", "afmr") != "direct_projection"
    )
    return settings
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if architecture.get("name") not in {"afmr_v1", "afmr_value_anchor", "afmr_hyper_operator"}:
        raise ValueError("architecture.name must be afmr_v1, afmr_value_anchor or afmr_hyper_operator")
    ...
    hyper_operator_settings(architecture)
    ...
    if salience_weight > 0 and architecture.get("name") == "afmr_hyper_operator":
        raise ValueError("afmr_hyper_operator is a CE-only architecture; salience_loss_weight must be zero")
    data = config["data"]
