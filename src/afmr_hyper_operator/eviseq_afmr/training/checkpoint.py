...
from ..config import contextual_value_settings, hyper_operator_settings
...
def architecture_spec(config: dict[str, Any]) -> dict[str, Any]:
    ...
    hyper = hyper_operator_settings(arch)
    if hyper["enabled"]:
        spec["hyper_operator"] = {
            "mechanism": "document_conditioned_layerwise_lowrank_kv",
            **{key: value for key, value in hyper.items() if key != "enabled"},
        }
