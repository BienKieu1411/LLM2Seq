...
def main() -> None:
    seed_everything(7)
    config = load_config(ROOT / "configs/hyper_operator_smoke.yaml")
    ...
    for name, parameter in model.named_parameters():
        if "hyper_operator" in name and parameter.requires_grad:
            if parameter.grad is None or not torch.isfinite(parameter.grad).all():
                raise RuntimeError(f"Hyper-operator CE gradient is missing or non-finite: {name}")
