...
if [[ -n "${AFMR_OUTPUT_DIR:-}" ]]; then
  ...
  OUTPUT_DIR="${ROOT}/runs/afmr_hyper_operator/arxiv_direct_projection_${COPY_VARIANT}"
else
  OUTPUT_DIR="${ROOT}/runs/afmr_hyper_operator/arxiv_${COPY_VARIANT}"
fi
...
echo "=== AFMR hyper-operator ArXiv ==="
