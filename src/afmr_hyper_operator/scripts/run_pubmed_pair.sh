...
AFMR_ARCHITECTURE="${AFMR_ARCHITECTURE:-afmr_hyper_operator}"
...
if [[ -n "${AFMR_OUTPUT_DIR:-}" ]]; then
  ...
  RUN_ROOT="${ROOT}/runs/afmr_hyper_operator/pubmed_pair_${AFMR_ARCHITECTURE}_local_topdown_value_${COPY_VARIANT}"
elif [[ "${BRIDGE_VARIANT}" == afmr && "${AFMR_SALIENCE_WEIGHT}" != 0 && "${AFMR_SALIENCE_WEIGHT}" != 0.0 ]]; then
  RUN_ROOT="${ROOT}/runs/afmr_hyper_operator/pubmed_pair_${AFMR_ARCHITECTURE}_evidence_prior_${COPY_VARIANT}"
elif [[ "${BRIDGE_VARIANT}" == afmr ]]; then
  RUN_ROOT="${ROOT}/runs/afmr_hyper_operator/pubmed_pair_${AFMR_ARCHITECTURE}_${COPY_VARIANT}"
else
  RUN_ROOT="${ROOT}/runs/afmr_hyper_operator/pubmed_pair_${AFMR_ARCHITECTURE}_${BRIDGE_VARIANT}_${COPY_VARIANT}"
fi
GENERATED_CONFIG_DIR="${RUN_ROOT}/configs"
LOG_DIR="${ROOT}/logs/afmr_hyper_operator"
...
[[ "${AFMR_ARCHITECTURE}" == afmr_hyper_operator ]] || die "AFMR_ARCHITECTURE must be afmr_hyper_operator"
[[ "${AFMR_BRIDGE_MODE}" == afmr || "${AFMR_BRIDGE_MODE}" == direct_projection ]] || die "AFMR_BRIDGE_MODE must be afmr or direct_projection"
[[ "${AFMR_SALIENCE_WEIGHT}" == 0 || "${AFMR_SALIENCE_WEIGHT}" == 0.0 ]] || die "afmr_hyper_operator is CE-only; AFMR_SALIENCE_WEIGHT must be 0"

echo "=== AFMR hyper-operator PubMed sequential benchmark ==="
...
echo "=== Document-conditioned K/V hyper-operator: $([[ "${AFMR_BRIDGE_MODE}" == afmr ]] && echo true || echo false) ==="
echo "=== Objective: token-level CE only ==="
