...
def prompted_source_features(
    ...
    if bool(data_config.get("use_chat_template", True)) and getattr(tokenizer, "chat_template", None):
        sentinel = "<GENBRIDGE_SOURCE_7F3A>"
...
def decoder_prompt_ids(tokenizer: Any, data_config: Dict[str, Any]) -> List[int]:
    """Return the identical decoder seed used by training and generation.

    Qwen3 has no ordinary BOS token. Starting its pretrained decoder from EOS
    alone exposes an unconditioned English continuation prior until the new
    cross-attention has become strong. A short task/language prefix anchors the
...
class EvidenceSeq2SeqDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor]:
        ...
        target_labels = torch.tensor(target_ids, dtype=torch.long)
        labels = torch.cat(
            [
                torch.full((ignored_prefix_length,), -100, dtype=torch.long),
                target_labels,
            ]
        )
        return {
            ...
            "decoder_input_ids": torch.cat(
                [
                    torch.tensor(decoder_seed, dtype=torch.long),
                    target_labels[:-1],
                ]
            ),
            "labels": labels,
...
class EvidenceSeq2SeqCollator:
    ...
    @staticmethod
    def _left_pad_1d(tensors: Iterable[torch.Tensor], length: int, value: float) -> torch.Tensor:
        result = []
        for tensor in tensors:
            tensor = tensor[-length:]
            padding = torch.full((length - tensor.numel(),), value, dtype=tensor.dtype)
            result.append(torch.cat([padding, tensor]))
        return torch.stack(result)
    ...
    def __call__(self, features: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            # follow the last real source token without a rotary-position gap.
            "input_ids": self._left_pad_1d((item["input_ids"] for item in features), source_length, self.pad_token_id),
