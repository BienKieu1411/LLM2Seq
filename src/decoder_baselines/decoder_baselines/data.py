"""Prompt-faithful causal summarization data utilities."""
...
def encode_prompt(
    ...
    *,
    ...
    """Build the same instruction text as T5Gemma, without a chat template.

    T5Gemma encodes ``source_prefix + source`` as its encoder input and
    supervises the target separately.  The causal control concatenates that
    exact text with the target and masks every prompt token in the loss.  A
    configurable suffix exists for experiments, but all bundled recipes leave
    it empty so the benchmark prompt remains T5Gemma-compatible.
    """

    source_limit = int(data["max_source_length"])
    if max_total_length is not None:
        # Reserve the response budget during generation.  The prompt is
        # truncated from the document tail only; the instruction and any
        # response marker remain intact.
        prefix = _ids(tokenizer, str(data.get("source_prefix", "")), add_special_tokens=True)
        suffix = _ids(tokenizer, str(data.get("prompt_suffix", "")), add_special_tokens=False)
...
class CausalSummarizationDataset(Dataset[dict[str, torch.Tensor]]):
    """Dataset with prompt-masked labels and explicit context accounting."""
...
class CausalCollator:
    """Right-pad causal examples while preserving the prompt ignore mask."""
