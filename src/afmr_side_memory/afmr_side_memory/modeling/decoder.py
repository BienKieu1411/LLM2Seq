...
class DecoderLayerWithCross(GradientCheckpointingLayer):
    def __init__(
        ...
        index: int,
        value_residual_max_relative_rms: float = 0.0,
        side_attention: bool = False,
        side_relative_rms_cap: float = 0.10,
    ):
        super().__init__()
        ...
        self.side_cross = None
        self.side_norm = None
        self.side_gate = None
        self.side_relative_rms_cap = float(side_relative_rms_cap)
        if side_attention:
            if not 0 < self.side_relative_rms_cap <= 1:
                raise ValueError("side_relative_rms_cap must lie in (0, 1]")
            self.side_norm = copy.deepcopy(base.input_layernorm)
            # This is a second, independently normalized attention. Side
            # tokens never share the base-token softmax or K/V projections.
            self.side_cross = CopiedCrossAttention(base.self_attn, base.input_layernorm, config, dropout)
            self.side_gate = nn.Parameter(torch.zeros((), dtype=torch.float32))

    def _bounded_side_delta(self, anchor: torch.Tensor, delta: torch.Tensor) -> torch.Tensor:
        """Smoothly cap each token's side update relative to its anchor RMS."""
        anchor_rms = anchor.detach().float().square().mean(-1, keepdim=True).sqrt()
        delta_float = delta.float()
        radius = self.side_relative_rms_cap * anchor_rms
        # Keep the zero-gate derivative finite: sqrt(delta^2) at delta=0 has
        # an undefined backward even though squaring it again cancels forward.
        scale = radius / torch.sqrt(radius.square() + delta_float.square().mean(-1, keepdim=True) + 1.0e-12)
        return (scale * delta_float).to(delta.dtype)
    ...
    def forward(
        ...
        side_memory_states: Optional[torch.Tensor] = None,
        side_memory_mask: Optional[torch.Tensor] = None,
        **kwargs: Any,
    ) -> torch.Tensor:
        ...
        if side_memory_states is not None:
            if self.side_cross is None or self.side_norm is None or self.side_gate is None:
                # Decoder layers outside the configured cadence intentionally
                # ignore the side bank.
                pass
            else:
                side = self.side_cross(
                    self.side_norm(hidden_states),
                    side_memory_states,
                    side_memory_mask,
                    None,
                )
                gated = torch.tanh(self.side_gate).to(side.dtype) * side
                hidden_states = hidden_states + self._bounded_side_delta(hidden_states, gated)
        residual = hidden_states
    ...
    def prepare_cache(
        ...
        value_memory: Optional[torch.Tensor] = None,
        value_residual: Optional[torch.Tensor] = None,
        side_memory: Optional[torch.Tensor] = None,
    ) -> None:
        self.cross.prepare_cache(memory, value_memory, value_residual)
        if self.side_cross is not None and side_memory is not None:
            self.side_cross.prepare_cache(side_memory)

    def clear_cache(self) -> None:
        self.cross.clear_cache()
        if self.side_cross is not None:
            self.side_cross.clear_cache()
...
class QwenCrossDecoder(nn.Module):
    def __init__(
        ...
        side_every = int(config.get("side_attention_every", 4))
        if side_every <= 0:
            raise ValueError("side_attention_every must be positive")
        side_cap = float(config.get("side_relative_rms_cap", 0.10))
        wrapped = []
        for index, layer in enumerate(list(self.backbone.layers)):
            ...
            wrapped.append(
                DecoderLayerWithCross(
                    ...
                    side_attention=side_memory_enabled and (index + 1) % side_every == 0,
                    side_relative_rms_cap=side_cap,
                )
            )
        self.backbone.layers = nn.ModuleList(wrapped)
        self.cross_attention_indices = tuple(range(len(wrapped)))
        self.side_attention_indices = tuple(
            index for index in range(len(wrapped)) if side_memory_enabled and (index + 1) % side_every == 0
        )
    ...
    def forward(
        ...
        outputs = self.backbone(
            ...
            side_memory_states=side_memory,
    ...
    def prepare_cross_cache(
        ...
        for layer in self.backbone.layers:
            if isinstance(layer, DecoderLayerWithCross):
                layer.prepare_cache(memory, value_memory, value_residual, side_memory)
    ...
    def select_cross_cache(self, indices: torch.Tensor) -> None:
        for layer in self.backbone.layers:
            if isinstance(layer, DecoderLayerWithCross) and layer.cross._cache is not None:
                ...
                if layer.side_cross is not None and layer.side_cross._cache is not None:
                    layer.side_cross._cache = tuple(value.index_select(0, indices) for value in layer.side_cross._cache)
        ...
        if not trainable:
            ...
            for layer in self.backbone.layers:
                if isinstance(layer, DecoderLayerWithCross):
                    ...
                    if layer.side_cross is not None:
                        for parameter in layer.side_cross.parameters():
                            parameter.requires_grad = True
                        layer.side_norm.requires_grad_(True)
                        assert layer.side_gate is not None
                        layer.side_gate.requires_grad = True
