...
def validate_config(config: dict[str, Any]) -> None:
    ...
    _check_keys(
        architecture,
        {
            ...
            "side_tokens",
            "side_num_heads",
            "side_resampler_dropout",
        },
        "architecture",
    )
    if architecture.get("name") != "afmr_side_memory":
        raise ValueError("architecture.name must be afmr_side_memory")
    if architecture.get("bridge_mode", "side_memory") not in {"side_memory", "direct_projection"}:
        raise ValueError("architecture.bridge_mode must be side_memory or direct_projection")
    if int(architecture.get("depth_taps", 1)) != 1:
        raise ValueError("The side-memory bridge consumes only the final encoder state; depth_taps must be 1")
    side_tokens = int(architecture.get("side_tokens", 24))
    if not 16 <= side_tokens <= 32:
        raise ValueError("architecture.side_tokens must lie in [16, 32]")
    if int(architecture.get("side_num_heads", 0)) <= 0:
        raise ValueError("architecture.side_num_heads must be positive")
    side_dropout = float(architecture.get("side_resampler_dropout", 0.0))
    if not 0 <= side_dropout < 1:
        raise ValueError("architecture.side_resampler_dropout must lie in [0, 1)")
    ...
    _check_keys(
        decoder,
        {
            ...
            "ce_chunk_size",
            "grounded_copy",
            "side_attention_every",
            "side_relative_rms_cap",
        },
        "decoder",
    )
    ...
    if int(decoder.get("side_attention_every", 4)) <= 0:
        raise ValueError("decoder.side_attention_every must be positive")
    if not 0 < float(decoder.get("side_relative_rms_cap", 0.10)) <= 1:
        raise ValueError("decoder.side_relative_rms_cap must lie in (0, 1]")
    ...
    if salience_weight != 0:
        raise ValueError("The side-memory architecture uses CE only; salience_loss_weight must be 0")
    data = config["data"]
