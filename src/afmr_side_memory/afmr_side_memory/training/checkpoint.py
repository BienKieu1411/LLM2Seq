...
def architecture_spec(config: dict[str, Any]) -> dict[str, Any]:
    ...
    spec = {
        "graph_version": "independent_side_memory_v1",
        "architecture": arch.get("name"),
        "bridge_mode": str(arch.get("bridge_mode", "side_memory")),
        "depth_taps": int(arch.get("depth_taps", 0)),
        "side_tokens": int(arch.get("side_tokens", 24)),
        "side_num_heads": int(arch.get("side_num_heads", 8)),
        "cross_attention_every": int(decoder.get("cross_attention_every", 1)),
        "cross_gate_max": float(decoder.get("cross_gate_max", 0.0)),
        "side_attention_every": int(decoder.get("side_attention_every", 4)),
        "side_relative_rms_cap": float(decoder.get("side_relative_rms_cap", 0.10)),
    }
