...
LOGGER = logging.getLogger("afmr_side_memory.runtime")
    ...
    for name in selected:
        ...
        collator = SummarizationCollator(
            ...
            salience_supervision=(
                ...
                and config["architecture"].get("bridge_mode", "side_memory") == "side_memory"
...
def train(
    ...
    *,
    ...
    try:
        ...
        model = SideMemorySummarizer(config).to(device=selected_device, dtype=torch.float32)
        counts = {
        ...
        if _is_main_process():
            LOGGER.info(
                "model parameters=%s total=%d | bridge=%s side_tokens=%d side_layers=%s | distributed=%s world_size=%d",
                counts,
                sum(p.numel() for p in model.parameters()),
                model.bridge.bridge_mode,
                int(config["architecture"].get("side_tokens", 24)),
                model.decoder.side_attention_indices,
                _distributed_active(),
...
def evaluate(
    ...
    *,
    ...
    model = SideMemorySummarizer(inference_config).to(device_obj)
