...
def generate_greedy(
    ...
    *,
    ...
    side_memory = getattr(bridge, "side_memory", None)
    side_memory_mask = getattr(bridge, "side_memory_mask", None)
    ...
    try:
        model.decoder.prepare_cross_cache(
            ...
            **({"side_memory": side_memory} if side_memory is not None else {}),
        )
        for step in range(max_new_tokens):
            ...
            logits, past, _ = model.decoder(
                ...
                **({"value_residual": value_residual} if value_residual is not None else {}),
                **({"side_memory": side_memory} if side_memory is not None else {}),
                **({"side_memory_mask": side_memory_mask} if side_memory_mask is not None else {}),
            ...
            if (
                ...
                if surviving.numel() != active_rows.numel():
                    ...
                    if side_memory is not None:
                        side_memory = side_memory.index_select(0, surviving)
                    if side_memory_mask is not None:
                        side_memory_mask = side_memory_mask.index_select(0, surviving)
