...
AFMR_BRIDGE_MODE="${AFMR_BRIDGE_MODE:-side_memory}"
...
if [[ -n "${AFMR_OUTPUT_DIR:-}" ]]; then
  ...
  OUTPUT_DIR="${ROOT}/runs/afmr_side_memory/arxiv_direct_projection_${COPY_VARIANT}"
else
  OUTPUT_DIR="${ROOT}/runs/afmr_side_memory/arxiv_side_memory_${COPY_VARIANT}"
fi
...
[[ "${AFMR_BRIDGE_MODE}" == side_memory || "${AFMR_BRIDGE_MODE}" == direct_projection ]] || die "AFMR_BRIDGE_MODE must be side_memory or direct_projection"
...
echo "Independent side memory: $([[ "${AFMR_BRIDGE_MODE}" == side_memory ]] && echo true || echo false)"
