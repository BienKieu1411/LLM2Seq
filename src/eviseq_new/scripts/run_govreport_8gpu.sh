...
CONFIG_TEMPLATE="${AFMR_CONFIG:-${ROOT}/configs/afmr_govreport.yaml}"
...
[[ "${#VISIBLE_GPUS[@]}" -eq 8 ]] || die "Set CUDA_VISIBLE_DEVICES to exactly eight GPU IDs"
SEEN_GPUS=","
...
if [[ "${RUN_MODE}" == eval ]]; then
  ...
  AFMR_OUTPUT_DIR="${AFMR_OUTPUT_DIR:-${ROOT}/runs/afmr/govreport_8gpu_$(date +%Y%m%d_%H%M%S)_$$}"
fi
OUTPUT_DIR="${AFMR_OUTPUT_DIR}"
mkdir -p "${OUTPUT_DIR}"
OUTPUT_DIR="$(cd "${OUTPUT_DIR}" && pwd)"
LOG_FILE="${OUTPUT_DIR}/run_govreport_8gpu.log"
...
print(f"Materialized GovReport config: {destination}")
PY

  echo "GPU: ${CUDA_VISIBLE_DEVICES}; train batch/GPU: ${TRAIN_BATCH_SIZE}; accumulation: ${GRADIENT_ACCUMULATION_STEPS}; global effective batch: $((8 * TRAIN_BATCH_SIZE * GRADIENT_ACCUMULATION_STEPS))"
...
echo "Evaluating GovReport test with eight independent GPU shards"
shards=()
pids=()
for shard_rank in "${!VISIBLE_GPUS[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES="${VISIBLE_GPUS[${shard_rank}]}" WORLD_SIZE=1 RANK=0 LOCAL_RANK=0 \
    "${PYTHON_BIN}" "${ROOT}/run_afmr.py" evaluate \
      ...
      --shard-rank "${shard_rank}" --num-shards 8 \
