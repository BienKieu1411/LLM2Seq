...
if [[ "${RUN_MODE}" == eval ]]; then
  ...
  AFMR_OUTPUT_DIR="${AFMR_OUTPUT_DIR:-${ROOT}/runs/afmr/govreport_${GPU_COUNT}gpu_$(date +%Y%m%d_%H%M%S)_$$}"
fi
OUTPUT_DIR="${AFMR_OUTPUT_DIR}"
mkdir -p "${OUTPUT_DIR}"
OUTPUT_DIR="$(cd "${OUTPUT_DIR}" && pwd)"
LOG_FILE="${OUTPUT_DIR}/run_govreport_${GPU_COUNT}gpu.log"
...
if Path(template).name == "afmr_govreport.yaml" and template_data.get("_base_") != "afmr_base.yaml":
    raise ValueError(
        f"{template} must inherit afmr_base.yaml, not {template_data.get('_base_')!r}; "
        "copy the updated eviseq_new GovReport config to the server"
    )
...
echo "Evaluating GovReport test with ${GPU_COUNT} independent GPU shards"
if (( GPU_COUNT == 1 )); then
