...
class DistributedBatchSampler(Sampler[list[int]]):
    """Shard deterministic batches across DDP ranks.

    Batches are formed once from the same seed and epoch on every rank, then
    strided by rank.  Padding with already-formed batches makes the number of
    local batches identical, which is required for every rank to enter the
    same number of backward/all-reduce calls.  At most ``world_size - 1``
    batches are repeated per epoch.
    """
    ...
    def __iter__(self):
        ...
        if self.shuffle and self.num_replicas > 1:
            # A rank processes one batch from each group at the same optimizer step.
            # Keep their padded sequence costs close so long-document batches do not
            # leave one GPU idle while the other approaches OOM.
