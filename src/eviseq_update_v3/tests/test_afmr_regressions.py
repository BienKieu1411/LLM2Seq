...
from eviseq_update_v3.data.collate import SummarizationCollator
from eviseq_update_v3.data.dataset import JsonlSummarizationDataset
from eviseq_update_v3.data.prepare import prepare_split
from eviseq_update_v3.data.schema import CanonicalRecord
from eviseq_update_v3.evaluation.generate import generate_greedy
from eviseq_update_v3.modeling.afmr import AdaptiveFullMemoryResidualBridge
from eviseq_update_v3.modeling.model import EviSeqAFMR
from eviseq_update_v3.modeling.outputs import EncoderState
from eviseq_update_v3.runtime import _TinyTokenizer, build_loaders, evaluate
from eviseq_update_v3.training.engine import AFMRTrainer, seed_everything
...
def test_partial_accumulation_matches_large_batch(tmp_path):
    cfg = config()
...
def test_training_log_is_compact_and_uses_accumulated_ce(caplog, tmp_path):
    cfg = config()
    cfg["experiment"]["output_dir"] = str(tmp_path)
    ...
    with caplog.at_level("INFO", logger="eviseq_update_v3.train"):
        metrics = trainer._run_epoch(loader, optimizer, "interface_warmup", True)
    messages = [record.getMessage() for record in caplog.records if record.name == "eviseq_update_v3.train"]
...
def test_resume_rejects_wrong_reference_before_model_load(tmp_path, monkeypatch):
    import eviseq_update_v3.runtime as runtime
