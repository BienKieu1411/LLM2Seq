...
from eviseq_update_v3.data.copy_alignment import COPY_INPUT_KEYS, align_copy_tokens, pad_copy_alignments
...
@pytest.mark.parametrize("semantic_read", [False, True])
def test_cached_copy_decoder_matches_full_prefix_after_compaction(dtype, semantic_read):
    config, _, batch, tokenizer = setup_model(semantic_read=semantic_read)
    ...
    with torch.no_grad():
        ...
        if semantic_read:
            torch.testing.assert_close(kwargs["copy_state"].semantic_values, state.copy_state.semantic_values[rows])
        step, _, _ = decoder(
            torch.tensor([[8]]),
            state.memory[rows],
