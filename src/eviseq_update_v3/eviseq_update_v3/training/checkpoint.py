...
def architecture_spec(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if decoder.get("query_cross_gate", False):
        spec["query_cross_gate"] = "headwise_identity_sigmoid_v3"
    ...
    if copy_config.get("enabled", False):
        ...
        if semantic_config.get("enabled", False):
            ...
            heads = int(semantic_config.get("num_heads", 1))
            if heads != 1:
                # Head count changes attention even though every weight shape
                # stays the same. Reject silently reinterpreting a checkpoint.
                spec["grounded_copy"]["semantic_read"].update(
