...
from eviseq_update.data.prepare import prepare_split
from eviseq_update.data.schema import CanonicalRecord
from eviseq_update.evaluation.generate import generate_greedy
from eviseq_update.modeling.afmr import AdaptiveFullMemoryResidualBridge
from eviseq_update.modeling.model import EviSeqAFMR
from eviseq_update.modeling.outputs import EncoderState
from eviseq_update.runtime import _TinyTokenizer, build_loaders, evaluate
...
def test_training_log_is_compact_and_uses_accumulated_ce(caplog, tmp_path):
    ...
    with caplog.at_level("INFO", logger="eviseq_update.train"):
        metrics = trainer._run_epoch(loader, optimizer, "interface_warmup", True)
    messages = [record.getMessage() for record in caplog.records if record.name == "eviseq_update.train"]
...
def test_resume_rejects_wrong_reference_before_model_load(tmp_path, monkeypatch):
    import eviseq_update.runtime as runtime
