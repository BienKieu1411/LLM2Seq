...
def contextual_value_settings(architecture: dict[str, Any]) -> dict[str, Any]:
    ...
    value_anchored_names = {"afmr_value_anchor", "afmr_adaptive_topdown"}
    if settings["enabled"] and architecture.get("name") not in value_anchored_names:
        raise ValueError("contextual_value requires a value-anchored architecture")
    return settings


def adaptive_topdown_settings(architecture: dict[str, Any]) -> dict[str, Any]:
    """Resolve and validate the optional adaptive regional key bridge."""
    defaults = {
        "enabled": False,
        "dim": 256,
        "num_heads": 4,
    ...
    supplied = architecture.get("adaptive_topdown", {})
    if not isinstance(supplied, dict):
        raise ValueError("architecture.adaptive_topdown must be a mapping")
    _check_keys(supplied, set(defaults), "architecture.adaptive_topdown")
    settings = {**defaults, **supplied}
    if not isinstance(settings["enabled"], bool):
        raise ValueError("architecture.adaptive_topdown.enabled must be a boolean")
    for key in ("dim", "num_heads", "mixer_layers", "region_width", "region_stride", "topdown_rank"):
        if int(settings[key]) <= 0:
            raise ValueError(f"architecture.adaptive_topdown.{key} must be a positive integer")
    if int(settings["dim"]) % int(settings["num_heads"]) != 0:
        raise ValueError("adaptive_topdown.dim must be divisible by num_heads")
    if int(settings["region_stride"]) > int(settings["region_width"]):
        raise ValueError("adaptive_topdown.region_stride must not exceed region_width")
    if not 0.0 < float(settings["gate_init"]) < float(settings["gate_max"]):
        raise ValueError("adaptive_topdown requires 0 < gate_init < gate_max")
    if not 0.0 < float(settings["gate_max"]) <= 1.0:
        raise ValueError("adaptive_topdown.gate_max must lie in (0, 1]")
    if not 0.0 < float(settings["max_relative_rms"]) <= 1.0:
        raise ValueError("adaptive_topdown.max_relative_rms must lie in (0, 1]")
    if not 0.0 < float(settings["output_init_rms"]) <= 0.1:
        raise ValueError("adaptive_topdown.output_init_rms must lie in (0, 0.1]")
    if architecture.get("bridge_mode", "afmr") == "direct_projection":
        settings["enabled"] = False
    return settings
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if architecture.get("name") not in {"afmr_v1", "afmr_value_anchor", "afmr_adaptive_topdown"}:
        raise ValueError("architecture.name must be afmr_v1, afmr_value_anchor or afmr_adaptive_topdown")
    ...
    topdown = adaptive_topdown_settings(architecture)
    if architecture.get("name") == "afmr_adaptive_topdown" and architecture.get("bridge_mode", "afmr") == "afmr":
        if not topdown["enabled"]:
            raise ValueError("afmr_adaptive_topdown requires architecture.adaptive_topdown.enabled=true")
