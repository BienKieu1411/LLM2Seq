...
from ..config import adaptive_topdown_settings, contextual_value_settings
...
class AdaptiveFullMemoryResidualBridge(nn.Module):
    def __init__(self, encoder_hidden: int, decoder_hidden: int, config: dict, gradient_checkpointing: bool = False):
        ...
        self.value_anchored = config.get("name", "afmr_v1") in {"afmr_value_anchor", "afmr_adaptive_topdown"}
        ...
        topdown = adaptive_topdown_settings(config)
        self.adaptive_topdown = None
        if topdown["enabled"]:
            # Keep the established AFMR initialization fixed so this branch is
            # the sole architectural difference in a controlled experiment.
            with torch.random.fork_rng(devices=[]):
                self.adaptive_topdown = AdaptiveTopDownKeyBridge(
                    self.decoder_hidden,
                    self.controller_dim,
                    topdown,
                )
    ...
    def forward(
        ...
        if self.adaptive_topdown is not None:
            memory = memory + self.adaptive_topdown(memory, content, controller)
