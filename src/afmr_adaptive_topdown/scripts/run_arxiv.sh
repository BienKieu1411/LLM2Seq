...
# Prepare, train and evaluate adaptive top-down AFMR on one or two GPUs.
...
if [[ -n "${AFMR_OUTPUT_DIR:-}" ]]; then
  ...
  OUTPUT_DIR="${ROOT}/runs/afmr_adaptive_topdown/arxiv_direct_projection_${COPY_VARIANT}"
elif [[ "${AFMR_CONTEXTUAL_VALUE}" == true ]]; then
  OUTPUT_DIR="${ROOT}/runs/afmr_adaptive_topdown/arxiv_local_topdown_value_${COPY_VARIANT}"
else
  OUTPUT_DIR="${ROOT}/runs/afmr_adaptive_topdown/arxiv_value_anchor_${COPY_VARIANT}"
fi
...
echo "=== AFMR adaptive top-down ArXiv ==="
...
echo "Adaptive regional top-down keys: $([[ "${AFMR_BRIDGE_MODE}" == afmr ]] && echo true || echo false)"
