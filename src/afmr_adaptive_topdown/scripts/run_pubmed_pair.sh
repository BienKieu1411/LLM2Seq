...
AFMR_ARCHITECTURE="${AFMR_ARCHITECTURE:-afmr_adaptive_topdown}"
...
if [[ -n "${AFMR_OUTPUT_DIR:-}" ]]; then
  ...
  RUN_ROOT="${ROOT}/runs/afmr_adaptive_topdown/pubmed_pair_${AFMR_ARCHITECTURE}_local_topdown_value_${COPY_VARIANT}"
elif [[ "${BRIDGE_VARIANT}" == afmr && "${AFMR_SALIENCE_WEIGHT}" != 0 && "${AFMR_SALIENCE_WEIGHT}" != 0.0 ]]; then
  RUN_ROOT="${ROOT}/runs/afmr_adaptive_topdown/pubmed_pair_${AFMR_ARCHITECTURE}_evidence_prior_${COPY_VARIANT}"
elif [[ "${BRIDGE_VARIANT}" == afmr ]]; then
  RUN_ROOT="${ROOT}/runs/afmr_adaptive_topdown/pubmed_pair_${AFMR_ARCHITECTURE}_${COPY_VARIANT}"
else
  RUN_ROOT="${ROOT}/runs/afmr_adaptive_topdown/pubmed_pair_${AFMR_ARCHITECTURE}_${BRIDGE_VARIANT}_${COPY_VARIANT}"
fi
GENERATED_CONFIG_DIR="${RUN_ROOT}/configs"
LOG_DIR="${ROOT}/logs/afmr_adaptive_topdown"
...
[[ "${AFMR_ARCHITECTURE}" == afmr_adaptive_topdown || "${AFMR_ARCHITECTURE}" == afmr_value_anchor || "${AFMR_ARCHITECTURE}" == afmr_v1 ]] || die "Unsupported AFMR_ARCHITECTURE"
[[ "${AFMR_BRIDGE_MODE}" == afmr || "${AFMR_BRIDGE_MODE}" == direct_projection ]] || die "AFMR_BRIDGE_MODE must be afmr or direct_projection"

echo "=== AFMR adaptive top-down PubMed sequential benchmark ==="
echo "=== GPU: CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} ==="
echo "=== Architecture: ${AFMR_ARCHITECTURE}; FP32 updates, BF16 compute ==="
echo "=== Bridge mode: ${AFMR_BRIDGE_MODE} ==="
echo "=== Adaptive regional top-down keys: $([[ "${AFMR_BRIDGE_MODE}" == afmr && "${AFMR_ARCHITECTURE}" == afmr_adaptive_topdown ]] && echo true || echo false) ==="
