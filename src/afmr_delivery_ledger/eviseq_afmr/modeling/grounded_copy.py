...
class GroundedCopyHead(nn.Module):
    ...
    def prepare(
        ...
        *,
        ...
        if source_region_ids is not None:
            ...
            region_ids = torch.where(valid_copy, region_ids, -1)
    ...
    def distribution(self, hidden: torch.Tensor, state: CopyState, region_prior: torch.Tensor | None = None):
        ...
        if region_prior is not None:
            if state.region_ids is None:
                raise ValueError("copy region prior requires mapped source regions")
            if region_prior.shape[:2] != hidden.shape[:2]:
                raise ValueError("copy region prior must match decoder time steps")
            if bool((state.region_ids[state.mask] >= region_prior.shape[-1]).any()):
                raise ValueError("copy region ID exceeds delivery-ledger region count")
            scores = scores + region_prior.float().gather(
    ...
    def mix_logits(
        self, hidden: torch.Tensor, logits: torch.Tensor, state: CopyState, region_prior: torch.Tensor | None = None
    ) -> torch.Tensor:
        log_attention, log_copy, log_generate = self.distribution(hidden, state, region_prior)
    ...
    def loss(
        ...
        def chunk_loss(states, targets, prior):
            log_attention, log_copy, log_generate = self.distribution(states, state, prior)
        ...
        for start in range(0, hidden.shape[1], stride):
            ...
            prior = None if region_prior is None else region_prior[:, start : start + stride]
            losses.append(
                checkpoint(chunk_loss, states, targets, prior, use_reentrant=False)
                if torch.is_grad_enabled()
                else chunk_loss(states, targets, prior)
            )
