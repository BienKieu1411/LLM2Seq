...
from .delivery_ledger import SourceDeliveryLedger
...
class QwenCrossDecoder(nn.Module):
    def __init__(
        ...
        ledger_config = config.get("delivery_ledger", {})
        self.ledger_enabled = bool(ledger_config.get("enabled", False))
        self.delivery_ledger = (
            SourceDeliveryLedger(int(model_config.hidden_size), ledger_config) if self.ledger_enabled else None
        )
    ...
    def forward(
        ...
        if self.delivery_ledger is not None and (return_logits or use_cache or labels is None):
            ...
            output_hidden, region_prior, coverage = self.delivery_ledger(
                ...
                region_states,
                region_mask,
                None if active_mask is None else active_mask[:, -output_hidden.shape[1] :],
                coverage,
            )
            if use_cache:
                self.delivery_ledger.set_coverage(coverage)
        logits = self.lm_head(output_hidden) if return_logits else None
        if logits is not None and self.grounded_copy is not None:
            logits = self.grounded_copy.mix_logits(output_hidden, logits, copy_state, region_prior)
        loss = None
        if labels is not None:
            ...
            if logits is not None:
                ...
                flat_hidden = ledger_hidden.reshape(-1, hidden.shape[-1])
    ...
    def prepare_cross_cache(self, memory: torch.Tensor, value_memory: Optional[torch.Tensor] = None) -> None:
        if self.delivery_ledger is not None:
            self.delivery_ledger.clear_state()
    ...
    def select_cross_cache(self, indices: torch.Tensor) -> None:
        if self.delivery_ledger is not None:
            self.delivery_ledger.select_state(indices)
        for layer in self.backbone.layers:
    ...
    def set_backbone_trainable(self, trainable: bool) -> None:
        ...
        if not trainable:
            if self.delivery_ledger is not None:
                self.delivery_ledger.requires_grad_(True)
