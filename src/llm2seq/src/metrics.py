...
def heter_sum_graph_rouge(
    ...
    *,
    ...
    """Return HeterSumGraph-compatible macro ROUGE F1 scores x 100.

    HeterSumGraph uses ``rouge==1.0.0`` with ``Rouge().get_scores(...,
    avg=True)`` and expects lowercase, whitespace-tokenized input. NFC and
    lowercase make that protocol deterministic for Vietnamese text.
    """
    ...
    if include_rouge_lsum:
        # rouge==1.0.0 exposes sentence-aware rouge-l but no separate Lsum.
