#!/usr/bin/env python3
"""Upload LLM2Seq run artifacts to Hugging Face Hub."""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Example: username/llm2seq-wikilingua. Defaults to HF_REPO_ID from env.",
    )
    ...
    if not args.repo_id:
        raise EnvironmentError("HF_REPO_ID is not set. Put it in llm2seq/env.txt or pass --repo_id.")

    token = os.environ.get("HF_TOKEN")
    if not token:
        raise EnvironmentError("HF_TOKEN is not set. Put it in llm2seq/env.txt or export it on the server.")
