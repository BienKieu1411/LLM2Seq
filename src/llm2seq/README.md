# LLM2Seq LLM2Seq Version

LLM2Seq version for long-context WikiLingua summarization.

This folder is self-contained for server upload. It includes the LLM2Seq training
entrypoints/configs and a bundled copy of the core `llm2seq` package under
`llm2seq/llm2seq`. It implements the architecture discussed for reducing
...
- Bigger decoder for LLM2Seq: 8 layers, hidden size 1024, 16 heads.
- Long context: source 4096 tokens, target 512 tokens.
- Three training phases.

## Architecture Overview

![LLM2Seq Architecture](/Users/kieugiangbien/Downloads/Project/Encoder-Decoder LLM/llm2seq/figures/image.png)
...
Upload the whole `llm2seq/` folder to the server. You do not need to
upload the parent repo or a separate `llm2seq/` folder.

Key files included:

```text
llm2seq/requirements.txt
llm2seq/llm2seq/                 # bundled core package
llm2seq/configs/*.yaml
llm2seq/scripts/*.sh
llm2seq/scripts/*.py
llm2seq/run_pipeline.sh
llm2seq/install_deps.sh
llm2seq/smoke_check.sh
```

Recommended server commands:

```bash
cd /path/containing/llm2seq
bash llm2seq/smoke_check.sh
bash llm2seq/install_deps.sh
cp llm2seq/env.example.txt llm2seq/env.txt
nano llm2seq/env.txt
bash llm2seq/run_pipeline.sh
```
...
The scripts prepend `llm2seq/` to `PYTHONPATH`, so training/evaluation
uses the bundled package rather than requiring a package installed elsewhere.

For LLM2Seq servers with NVIDIA driver CUDA 12.4, `install_deps.sh` installs a
...
```text
llm2seq/wikilingua/train.json
llm2seq/wikilingua/val.json
llm2seq/wikilingua/test.json
```
...
`llm2seq/data/processed/`. They are not required in the upload folder
...
```bash
llm2seq/configs/wikilingua_phase1.yaml
```
...
```bash
llm2seq/configs/wikilingua_phase2.yaml
```
...
```bash
llm2seq/configs/wikilingua_phase3.yaml
```
...
```bash
python llm2seq/scripts/prepare_wikilingua_json.py \
  --input_dir llm2seq/wikilingua \
...
Fill `HF_TOKEN` and `HF_REPO_ID` in `llm2seq/env.txt` if you want
automatic upload to Hugging Face. Do not share `env.txt` because it contains
secrets.

The LLM2Seq configs enable Hugging Face uploads by default. Training reads
...
```bash
bash llm2seq/scripts/train_phase1.sh
```

Default for LLM2Seq: 4 epochs, micro-batch size 12, gradient accumulation 4,
effective batch 48. Long-context 4096-token training is memory dominated, so
micro-batch is kept below the point that fills the whole LLM2Seq.

Phase 2:

```bash
bash llm2seq/scripts/train_phase2.sh runs/llm2seq_llm2seq_phase1_warmup/best.pt
```
...
fine-tuning and is suitable for LLM2Seq/H100-class runs, though 4096-token context
still keeps micro-batch small.

Phase 3:

```bash
bash llm2seq/scripts/train_phase3.sh runs/llm2seq_llm2seq_phase2_lora_encoder/best.pt
```

Default for LLM2Seq: 4 epochs, cascaded 4-head MTP-D, micro-batch size 8,
...
```text
checkpoints/llm2seq_phase1_warmup/epochs/epoch_XXX_step_YYY.pt
checkpoints/llm2seq_phase2_lora_encoder/epochs/epoch_XXX_step_YYY.pt
checkpoints/llm2seq_phase3_mtp_self_distill/epochs/epoch_XXX_step_YYY.pt
```
...
```text
checkpoints/llm2seq_phase1_warmup/best.pt
checkpoints/llm2seq_phase2_lora_encoder/best.pt
checkpoints/llm2seq_phase3_mtp_self_distill/best.pt
```

Fallback behavior:

- `train_phase2.sh` first looks for local Phase 1 `best.pt`; if missing, it
  downloads `checkpoints/llm2seq_phase1_warmup/best.pt` from HF.
- `train_phase3.sh` first looks for local Phase 2 `best.pt`; if missing, it
  downloads `checkpoints/llm2seq_phase2_lora_encoder/best.pt` from HF.
...
```bash
bash llm2seq/scripts/run_llm2seq_pipeline.sh
```

The script automatically loads `llm2seq/env.txt` when that file exists.
You can also point to a different file with `ENV_FILE=/path/to/env.txt`.

Default evaluation folders:

```text
llm2seq/eval_outputs/full_test_phase1_main
llm2seq/eval_outputs/full_test_phase2_main
llm2seq/eval_outputs/full_test_phase3_main
llm2seq/eval_outputs/full_test_phase3_mtp_verified
llm2seq/eval_outputs/phase3_speed_comparison
```
...
```bash
python llm2seq/scripts/evaluate_full_test.py \
  --config llm2seq/configs/wikilingua_phase3.yaml \
  --checkpoint runs/llm2seq_llm2seq_phase3_mtp_self_distill/best.pt \
  --base_checkpoint runs/llm2seq_llm2seq_phase2_lora_encoder/best.pt \
  --test_file llm2seq/data/processed/test.jsonl \
  --output_dir llm2seq/eval_outputs/full_test_phase3_main \
...
python llm2seq/scripts/evaluate_full_test.py \
  ...
  --base_checkpoint runs/llm2seq_llm2seq_phase2_lora_encoder/best.pt \
  --test_file llm2seq/data/processed/test.jsonl \
  --output_dir llm2seq/eval_outputs/full_test_phase3_mtp_verified \
...
```text
llm2seq/eval_outputs/phase3_speed_comparison/speed_comparison.json
```
...
```bash
python llm2seq/scripts/push_to_hf.py \
  --folder runs/llm2seq_llm2seq_phase1_warmup \
  --folder runs/llm2seq_llm2seq_phase2_lora_encoder \
  --folder runs/llm2seq_llm2seq_phase3_mtp_self_distill \
  --folder llm2seq/eval_outputs/full_test_phase1_main \
  --folder llm2seq/eval_outputs/full_test_phase2_main \
  --folder llm2seq/eval_outputs/full_test_phase3_main \
  --folder llm2seq/eval_outputs/full_test_phase3_mtp_verified \
  --folder llm2seq/eval_outputs/phase3_speed_comparison \
  --folder llm2seq/logs \
  --commit_message "Upload LLM2Seq LLM2Seq run"
```
...
- `encoder_torch_dtype: bfloat16` is intended for LLM2Seq/A100/H100-class GPUs.
- The LLM2Seq configs intentionally keep VRAM headroom instead of maxing out the
...
- For 8B encoders or multi-GPU LLM2Seq, the current trainer should eventually be
