...
AFMR_ARCHITECTURE="${AFMR_ARCHITECTURE:-layerwise_coupled_depth_kv}"
...
[[ "${AFMR_ARCHITECTURE}" == layerwise_coupled_depth_kv ]] || die "Unsupported AFMR_ARCHITECTURE"
[[ "${AFMR_BRIDGE_MODE}" == depth_kv || "${AFMR_BRIDGE_MODE}" == direct_projection ]] || die "AFMR_BRIDGE_MODE must be depth_kv or direct_projection"

echo "=== Layer-wise coupled-depth K/V PubMed benchmark ==="
