...
AFMR_BRIDGE_MODE="${AFMR_BRIDGE_MODE:-depth_kv}"
...
if [[ -n "${AFMR_OUTPUT_DIR:-}" ]]; then
  ...
  OUTPUT_DIR="${ROOT}/runs/layerwise_depth_kv/arxiv_direct_projection_${COPY_VARIANT}"
else
  OUTPUT_DIR="${ROOT}/runs/layerwise_depth_kv/arxiv_depth_kv_${COPY_VARIANT}"
fi
...
MAX_SOURCE_LENGTH="${MAX_SOURCE_LENGTH:-8192}"
...
LOG_DIR="${ROOT}/logs/layerwise_depth_kv"
...
[[ "${AFMR_BRIDGE_MODE}" == depth_kv || "${AFMR_BRIDGE_MODE}" == direct_projection ]] || die "AFMR_BRIDGE_MODE must be depth_kv or direct_projection"
...
if [[ ! -s "${DATA_DIR}/train.jsonl" || ! -s "${DATA_DIR}/validation.jsonl" || ! -s "${DATA_DIR}/test.jsonl" ]]; then
  ...
  echo "=== Preparing ArXiv ==="
  PYTHON="${PYTHON_BIN}" bash "${ROOT}/scripts/prepare_afmr.sh" "${prepare_args[@]}"
else
  echo "=== Prepared ArXiv data found; skipping preparation ==="
fi

GENERATED_CONFIG_DIR="${OUTPUT_DIR}/configs"
GENERATED_CONFIG="${GENERATED_CONFIG_DIR}/afmr_arxiv.yaml"
...
print(f"Materialized ArXiv config: {destination}")
PY

echo "=== Layerwise coupled-depth K/V ArXiv ==="
...
echo "Layerwise coupled-depth K/V: $([[ "${AFMR_BRIDGE_MODE}" == depth_kv ]] && echo true || echo false)"
