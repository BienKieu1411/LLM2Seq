...
def train(
    ...
    *,
    ...
    try:
        ...
        if _is_main_process():
            LOGGER.info(
                "model parameters=%s total=%d | bridge=%s layer_memories=%d | distributed=%s world_size=%d",
                counts,
                sum(p.numel() for p in model.parameters()),
                model.bridge.bridge_mode,
                len(model.decoder.backbone.layers) if model.bridge.bridge_mode == "depth_kv" else 0,
                _distributed_active(),
