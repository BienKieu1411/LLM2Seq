...
def architecture_spec(config: dict[str, Any]) -> dict[str, Any]:
    ...
    spec = {
        "graph_version": "layerwise_coupled_depth_kv_v1",
        "architecture": arch.get("name"),
        "bridge_mode": str(arch.get("bridge_mode", "depth_kv")),
        "controller_dim": int(arch.get("controller_dim", 0)),
        "depth_taps": int(arch.get("depth_taps", 0)),
        "depth_rank": int(arch.get("depth_rank", 0)),
        "depth_gate_init": float(arch.get("depth_gate_init", 0.0)),
        "depth_gate_max": float(arch.get("depth_gate_max", 0.0)),
        "residual_max_relative_rms": float(arch.get("residual_max_relative_rms", 0.0)),
        "final_tap_bias": float(arch.get("final_tap_bias", 0.0)),
