...
from .afmr import LayerwiseCoupledDepthBridge
...
class EviSeqAFMR(nn.Module):
    def __init__(self, config: dict[str, Any]):
        ...
        self.bridge = LayerwiseCoupledDepthBridge(
            self.encoder.hidden_size,
            int(self.decoder.config.hidden_size),
            len(self.decoder.backbone.layers),
            config["architecture"],
        )
    ...
    def encode_source(
        ...
        **copy_inputs: torch.Tensor,
        ...
        if bridge.memory.dtype != decoder_dtype:
            bridge = BridgeState(
                memory=bridge.memory.to(decoder_dtype),
                memory_mask=bridge.memory_mask,
                content_mask=bridge.content_mask,
                source_bias=bridge.source_bias,
                controller=bridge.controller,
                value_memory=None if bridge.value_memory is None else bridge.value_memory.to(decoder_dtype),
                value_residual=None if bridge.value_residual is None else bridge.value_residual.to(decoder_dtype),
                layer_memories=(
                    None
                    if bridge.layer_memories is None
                    else tuple(memory.to(decoder_dtype) for memory in bridge.layer_memories)
                ),
            )
    ...
    def forward(
        ...
        **copy_inputs: torch.Tensor,
        ...
        logits, _, loss_ce = self.decoder(
            ...
            layer_memories=bridge.layer_memories,
        )
        loss_salience = None
        total_loss = loss_ce
        if labels is not None and self.salience_loss_weight > 0 and self.bridge.bridge_mode == "depth_kv":
