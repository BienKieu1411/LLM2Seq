...
class DecoderLayerWithCross(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        encoder_layer_hidden_states: Optional[tuple[torch.Tensor, ...]] = None,
        ...
        **kwargs: Any,
        ...
        if encoder_hidden_states is not None:
            memory = (
                ...
                if encoder_layer_hidden_states is None
                else encoder_layer_hidden_states[self.index]
            )
            cross = self.cross(
...
class QwenCrossDecoder(nn.Module):
    ...
    def forward(
        ...
        value_residual: Optional[torch.Tensor] = None,
        layer_memories: Optional[tuple[torch.Tensor, ...]] = None,
        ...
        if layer_memories is not None and len(layer_memories) != len(self.backbone.layers):
            raise ValueError("layer_memories must contain one token-aligned memory per decoder layer")
        if layer_memories is not None and (value_memory is not None or value_residual is not None):
            raise ValueError("coupled-depth K/V cannot be combined with a separate value memory")
        position_ids = None
        ...
        outputs = self.backbone(
            ...
            encoder_layer_hidden_states=layer_memories,
    ...
    def prepare_cross_cache(
        ...
        for layer in self.backbone.layers:
            if isinstance(layer, DecoderLayerWithCross):
                selected = memory if layer_memories is None else layer_memories[layer.index]
                layer.prepare_cache(selected, value_memory, value_residual)
