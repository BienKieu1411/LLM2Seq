...
def validate_config(config: dict[str, Any]) -> None:
    ...
    _check_keys(
        architecture,
        {
            ...
            "depth_gate_init",
            "depth_gate_max",
            "residual_max_relative_rms",
            "final_tap_bias",
        },
        "architecture",
    )
    if architecture.get("name") != "layerwise_coupled_depth_kv":
        raise ValueError("architecture.name must be layerwise_coupled_depth_kv")
    if architecture.get("bridge_mode", "depth_kv") not in {"depth_kv", "direct_projection"}:
        raise ValueError("architecture.bridge_mode must be depth_kv or direct_projection")
    taps = int(architecture.get("depth_taps", 0))
    if taps <= 1:
        raise ValueError("architecture.depth_taps must be greater than one")
    for key in ("controller_dim", "depth_rank"):
        if int(architecture.get(key, 0)) <= 0:
            raise ValueError(f"architecture.{key} must be positive")
    if not 0 < float(architecture.get("depth_gate_init", 0)) < float(architecture.get("depth_gate_max", 0)) <= 1:
        raise ValueError("Require 0 < architecture.depth_gate_init < depth_gate_max <= 1")
    if not 0 < float(architecture.get("residual_max_relative_rms", 0)) <= 1:
        raise ValueError("architecture.residual_max_relative_rms must lie in (0, 1]")
    if not math.isfinite(float(architecture.get("final_tap_bias", 0))):
        raise ValueError("architecture.final_tap_bias must be finite")
    ...
    if salience_weight != 0:
        raise ValueError("layerwise coupled-depth K/V uses CE only; salience_loss_weight must be 0")
    data = config["data"]
