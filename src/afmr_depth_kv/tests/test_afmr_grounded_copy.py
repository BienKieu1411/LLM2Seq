...
def test_cached_copy_decoder_matches_full_prefix_after_compaction(dtype):
    ...
    with torch.no_grad():
        ...
        kwargs = dict(layer_memories=state.layer_memories, copy_state=state.copy_state)
        decoder.prepare_cross_cache(state.memory, layer_memories=state.layer_memories)
        ...
        cached_kwargs = dict(layer_memories=state.layer_memories, copy_state=state.copy_state.index_select(rows))
        step, _, _ = decoder(
            torch.tensor([[8]]),
            state.memory[rows],
        ...
        selected_memories = tuple(memory[rows] for memory in state.layer_memories)
        full, _, _ = decoder(
            torch.tensor([[6, 7, 8]]),
            state.memory[rows],
            state.memory_mask[rows],
            state.source_bias[rows],
            layer_memories=selected_memories,
