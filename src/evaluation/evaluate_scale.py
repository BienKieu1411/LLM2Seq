"""Evaluate source support with SCALE on local checkpoints.

This module follows the released SCALE inference path (Lattimer et al.,
EMNLP 2023): each prediction is split into sentences, the source is split
into overlapping token windows, and a Flan-T5 NLI prompt estimates the
probability of ``Yes`` for every window.  The best window supports each
prediction sentence; the document score is the mean of those sentence scores.

The implementation uses ``AutoModelForSeq2SeqLM`` and batched generation so a
local Hugging Face directory can be used without the older ``scale-score``
loader downloading a model.  The prompt, chunking rule, and Yes/No decision
are kept compatible with the released implementation.  No model or tokenizer
download is attempted.

``scale_consistency`` is a source-support score in ``[0, 1]`` and is
high-is-better.  ``unsupported_content_proxy = 1 - scale_consistency`` is
provided only as a monotonic, low-is-better diagnostic; it is not a calibrated
...
except ImportError:  # Direct ``python src/rouge155/evaluate_scale.py`` execution.
    from evaluate_alignscore import _attach_sources, _sentence_split, sentence_splitter_name
    from metric_io import EvaluationRow, load_jsonl, metric_output_path
...
def _sentence_split_fallback(text: str) -> list[str]:
    """Keep a deterministic fallback when the AlignScore helper is unavailable."""
...
def _build_chunks(
    ...
    *,
    ...
    """Build SCALE's source chunks, retaining the released token-window rule."""
    ...
    # The released implementation removes the pre-prompt EOS token before
    # concatenating the source chunk, then appends the post-prompt tokens.
    ...
    for index in range(num_windows):
        ...
        # Keep the final source EOS token in the post-prompt, as in SCALE's
        # released get_chunks implementation.
...
class LocalSCALE:
    """Batched, local-checkpoint implementation of the released SCALE score."""
