"""Shared input validation for text-generation metrics."""
...
class EvaluationRow:
    """One prediction and its reference(s), optionally with the source text."""
...
def canonical(value: Any) -> Any:
    """Normalize values used when binding metric output to input rows."""
...
def load_jsonl(
    ...
    *,
    ...
    """Load and validate prediction JSONL while preserving row order."""
...
def metric_output_path(predictions: Path, output: Path | None, suffix: str) -> Path:
    """Resolve a default metric output beside the prediction file."""
