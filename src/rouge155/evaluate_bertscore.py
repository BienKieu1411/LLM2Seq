"""Compute BERTScore with a local Transformers model and no network access."""
...
except ImportError:  # Direct ``python src/rouge155/evaluate_bertscore.py`` execution.
    from metric_io import EvaluationRow, load_jsonl, metric_output_path
...
def _infer_num_layers(model_path: Path) -> int:
    """Read the encoder depth for a local model when BERTScore cannot infer it."""

    try:
        from transformers import AutoConfig

        config = AutoConfig.from_pretrained(str(model_path), local_files_only=True)
    except Exception as exc:  # Transformers emits several model-specific exception types.
        raise ValueError(
...
def _as_unit_scores(values: Any, name: str) -> list[float]:
    ...
    # Cosine accumulation can exceed one by a few ulps on some torch kernels.
...
def _build_scorer(
    ...
    *,
    ...
    # The evaluator accepts a local directory only and must never silently fetch a model.
