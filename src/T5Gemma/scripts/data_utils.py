"""Dataset utilities shared by the T5Gemma train/eval entry points.

The maintained EviSeq pipeline stores prepared examples as ``id``, ``text``
and ``summary``.  Keeping the normalization and field fallback rules here
lets T5Gemma consume those files directly while retaining compatibility with
older ``source``/``target`` JSONL files.
"""
...
def canonicalize_record(row: Mapping[str, Any], data: Mapping[str, Any], *, index: int) -> SummarizationRecord:
    """Convert one row using the same defaults and fallbacks as EviSeq."""
...
def load_summarization_jsonl(
    ...
    *,
    ...
    """Load canonical EviSeq JSONL rows with strict, deterministic validation."""
...
def resolve_path(value: str | Path, *, base: Path) -> Path:
    """Resolve a path against the caller's directory, then the source tree."""
