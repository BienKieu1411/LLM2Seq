...
from .data.source_alignment import SOURCE_ALIGNMENT_KEYS
from .modeling.model import XOVModel
from .training.checkpoint import load_checkpoint
from .training.engine import XOVTrainer, seed_everything

LOGGER = logging.getLogger("xov.runtime")
...
def _init_distributed(device: str | None) -> tuple[torch.device, bool]:
    ...
    if device == "cpu" or not torch.cuda.is_available():
        initialized_here = not _distributed_active()
        if initialized_here:
            dist.init_process_group(backend="gloo", init_method="env://")
        return torch.device("cpu"), initialized_here
...
def _tokenizers(config: dict[str, Any]):
    ...
    encoder = (
        ...
        else AutoTokenizer.from_pretrained(str(model["encoder_name"]), local_files_only=True, **kwargs)
    )
    decoder = (
        _TinyTokenizer()
        if str(model["decoder_name"]) == "__tiny__"
        else AutoTokenizer.from_pretrained(str(model["decoder_name"]), local_files_only=True, **kwargs)
    )
...
def build_loaders(
    ...
    for name in selected:
        ...
        collator = SummarizationCollator(
            ...
            source_alignment=config["architecture"].get("bridge_mode", "cross_tokenizer_ordered_value")
...
def train(
    ...
    *,
    ...
    try:
        ...
        model = XOVModel(config).to(device=selected_device, dtype=torch.float32)
        counts = {
        ...
        if _distributed_active():
            model = DistributedDataParallel(
                ...
                output_device=selected_device.index if selected_device.type == "cuda" else None,
        ...
        trainer = XOVTrainer(model, config, selected_device)
        if checkpoint and _is_main_process():
            LOGGER.info("resumed XOV checkpoint: %s", checkpoint)
...
def evaluate(
    ...
    *,
    ...
    model = XOVModel(inference_config).to(device_obj)
