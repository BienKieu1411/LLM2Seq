"""Shared visible-source alignment for the lexical value route and optional copy head.

The copy_* tensor keys retain the existing copy-head wire format; requesting
these tensors does not enable grounded copying.
"""
...
def align_source_tokens(source: str, prefix_length: int, encoder_offsets, tokenizer: Any) -> dict[str, list]:
    spans = [
        (i, max(0, start - prefix_length), min(len(source), end - prefix_length))
        for i, (start, end) in enumerate(encoder_offsets)
        if end > max(start, prefix_length) and start < prefix_length + len(source)
    ]
    ...
    if "offset_mapping" not in encoded:
        raise ValueError("Source alignment requires a fast decoder tokenizer with offset mapping")
    ...
    ids, positions, enc_indices, token_indices, weights = [], [], [], [], []
    cursor = 0
    for position, (token, (start, end)) in enumerate(zip(encoded["input_ids"], encoded["offset_mapping"])):
    ...
    return dict(
        copy_token_ids=ids,
        copy_token_positions=positions,
...
def pad_source_alignments(rows: list[dict[str, list]]) -> dict[str, torch.Tensor]:
    result = {}
    for key in (
        "copy_token_positions",
        "copy_token_ids",
