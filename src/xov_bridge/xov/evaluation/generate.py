"""Greedy and sampled generation helpers with append-only JSONL resume."""
...
def generate_greedy(
    ...
    *,
    ...
    bridge = model.encode_source(
        ...
        **{key: batch[key] for key in SOURCE_ALIGNMENT_KEYS if key in batch},
    )
    ...
    try:
        model.decoder.prepare_cross_cache(bridge.memory, value_memory=bridge.value_memory)
        for step in range(max_new_tokens):
            ...
            logits, past, _ = model.decoder(
                ...
                attention_mask=decode_mask.index_select(0, active_rows),
            ...
            if (
                ...
                if surviving.numel() != active_rows.numel():
                    ...
                    bridge = bridge.index_select(surviving)
    finally:
...
def generate_sampled(
    ...
    *,
    ...
    """Sample one summary per source; training never calls this path."""
