...
from eviseq_xov.config import load_config, validate_config
from eviseq_xov.data.collate import SummarizationCollator
from eviseq_xov.data.schema import CanonicalRecord
from eviseq_xov.data.source_alignment import SOURCE_ALIGNMENT_KEYS
from eviseq_xov.data.source_alignment import align_source_tokens
from eviseq_xov.modeling.decoder import CopiedCrossAttention
from eviseq_xov.modeling.grounded_copy import GroundedCopyHead
from eviseq_xov.modeling.model import EviSeqXOV
from eviseq_xov.modeling.outputs import EncoderState
from eviseq_xov.modeling.xov import CrossTokenizerOrderedValueBridge
from eviseq_xov.evaluation.generate import generate_greedy
from eviseq_xov.runtime import _TinyTokenizer
from eviseq_xov.runtime import evaluate
...
def test_direct_projection_state_is_identical_and_xov_is_exact_noop_at_init() -> None:
    ...
    assert torch.equal(xov_out.value_memory, xov_out.memory)
...
def test_empty_alignment_is_identity_and_backward_graph_safe() -> None:
    ...
    output.value_memory.square().mean().backward()
    assert all(parameter.grad is None or parameter.grad.isfinite().all() for parameter in bridge.parameters())
...
def test_active_value_route_keeps_keys_and_copy_state_on_base_memory() -> None:
    ...
    state_a = copy_a.prepare(output.copy_memory, output.content_mask, _embedding(), **alignment)
    state_b = copy_b.prepare(output.memory, output.content_mask, _embedding(), **alignment)
...
def test_production_model_ce_steps_open_xov_gradients() -> None:
    config = load_config(CONFIG_ROOT / "xov_smoke.yaml")
    loader = __import__("eviseq_xov.runtime", fromlist=["build_loaders"]).build_loaders(config, max_train_examples=2)[
        "train"
    ]
...
def test_production_cached_generation_compacts_active_value_state() -> None:
    config = load_config(CONFIG_ROOT / "xov_smoke.yaml")
    from eviseq_xov.runtime import build_loaders
