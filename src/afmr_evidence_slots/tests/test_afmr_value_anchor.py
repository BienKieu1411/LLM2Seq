...
def test_value_anchor_remains_exact_h0_projection_with_evidence_slots():
    _, model, batch = _setup()
    model.eval()
    with torch.no_grad():
        encoder = model.encoder(batch["input_ids"], batch["attention_mask"], batch["source_content_mask"])
        bridge = _bridge(model, batch)
        expected = model.bridge.base_projection(encoder.final.float()).masked_fill(
            ~encoder.attention_mask.bool().unsqueeze(-1), 0
        )
    torch.testing.assert_close(bridge.value_memory, expected, rtol=0, atol=0)
