...
def _zero_evidence_key_output(
    _module: torch.nn.Module,
    _inputs: tuple[Any, ...],
    output: tuple[torch.Tensor, torch.Tensor],
) -> tuple[torch.Tensor, torch.Tensor]:
    if not isinstance(output, tuple) or len(output) != 2:
        raise TypeError("Evidence-slot probe expected (key_residual, source_prior)")
    ...
    if zero_residuals:
        for name in ("depth_out", "feature_up"):
            ...
            if module is not None:
                ...
                hooked.append(name)
        evidence_slots = getattr(model.bridge, "evidence_slots", None)
        if evidence_slots is not None:
            handles.append(evidence_slots.register_forward_hook(_zero_evidence_key_output))
            hooked.append("evidence_slots")
...
def run_probe(
    ...
    *,
    ...
    return {
        ...
        "note": "These are teacher-forced CE counterfactuals from one checkpoint; they are not retrained ablations or ROUGE evidence. Residual variants disable depth, feature, and evidence-slot key updates.",
