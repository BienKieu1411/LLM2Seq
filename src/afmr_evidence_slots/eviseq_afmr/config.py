...
def evidence_slot_settings(architecture: dict[str, Any]) -> dict[str, Any]:
    """Resolve and validate the controller-conditioned evidence-slot bridge."""
    defaults = {
        "enabled": False,
        "num_slots": 32,
        "dim": 256,
    ...
    supplied = architecture.get("evidence_slots", {})
    if not isinstance(supplied, dict):
        raise ValueError("architecture.evidence_slots must be a mapping")
    _check_keys(supplied, set(defaults), "architecture.evidence_slots")
    settings = {**defaults, **supplied}
    for key in ("enabled", "prior_enabled"):
        if not isinstance(settings[key], bool):
            raise ValueError(f"architecture.evidence_slots.{key} must be a boolean")
    for key in ("num_slots", "dim", "num_heads", "rank", "refine_layers"):
        value = settings[key]
        if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
            raise ValueError(f"architecture.evidence_slots.{key} must be a positive integer")
    if settings["num_slots"] < 2:
        raise ValueError("architecture.evidence_slots.num_slots must be at least 2")
    if settings["dim"] % settings["num_heads"]:
        raise ValueError("architecture.evidence_slots.dim must be divisible by num_heads")
    for initial, maximum in (("gate_init", "gate_max"), ("prior_init", "prior_max")):
        settings[initial] = float(settings[initial])
        settings[maximum] = float(settings[maximum])
        if not 0.0 < settings[initial] < settings[maximum] <= 1.0:
            raise ValueError(f"Require 0 < architecture.evidence_slots.{initial} < {maximum} <= 1")
    if architecture.get("bridge_mode", "afmr") == "direct_projection":
        settings["enabled"] = False
    if settings["enabled"] and architecture.get("name") != "afmr_value_anchor":
        raise ValueError("evidence_slots requires architecture.name=afmr_value_anchor")
    return settings
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    evidence_slot_settings(architecture)
