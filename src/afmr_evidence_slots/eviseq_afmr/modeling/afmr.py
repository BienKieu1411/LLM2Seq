...
from ..config import contextual_value_settings, evidence_slot_settings
from .contextual_value import ContextualValueBridge
from .controller import FocusController
from .evidence_slots import EvidenceSlotBridge
...
class AdaptiveFullMemoryResidualBridge(nn.Module):
    def __init__(self, encoder_hidden: int, decoder_hidden: int, config: dict, gradient_checkpointing: bool = False):
        ...
        slot_settings = evidence_slot_settings(config)
        self.evidence_slots = None
        if slot_settings["enabled"]:
            self.evidence_slots = EvidenceSlotBridge(
                self.encoder_hidden,
                self.decoder_hidden,
                self.controller_dim,
                slot_settings,
    ...
    def forward(
        ...
        if self.evidence_slots is not None:
            slot_residual, slot_prior = self.evidence_slots(final.float(), content, controller, memory)
            memory = memory + slot_residual
            source_bias = source_bias + slot_prior
