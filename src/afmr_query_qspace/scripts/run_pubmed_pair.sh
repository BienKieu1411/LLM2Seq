...
if [[ "${AFMR_BRIDGE_MODE}" == direct_projection && -z "${AFMR_REGION_QUERY:-}" ]]; then
  echo "Set AFMR_REGION_QUERY=true or false explicitly for the direct_projection ablation" >&2
  exit 1
fi
AFMR_REGION_QUERY="${AFMR_REGION_QUERY:-true}"
...
[[ "${AFMR_REGION_QUERY}" == true || "${AFMR_REGION_QUERY}" == false ]] || { echo "AFMR_REGION_QUERY must be true or false" >&2; exit 1; }
...
elif [[ "${BRIDGE_VARIANT}" == afmr && "${AFMR_REGION_QUERY}" == true && "${AFMR_SALIENCE_WEIGHT}" != 0 && "${AFMR_SALIENCE_WEIGHT}" != 0.0 ]]; then
  RUN_ROOT="${ROOT}/runs/afmr/pubmed_pair_${AFMR_ARCHITECTURE}_query_qspace_evidence_prior_${COPY_VARIANT}"
elif [[ "${AFMR_REGION_QUERY}" == true ]]; then
  RUN_ROOT="${ROOT}/runs/afmr/pubmed_pair_${AFMR_ARCHITECTURE}_${BRIDGE_VARIANT}_query_qspace_${COPY_VARIANT}"
...
LOG_FILE="${LOG_DIR}/pubmed_pair_${BRIDGE_VARIANT}_region_${AFMR_REGION_QUERY}_$(date +%Y%m%d_%H%M%S)_$$.log"
...
echo "=== Decoder-query region bridge: ${AFMR_REGION_QUERY} ==="
...
make_config() {
  ...
  local output_dir="$4"
  "${PYTHON_BIN}" - "${base_config}" "${output_config}" "${encoder_name}" "${DECODER_MODEL}" "${output_dir}" "${PROCESSED_DATA_DIR}" "${AFMR_ARCHITECTURE}" "${AFMR_BRIDGE_MODE}" "${AFMR_GROUNDED_COPY}" "${AFMR_CONTEXTUAL_VALUE}" "${AFMR_REGION_QUERY}" "${AFMR_SALIENCE_WEIGHT}" <<'PY'
import sys
...
base, destination, encoder, decoder, output_dir, data_dir, architecture, bridge_mode, grounded_copy, context, region_query, salience_weight = sys.argv[1:]
...
config["architecture"].setdefault("region_query", {})["enabled"] = region_query == "true"
