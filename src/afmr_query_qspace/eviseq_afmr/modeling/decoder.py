...
from .region_query import RegionQueryRead
...
class CopiedCrossAttention(nn.Module):
    def __init__(
        ...
        region_query_config: Optional[dict[str, Any]] = None,
        region_layer_index: int = 0,
    ):
        super().__init__()
        ...
        self.region_read = None
        if region_query_config and region_query_config.get("enabled", False):
            # Isolate regional initialization from the shared A/B/C RNG
            # stream, while giving each decoder layer distinct projections.
            with torch.random.fork_rng(devices=[]):
                layer_seed = (torch.initial_seed() + int(region_layer_index) + 1) % (2**63 - 1)
                torch.set_rng_state(torch.Generator(device="cpu").manual_seed(layer_seed).get_state())
                self.region_read = RegionQueryRead(
                    self.hidden_size,
                    int(region_query_config["rank"]),
                    float(region_query_config["gate_init"]),
                    float(region_query_config["gate_max"]),
                    num_query_heads=self.num_heads,
                    query_head_dim=self.head_dim,
                ).to(device=self.q_proj.weight.device, dtype=self.q_proj.weight.dtype)
    ...
    def forward(
        ...
        q_base = self.q_proj(query_states).view(batch, query_length, self.num_heads, self.head_dim)
        if self.region_read is not None:
            if region_keys is None or region_values is None or region_mask is None:
                raise ValueError("Region query is enabled but pooled source regions are missing")
            q_base = q_base + self.region_read(query_states, q_base, region_keys, region_values, region_mask)
        query = self.q_norm(q_base).transpose(1, 2)
        key, value = (
            self._cache
    ...
    def prepare_cache(
        ...
        if self.region_read is not None:
            if region_keys is None or region_values is None or region_mask is None:
                raise ValueError("Region query is enabled but pooled source regions are missing")
            self.region_read.prepare_cache(region_keys, region_values, region_mask)

    def clear_cache(self) -> None:
        self._cache = None
        if self.region_read is not None:
            self.region_read.clear_cache()

    def select_cache(self, indices: torch.Tensor) -> None:
        if self._cache is not None:
            self._cache = tuple(value.index_select(0, indices) for value in self._cache)
        if self.region_read is not None:
            self.region_read.select_cache(indices)
...
class DecoderLayerWithCross(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        encoder_region_keys: Optional[torch.Tensor] = None,
        encoder_region_values: Optional[torch.Tensor] = None,
        ...
        **kwargs: Any,
        ...
        if encoder_hidden_states is not None:
            cross = self.cross(
                ...
                region_keys=encoder_region_keys,
                region_values=encoder_region_values,
    ...
    def prepare_cache(
        ...
        self.cross.prepare_cache(memory, value_memory, value_residual, region_keys, region_values, region_mask)

    def clear_cache(self) -> None:
        self.cross.clear_cache()

    def select_cache(self, indices: torch.Tensor) -> None:
        self.cross.select_cache(indices)
...
class QwenCrossDecoder(nn.Module):
    ...
    def forward(
        ...
        outputs = self.backbone(
            ...
            encoder_region_keys=region_keys,
            encoder_region_values=region_values,
    ...
    def prepare_cross_cache(
        ...
        for layer in self.backbone.layers:
            if isinstance(layer, DecoderLayerWithCross):
                layer.prepare_cache(memory, value_memory, value_residual, region_keys, region_values, region_mask)
