...
This comparison is based on the implementations and canonical YAML files, not
on an assumed score improvement. EviSeq is a stronger and cleaner hypothesis;
only matched validation and locked-test runs can show that it is a stronger
model.
...
5. **The paper claim is narrower and testable.** The main experiment has one
   ...
   WikiLingua, CNN/DailyMail, and PubMed, saves only `last.pt`, and locks the
   final test protocol. This separates encoder conversion, evidence guidance,
...
- The decisive evidence is the matched `c0`, `c2`, `c3-no-cl`, and `c3`
  validation table followed by one locked full-test evaluation. If `c3` does
