        ...
        if (
            ...
            if unit_count > 0:
                ...
                if self.evidence_contrastive_mode == "sentence_aligned":
                    ...
                    # The last prompt state is target-free but task-aware.
                    # Fuse it with a pooled bridge-memory source context so a
                    # shared instruction cannot collapse all document queries
                    # to the same vector.  Both inputs are available before
                    # the first greedy summary token is generated.
                    assert self.prompt_conditioned_evidence_head is not None
                    prompt_repr = last_prompt_states(states, labels)
                    source_mask = attention_mask.bool() & unit_ids.gt(0)
                    source_context = masked_mean_pool(bridge.memory, source_mask)
                    q, k = self.prompt_conditioned_evidence_head(prompt_repr, source_context, sentence_reprs)
                    ...
                    evi_results["prompt_context_gate"] = self.prompt_conditioned_evidence_head.context_gate().detach()
                else:
