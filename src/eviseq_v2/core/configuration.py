...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    if use_evidence_contrastive:
        ...
        if evidence_mode not in {"document", "sentence_aligned", "prompt_conditioned"}:
            raise ValueError(
                "objectives.evidence_contrastive_mode must be document, sentence_aligned, or prompt_conditioned"
            )
        ...
        prompt_context_gate = float(objectives.get("evidence_prompt_context_gate_init", 0.5))
        if evidence_mode == "prompt_conditioned" and not 0.0 < prompt_context_gate < 1.0:
            raise ValueError("objectives.evidence_prompt_context_gate_init must be in (0, 1)")
