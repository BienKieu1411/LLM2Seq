...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    # --- Document-level contrastive (V1, off by default in V2) ---
    ...
    if any(float(objectives.get(name, 0.0)) != 0.0 for name in forbidden):
        raise ValueError("EviSeq V2 permits CE, salience, evidence contrastive, and optional document InfoNCE only")

    # --- Candidate ranking (V2 new) ---
    ...
    if ranking_enabled:
        ...
        if float(ranking.get("length_penalty_alpha", 0.0)) <= 0.0:
            raise ValueError("ranking.length_penalty_alpha must be positive")
