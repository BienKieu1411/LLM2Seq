...
LOGGER = logging.getLogger("eviseq.runtime")
...
) -> Tuple[LLM2SeqV2, Any, Any, SummarizationDataset | None, SummarizationDataset | None]:
    encoder_tokenizer, decoder_tokenizer = _tokenizers(config)
    model = LLM2SeqV2(config)
    data = config["data"]
...
def _component_lrs(training: Dict[str, Any], stage: str) -> Dict[str, float]:
    ...
    if stage == "ranking_finetune":
        return {
            "encoder": float(training.get("ranking_encoder_lr", 1e-6)),
            "adapter": float(training.get("ranking_adapter_lr", 5e-6)),
            "decoder": float(training.get("ranking_decoder_lr", 2e-6)),
            "cross_attention": float(training.get("ranking_cross_attention_lr", 5e-6)),
        }
    return {
