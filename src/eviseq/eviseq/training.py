"""EviSeq training entry point built on the audited V2 last-only runner."""
...
def _contrastive_scale(
    ...
    """Linearly introduce InfoNCE during interface warm-up, as in V3."""
...
def _run_stage(
    ...
    """V2 runner plus V3's audited contrastive ramp and diagnostics."""
...
def _parameter_component(name: str) -> str:
    if name.startswith("adapter.") or name.startswith("alignment_head."):
        return "adapter"
...
def train(config_path: str, overwrite_output_dir: bool = False) -> Path:
    ...
    try:
        return stable.train(config_path, overwrite_output_dir)
    finally:
