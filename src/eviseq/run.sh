...
train_and_validate() {
  ...
  "$PYTHON_BIN" -m eviseq.training --config "$config" "$@"
  "$PYTHON_BIN" -m eviseq.evaluate \
...
wiki_dev_table() {
  ...
  "$PYTHON_BIN" -m eviseq.dev_table \
    ...
    --output "runs/eviseq/wikilingua_dev_ablation_table.json"
}
...
case "$MODE" in
  test)
    ...
    ;;
  inspect)
    "$PYTHON_BIN" -m eviseq.inspect_config --config "${1:-$WIKI_CONFIG}"
    ;;
  audit-data)
    "$PYTHON_BIN" -m eviseq.data_integrity --config "${1:-$WIKI_CONFIG}"
    ;;
  smoke)
  ...
  compare-paper)
    ...
    OUTPUT="${6:-}"
    COMMAND=(
      "$PYTHON_BIN" -m eviseq.paper_compare
      --config "$CONFIG"
  ...
  prepare-cnndm)
    ...
    "$PYTHON_BIN" -m eviseq.prepare_cnndm \
  ...
  prepare-pubmed)
    ...
    "$PYTHON_BIN" -m eviseq.prepare_pubmed \
      --input-dir "$SOURCE_DIR" \
...
EviSeq (run from this folder or from src; no upload/push command exists)
  ...
  bash eviseq/run.sh nemotron --overwrite-output-dir
  bash eviseq/run.sh c0|c2|c3-no-cl --overwrite-output-dir
...
Final test is intentionally separate and allowed only on the complete split:
  ...
  bash eviseq/run.sh rouge155 runs/.../last_test_predictions.jsonl --details
  bash eviseq/run.sh bootstrap CANDIDATE_ROUGE155 BASELINE_ROUGE155 OUTPUT_JSON
  bash eviseq/run.sh compare-paper RESOLVED_CONFIG CANDIDATE_ROUGE \
