...
from eviseq.data_integrity import audit
...
def test_forbidden_objectives_fail_closed() -> None:
    ...
    for name in (
        ...
        with pytest.raises(ValueError, match="target-free prompt-source InfoNCE"):
...
def test_validation_is_default_and_test_requires_explicit_gate() -> None:
    source = (ROOT / "eviseq" / "evaluate.py").read_text(encoding="utf-8")
