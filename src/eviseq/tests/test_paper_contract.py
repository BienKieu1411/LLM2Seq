...
from eviseq.dev_table import build_table
from eviseq.evaluate import (
    _complete_paper_test,
    _load_verified_checkpoint,
    _reserve_paper_test,
)
from eviseq.paper_compare import compare
from eviseq.parameter_manifest import build_parameter_manifest
...
def _dev_artifacts(tmp_path: Path) -> dict[str, dict[str, Path]]:
    ...
    for index, (name, base) in enumerate(bases.items()):
        ...
        if fingerprint is None:
            from eviseq.data import dataset_fingerprint
