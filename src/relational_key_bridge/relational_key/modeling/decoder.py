...
def _load_decoder(
    ...
    if str(name) == "__tiny__":
        ...
        raw = AutoConfig.from_pretrained(name, trust_remote_code=trust_remote_code, local_files_only=True)
...
class CopiedCrossAttention(nn.Module):
    ...
    def _memory_kv(
        ...
        value_hidden = hidden if value_memory is None or value_memory is memory else self.memory_norm(value_memory)
...
class QwenCrossDecoder(nn.Module):
    def __init__(
        ...
        if int(config.get("cross_attention_every", 1)) != 1:
            raise ValueError("RelationalKey requires cross-attention in every decoder layer")
        ...
        for index, layer in enumerate(list(self.backbone.layers)):
            if not hasattr(layer, "self_attn"):
                raise ValueError("RelationalKey requires decoder layers with self_attn")
    ...
    def forward(
        ...
        if labels is not None and use_cache:
            raise ValueError("Training with labels requires use_cache=False")
