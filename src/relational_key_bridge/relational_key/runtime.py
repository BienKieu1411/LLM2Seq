...
from .training.engine import RelationalKeyTrainer, seed_everything

LOGGER = logging.getLogger("relational_key.runtime")
...
def train(
    ...
    *,
    ...
    try:
        ...
        model = RelationalKeyModel(config).to(device=selected_device, dtype=torch.float32)
        counts = {
        ...
        trainer = RelationalKeyTrainer(model, config, selected_device)
        if checkpoint and _is_main_process():
            LOGGER.info("resumed RelationalKey checkpoint: %s", checkpoint)
...
def evaluate(
    ...
    *,
    ...
    model = RelationalKeyModel(inference_config).to(device_obj)
