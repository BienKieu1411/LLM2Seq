...
def _get_field(row: dict[str, Any], key: str) -> Any:
    """Read a field, allowing dotted paths for nested local exports."""
...
def _records_from_value(value: Any, path: Path, line_number: int = 1) -> Iterator[dict[str, Any]]:
    if isinstance(value, dict):
        # A few dataset exporters wrap a complete split in {"data": [...]}; do
        # not unwrap ordinary records that happen to contain a data field.
        if len(value) == 1:
...
def _iter_json_records(path: Path) -> Iterator[dict[str, Any]]:
    """Stream JSONL and accept a whole JSON array/object in ``*.json`` files."""
    ...
    if path.suffix.lower() == ".json":
        try:
            ...
            # Some mirrors use a JSONL stream with a .json suffix.
            pass
        else:
            yield from _records_from_value(value, path)
            return

    # Keep JSONL streaming: GovReport records are long and the complete file
    # should never be materialized in memory during preparation.
    with path.open("r", encoding="utf-8-sig") as handle:
...
def _split_candidate_groups(dataset: str, split: str) -> tuple[tuple[str, ...], ...]:
    ...
    if dataset == "govreport":
        ...
        # The public GovReport release stores GAO and CRS records separately.
        # Prefer a flat pre-joined split when one is present, otherwise merge
        # both sources (or use whichever source the user downloaded).
        govreport = (
...
def _copy_raw(source: Path, raw_dir: Path, split: str) -> Path:
    del split  # Kept in the private signature for compatibility with old callers.
...
def _join_text(value: Any, separator: str, *, role: str) -> str:
    """Turn strings, lists and GovReport's nested section objects into text."""
    ...
    if isinstance(value, dict):
        ...
        # Preserve unknown textual fields while dropping structural metadata.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare summarization datasets for RelationalKey")
