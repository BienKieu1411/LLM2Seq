...
from .source_alignment import align_source_tokens, pad_source_alignments
...
class SummarizationCollator:
    def __init__(
        ...
        *,
        ...
        self.source_alignment = source_alignment or grounded_copy
    ...
    def _encode_source(self, record: CanonicalRecord, return_offsets: bool = False):
        try:
            ...
            raise ValueError("RelationalKey requires a fast encoder tokenizer with offset mapping") from exc
    ...
    def __call__(self, records: Sequence[CanonicalRecord]) -> dict[str, Any]:
        ...
        for record in records:
            ...
            if self.source_alignment:
                copy_rows.append(
                    align_source_tokens(record.source, len(self.encoder_prefix), offsets, self.decoder_tokenizer)
                )
        ...
        if self.source_alignment:
            result.update(pad_source_alignments(copy_rows))
        return result
