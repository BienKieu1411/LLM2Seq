#!/usr/bin/env python3
"""Convert local WikiLingua train/val/test JSON files to summarization JSONL."""
...
def normalize_records(raw: Any) -> List[Dict[str, Any]]:
    ...
    if isinstance(raw, dict):
        if "src" in raw or "source" in raw:
            return [raw]
        ...
        if values and all(isinstance(value, dict) for value in values):
            return values
    raise ValueError("Expected list[dict], dict with src/source, or dict of records")
...
def main() -> None:
    ...
    parser.add_argument("--output_dir", default="T5Gemma/data/processed")
    ...
    if not val_file.exists():
        raise FileNotFoundError(f"Missing val.json or validation.json in {input_dir}")
