\documentclass[aspectratio=169,11pt]{beamer}

\usetheme{SimplePlus}

\usepackage{iftex}
\ifPDFTeX
  \usepackage[T5,T1]{fontenc}
  \usepackage[utf8]{inputenc}
\else
  \usepackage{fontspec}
  \setmainfont{Avenir Next}
  \setsansfont{Avenir Next}
  \setmonofont{Avenir Next}
\fi

\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{array}
\usepackage{multirow}
\usepackage{colortbl}
\usepackage{amsmath}
\usepackage{tikz}
\usepackage{hyperref}
\usetikzlibrary{arrows.meta,positioning,fit}

\graphicspath{{figures/}}

\definecolor{LLMRed}{HTML}{A61C2A}
\definecolor{LLMDark}{HTML}{222222}
\definecolor{LLMBlue}{HTML}{BFCDE0}
\definecolor{LLMGreen}{HTML}{CDECCF}
\definecolor{LLMOrange}{HTML}{F4D6A0}
\definecolor{LLMLight}{HTML}{F7F7F7}

\setbeamercolor{alerted text}{fg=LLMRed}
\renewcommand{\familydefault}{\sfdefault}
\setbeamerfont{block title}{family=\sffamily,series=\bfseries,size=\small}
\setbeamerfont{block body}{family=\sffamily,size=\small}
\setbeamerfont{normal text}{family=\sffamily}
\hyphenpenalty=10000
\exhyphenpenalty=10000

\newcommand{\smallnote}[1]{\vspace{0.3em}{\scriptsize\color{gray}#1}}
\newcommand{\metric}[2]{\begin{block}{#1}\begin{minipage}[c][1.05cm][c]{\linewidth}\centering\bfseries #2\end{minipage}\end{block}}
\newcommand{\tightv}{\vspace{-0.45em}}

\title[LLM2Seq]{LLM2Seq: Repurposing LLM Encoders for Encoder-Decoder Summarization with Self-Distilled Multi-Token Prediction}
\subtitle{Vietnamese Summarization with Repurposed LLM Encoders and Verified MTP}
\author[Kiều Giang Biên]{Kiều Giang Biên}
\institute[HUST]{Hanoi University of Science and Technology}
\date{}

\begin{document}

\begin{frame}
  \titlepage
\end{frame}

\begin{frame}{Từ bài toán đến hướng tiếp cận}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.31\textwidth}
      \begin{block}{Bài toán}
        \begin{minipage}[t][1.35cm][t]{\linewidth}
        \raggedright Tóm tắt cần đọc văn bản nguồn rồi viết lại ngắn gọn.
        \end{minipage}
      \end{block}
    \end{column}
    \begin{column}{0.31\textwidth}
      \begin{block}{Khoảng trống}
        \begin{minipage}[t][1.35cm][t]{\linewidth}
        \raggedright Decoder-only LLM sinh văn bản tốt, nhưng không có giao diện encoder-decoder trực tiếp cho tóm tắt.
        \end{minipage}
      \end{block}
    \end{column}
    \begin{column}{0.31\textwidth}
      \begin{block}{Hướng làm}
        \begin{minipage}[t][1.35cm][t]{\linewidth}
        \raggedright Dùng LLM đọc nguồn. Adapter chuyển biểu diễn sang decoder.
        \end{minipage}
      \end{block}
    \end{column}
  \end{columns}

  \vfill
  \begin{center}
    \normalsize\alert{LLM2Seq giữ thông tin theo từng token, thay vì nén nguồn quá sớm.}

    \smallskip
    \small Decoder còn thấy chi tiết trong nguồn, đồng thời tận dụng biểu diễn mạnh từ LLM.
  \end{center}
\end{frame}

\begin{frame}{Vì sao không chỉ dùng nén ngữ cảnh?}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{block}{Nén ngữ cảnh}
        \begin{minipage}[t][3.25cm][t]{\linewidth}
        \begin{itemize}
          \item Nén văn bản dài thành chuỗi ẩn ngắn hơn.
          \item Hữu ích cho suy luận với ngữ cảnh dài.
          \item Không được thiết kế riêng cho tóm tắt bám nguồn.
        \end{itemize}
        \end{minipage}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{LLM2Seq}
        \begin{minipage}[t][3.25cm][t]{\linewidth}
        \begin{itemize}
          \item Giữ source memory theo từng token nguồn.
          \item Decoder đọc nguồn bằng cross-attention.
          \item Giảm nguy cơ mất tên riêng, số liệu và bước thao tác.
        \end{itemize}
        \end{minipage}
      \end{block}
    \end{column}
  \end{columns}

  \vspace{0.35em}
  \begin{center}
    \begin{tikzpicture}[node distance=0.52cm, every node/.style={font=\small}]
      \node[draw,rounded corners,fill=LLMBlue,minimum width=2.65cm,minimum height=0.62cm] (src1) {Văn bản nguồn};
      \node[draw,rounded corners,fill=LLMOrange,right=0.85cm of src1,minimum width=2.5cm,minimum height=0.62cm] (bottleneck) {Biểu diễn nén};
      \node[draw,rounded corners,fill=LLMGreen,right=0.85cm of bottleneck,minimum width=2.35cm,minimum height=0.62cm] (summary1) {Suy luận};
      \draw[-{Latex[length=2mm]},thick] (src1) -- (bottleneck);
      \draw[-{Latex[length=2mm]},thick] (bottleneck) -- (summary1);

      \node[draw,rounded corners,fill=LLMBlue,below=0.55cm of src1,minimum width=2.65cm,minimum height=0.62cm] (src2) {Văn bản nguồn};
      \node[draw,rounded corners,fill=LLMBlue,right=0.85cm of src2,minimum width=2.5cm,minimum height=0.62cm] (memory) {Token-level memory};
      \node[draw,rounded corners,fill=LLMGreen,right=0.85cm of memory,minimum width=2.35cm,minimum height=0.62cm] (decoder) {Decoder};
      \draw[-{Latex[length=2mm]},thick] (src2) -- (memory);
      \draw[-{Latex[length=2mm]},thick] (memory) -- (decoder);
    \end{tikzpicture}
  \end{center}
\end{frame}

\begin{frame}{LLM2Seq: tổng quan kiến trúc}
  \centering
  \includegraphics[width=0.98\textwidth,height=0.78\textheight,keepaspectratio]{architecture_crop.pdf}

  \smallnote{LLM đọc nguồn $\rightarrow$ adapter tạo source memory $\rightarrow$ decoder nhẹ $\rightarrow$ verified MTP.}
\end{frame}

\begin{frame}{Adapter: nối biểu diễn nguồn với decoder}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.53\textwidth}
      \begin{block}{Ý tưởng}
        \begin{itemize}
          \item Lấy hidden states từ LLM đọc nguồn.
          \item Trộn thông tin từ nhiều layer của LLM.
          \item Gated projection đưa biểu diễn sang không gian decoder.
          \item Decoder đọc source memory bằng cross-attention.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.43\textwidth}
      \begin{center}
      \includegraphics[width=0.94\linewidth,height=0.68\textheight,keepaspectratio]{adapter_flow_redrawn.pdf}
      \end{center}
    \end{column}
  \end{columns}

\end{frame}

\begin{frame}{Self-distilled MTP: tăng tốc có kiểm chứng}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{block}{Huấn luyện}
        \begin{minipage}[t][2.1cm][t]{\linewidth}
        \begin{itemize}
          \item Huấn luyện summarizer trước.
          \item Đóng băng encoder, adapter và decoder.
          \item Chỉ cập nhật 4 MTP heads.
        \end{itemize}
        \end{minipage}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Suy luận}
        \begin{minipage}[t][2.1cm][t]{\linewidth}
        \begin{itemize}
          \item MTP dự đoán vài token tiếp theo.
          \item Decoder kiểm chứng token nháp.
          \item Chỉ ghi token đã được xác nhận.
        \end{itemize}
        \end{minipage}
      \end{block}
    \end{column}
  \end{columns}

  \vfill
  \begin{center}
    \begin{tikzpicture}[node distance=0.7cm, every node/.style={font=\small}]
      \node[draw,rounded corners,fill=LLMBlue,minimum width=2.4cm,minimum height=0.72cm] (main) {Decoder chính};
      \node[draw,rounded corners,fill=LLMOrange,right=1.0cm of main,minimum width=2.5cm,minimum height=0.72cm] (draft) {Token nháp};
      \node[draw,rounded corners,fill=LLMGreen,right=1.0cm of draft,minimum width=2.5cm,minimum height=0.72cm] (verify) {Kiểm chứng};
      \node[draw,rounded corners,fill=LLMBlue,right=1.0cm of verify,minimum width=2.2cm,minimum height=0.72cm] (emit) {Ghi output};
      \draw[-{Latex[length=2mm]},thick] (main) -- (draft);
      \draw[-{Latex[length=2mm]},thick] (draft) -- (verify);
      \draw[-{Latex[length=2mm]},thick] (verify) -- (emit);
      \node[below=0.25cm of verify,color=LLMRed] {chỉ append token sau khi decoder chính xác nhận};
    \end{tikzpicture}
  \end{center}
\end{frame}

\begin{frame}{Dữ liệu: quy mô train/validation/test}
  \centering
  \includegraphics[width=0.96\textwidth,height=0.72\textheight,keepaspectratio]{dataset_split_redrawn.pdf}

  \vfill
  \small\alert{Hai bộ dữ liệu có quy mô train lớn hơn rõ rệt so với validation/test, phù hợp cho fine-tuning và đánh giá held-out.}
\end{frame}

\begin{frame}{Dữ liệu: hai nguồn tóm tắt tiếng Việt}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{block}{WikiLingua}
        \begin{itemize}
          \item Nguồn: WikiHow-style instructional articles.
          \item Split: 13,999 train / 1,680 validation / 3,901 test.
          \item Kiểm tra khả năng tóm tắt văn bản hướng dẫn nhiều bước.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{LR-Sum}
        \begin{itemize}
          \item Nguồn: VOA newswire.
          \item Split: 11,676 train / 1,459 validation / 1,460 test.
          \item Kiểm tra khả năng tóm tắt tin tức ngắn gọn hơn.
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}

  \vfill
  \begin{center}
    \small\alert{Dùng cả hai dataset giúp kiểm tra LLM2Seq trên hai kiểu văn bản khác nhau.}
  \end{center}
\end{frame}

\begin{frame}{Thiết lập thí nghiệm}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.30\textwidth}
      \begin{block}{Hệ thống so sánh}
        \begin{minipage}[t][3.05cm][t]{\linewidth}
        \begin{itemize}
          \item LLM2Seq-Llama
          \item LLM2Seq-Qwen
          \item T5Gemma LoRA
        \end{itemize}
        \end{minipage}
      \end{block}
    \end{column}
    \hfill
    \begin{column}{0.30\textwidth}
      \begin{block}{Thước đo}
        \begin{minipage}[t][3.05cm][t]{\linewidth}
        \begin{itemize}
          \item ROUGE-1/2/L
          \item BERTScore mBERT
          \item Thống kê độ dài
          \item Token/s và độ trễ
        \end{itemize}
        \end{minipage}
      \end{block}
    \end{column}
    \hfill
    \begin{column}{0.30\textwidth}
      \begin{block}{Dữ liệu}
        \begin{minipage}[t][3.05cm][t]{\linewidth}
        \begin{itemize}
          \item WikiLingua
          \item LR-Sum
        \end{itemize}
        \end{minipage}
      \end{block}
    \end{column}
  \end{columns}

  \vfill
  \begin{center}
    \normalsize\alert{Ba câu hỏi: chất lượng, độ dài, và tốc độ.}
  \end{center}
\end{frame}

\begin{frame}{Kết quả chính: chất lượng tóm tắt}
  \centering
  \includegraphics[width=0.98\textwidth,height=0.68\textheight,keepaspectratio]{quality_metrics.pdf}

  \vfill
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \small\alert{WikiLingua}: Qwen gần T5Gemma ở ROUGE-1, nhưng thấp hơn ở ROUGE-2 và ROUGE-L.
    \end{column}
    \begin{column}{0.48\textwidth}
      \small\alert{LR-Sum}: Trong cấu hình LLM2Seq, Qwen hơn Llama, nhưng T5Gemma vẫn mạnh nhất.
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Bảng kết quả chính}
  \centering
  \footnotesize
  \setlength{\tabcolsep}{5pt}
  \renewcommand{\arraystretch}{1.12}
  \begin{tabular}{llrrrr}
    \toprule
    Dataset & Model & R-1 & R-2 & R-L & BERT-F1 \\
    \midrule
    \multirow{3}{*}{WikiLingua} & LLM2Seq-Llama & 48.36 & 15.54 & 29.05 & 73.02 \\
    \rowcolor{LLMBlue!35} & LLM2Seq-Qwen & 54.01 & 20.83 & 31.83 & 73.42 \\
    \rowcolor{LLMGreen!35} & T5Gemma & \textbf{54.24} & \textbf{27.42} & \textbf{33.89} & \textbf{74.98} \\
    \midrule
    \multirow{3}{*}{LR-Sum} & LLM2Seq-Llama & 50.49 & 12.08 & 29.41 & 69.98 \\
    \rowcolor{LLMBlue!35} & LLM2Seq-Qwen & 51.59 & 15.58 & 31.21 & 71.62 \\
    \rowcolor{LLMGreen!35} & T5Gemma & \textbf{58.79} & \textbf{27.34} & \textbf{39.86} & \textbf{74.65} \\
    \bottomrule
  \end{tabular}

  \vfill
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{block}{Nhìn chung}
        \begin{minipage}[t][0.95cm][t]{\linewidth}
        Trong cấu hình LLM2Seq, Qwen tốt hơn Llama trên cả hai dataset.
        \end{minipage}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{So với T5Gemma}
        \begin{minipage}[t][0.95cm][t]{\linewidth}
        T5Gemma vẫn mạnh hơn, nhất là ROUGE-2 và ROUGE-L.
        \end{minipage}
      \end{block}
    \end{column}
  \end{columns}

\end{frame}

\begin{frame}{Kết quả độ dài: không chỉ nhìn ROUGE}
  \centering
  \includegraphics[width=0.90\textwidth,height=0.67\textheight,keepaspectratio]{length_behavior.pdf}

  \vfill
  \small\alert{Nhận xét}: Llama thường ngắn. Qwen gần độ dài tham chiếu hơn. T5Gemma dài hơn, giúp tăng overlap nhưng kém cô đọng hơn.
\end{frame}

\begin{frame}{MTP trên WikiLingua: tăng tốc vừa phải}
  \centering
  \includegraphics[width=0.98\textwidth,height=0.64\textheight,keepaspectratio]{mtp_speed.pdf}

  \vfill
  \includegraphics[width=0.96\textwidth,height=0.22\textheight,keepaspectratio]{qwen_mtp_observation.pdf}
\end{frame}

\begin{frame}{Vì sao MTP trên WikiLingua chưa tăng tốc mạnh?}
  \centering
  \includegraphics[width=0.98\textwidth,height=0.56\textheight,keepaspectratio]{mtp_latency_accounting.pdf}

  \vspace{0.2em}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \small
      \begin{block}{Nếu tính lý tưởng}
        \begin{minipage}[t][0.72cm][t]{\linewidth}
        2.30 token/bước cho latency lý tưởng 0.339s; thực tế là 0.662s.
        \end{minipage}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \small
      \begin{block}{Phần còn lại}
        \begin{minipage}[t][0.72cm][t]{\linewidth}
        Chênh 0.323s đến từ verification, cache, tensor control và Python overhead.
        \end{minipage}
      \end{block}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Demo hệ thống}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=0.73\textheight,keepaspectratio]{demo_autoregressive_cropped.pdf}

      \scriptsize Autoregressive
    \end{column}
    \begin{column}{0.49\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=0.73\textheight,keepaspectratio]{demo_mtp_cropped.pdf}

      \scriptsize Verified MTP
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Thảo luận và hạn chế}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{block}{Kết quả rút ra}
        \begin{minipage}[t][3.35cm][t]{\linewidth}
        \begin{itemize}
          \item Có thể dùng LLM làm phần đọc nguồn cho decoder.
          \item Trong cấu hình LLM2Seq, Qwen tốt hơn Llama trên cả hai dataset.
          \item Verified MTP giúp chạy nhanh hơn trên WikiLingua.
        \end{itemize}
        \end{minipage}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Hạn chế hiện tại}
        \begin{minipage}[t][3.35cm][t]{\linewidth}
        \begin{itemize}
          \item Chưa chạy đầy đủ ablation cho từng phần của adapter.
          \item Chưa có đánh giá thủ công về factuality / độ đúng nội dung.
          \item MTP vẫn tốn chi phí kiểm chứng và cache.
        \end{itemize}
        \end{minipage}
      \end{block}
    \end{column}
  \end{columns}

  \vfill
  \begin{center}
    \normalsize\alert{Trong cấu hình LLM2Seq, Qwen tốt hơn Llama, nhưng chưa vượt T5Gemma.}
  \end{center}
\end{frame}

\begin{frame}{Thông điệp chính}
  \vspace{0.3em}
  \begin{itemize}
    \item LLM2Seq chuyển hidden states từ LLM encoder thành source memory cho decoder.
    \item Trong cấu hình LLM2Seq, Qwen tốt hơn Llama trên WikiLingua và LR-Sum tiếng Việt.
    \item T5Gemma vẫn mạnh hơn, đặc biệt ở ROUGE-2 và ROUGE-L.
    \item Verified MTP đạt 1.17x trên WikiLingua, nhưng còn tốn chi phí kiểm chứng.
  \end{itemize}

  \vfill
  \begin{block}{Bước tiếp theo}
    Chạy thêm ablation cho adapter, đánh giá factuality / độ đúng nội dung, và tối ưu verified decoding.
  \end{block}
\end{frame}

\begin{frame}{Hỏi đáp}
  \centering
  \vspace{1.0em}
  {\Huge\bfseries Thank you}

  \vspace{0.45em}
  {\Large Q\&A}

  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=south east,align=right,xshift=-1.98cm,yshift=0.85cm] at (current page.south east) {
      \scriptsize GitHub:\\[-0.1em]
      \scriptsize\href{https://github.com/BienKieu1411/LLM2Seq}{github.com/BienKieu1411/LLM2Seq}
    };
    \node[anchor=south east,xshift=-0.45cm,yshift=0.54cm] at (current page.south east) {
      \href{https://github.com/BienKieu1411/LLM2Seq}{\includegraphics[width=1.25cm]{github_qr.pdf}}
    };
  \end{tikzpicture}
\end{frame}

\end{document}
